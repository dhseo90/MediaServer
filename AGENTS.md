...
9. 개발 작업에는 Superpowers 절차를 활용하되, 모델 선택과 위임은 1.3의 `아스트라 xhigh 책임자 + 단일 개발 담당자` 원칙을 따른다.
...
이 절은 사용자가 2026-09-06에 승인한 `Astra 책임자 + 단일 개발 담당자` 방식이다. 모델·위임에 관한 스킬의 일반 지침보다 이 절을 우선한다. 자동 주입된 지침이 일부여도 작업 시작 시 이 절과 모델 선정에 필요한 13장을 실제 파일에서 확인한다.

1. 사용자는 각 버전 시작 시 메인 에이전트를 `GPT-6 Astra` (`gpt-6-astra`), 추론 수준 `xhigh`로 선택한다. 메인은 로드맵 조율부터 승인된 개발 단계, 통합, 결과 보고까지 이 선택을 유지한다. 작업이 단순하거나 점수가 낮다는 이유로 메인 모델·추론 수준을 바꾸지 않는다. 다른 선택을 사용자가 명시하면 그 지시를 따른다.
...
6. 메인은 위임 전에 13장의 기준으로 **서브에이전트의 모델과 추론 수준**을 정하고, 사용 가능한 도구가 지원하는 정확한 값으로 지정한다. 상한은 `gpt-5.6-sol` / `xhigh`이며, 모든 작업에 이 상한을 기본값으로 쓰지는 않는다. 같은 작업 묶음에서 설정을 유지하고, 실행 중 변경 기능이 없으면 실제 변경한 것처럼 보고하지 않는다. 기존 에이전트로 처리할 수 없는 난이도이면 메인이 회수하며, 모델을 바꾸기 위해 두 번째 에이전트를 추가하지 않는다.
...
9. Superpowers의 설계, TDD, 원인 분석, 변경 검토와 완료 검증 절차는 작업에 맞게 활용한다. `메인은 직접 수정 금지`, `작업마다 새 에이전트 생성`, `별도 검토 에이전트 추가`, `비용 절감을 위한 메인 모델 하향` 규칙은 이 프로젝트에서 적용하지 않는다. 이 충돌은 본 절에 따라 해소하며 추가 에이전트나 모델 변경 승인을 반복 요청하지 않는다.
...
6. 사용자가 특정 모델과 추론 수준을 명시하면 그 지시를 우선하고, 추천 모델과 실제 사용 모델을 분리해 적는다. Codex 메인의 버전 시작 기준은 1.3의 `gpt-6-astra` / `xhigh`다.

### 13.1.1 Codex 메인과 서브에이전트 모델 기준

Codex 런타임에서만 사용한다. 역할과 상한은 1.3을 따르며, 메인은 `gpt-6-astra` / `xhigh`, 서브에이전트는 최대 `gpt-5.6-sol` / `xhigh`다. 모델 이름과 지원 추론 수준은 현재 실행 도구가 노출하는 목록으로 확인하며, 문서에 있다는 이유만으로 선택 가능하다고 보고하지 않는다.

- 공식 모델 참고: <https://developers.openai.com/api/docs/models/gpt-6-astra>
- `GPT-6 Astra` (`gpt-6-astra`): 메인 책임자 전용. 설계·계약·안전 경계·diff/evidence 검토·통합·최종 판단 및 1.3의 직접 수정 예외 담당. 서브에이전트 후보로 선택하지 않음
- `GPT-5.6 Sol` (`gpt-5.6-sol`): 여러 근거를 대조해야 하는 조사·검토와 확정된 계약에 따른 복합 구현. 서브에이전트 모델 상한
...
2. 범위가 작고 요구사항과 검증 방법이 분명한 개발은 `gpt-5.6-terra`의 `medium`을 출발점으로 삼고, 교차 검증이 필요하면 `high`를 검토한다.
3. 여러 모듈의 근거 대조나 의미 검토가 필요한 구현·조사는 `gpt-5.6-sol`의 `high`를 검토한다. 확정된 계약 안에서도 복잡한 저장·권한·미디어 경계의 구현과 교차 검증이 필요하면 `xhigh`를 검토하며, 구체적인 이유를 기록한다.
...
| 6~8점 | `5.6 Sol`; 계약 미확정·회수 사유가 있으면 메인 판단 선행 | `grok-4.6` | 복잡도, 영향, 불확실성, 검증 비용이 큰 작업 |

해당 패밀리의 13.1 작업 종류 초기 후보와 점수 결과가 다르면 필요한 판단 역량을 충족하는 쪽을 선택하고 근거를 적는다. 반복 작업이라는 이유만으로 정확도나 데이터 위험을 무시해 Codex `Luna` 또는 Grok `grok-4.5`로 낮추지 않는다. Codex에서는 메인 직접 처리 항목에도 점수와 근거를 기록하되 메인의 `Astra` / `xhigh` 선택을 낮추는 데 사용하지 않는다.

### 13.3 추론 수준 기준

추론 수준은 현재 모델과 실행 도구가 함께 지원하는 값만 사용한다. Codex 메인은 `xhigh`를 유지하고, 서브에이전트도 `xhigh`를 상한으로 한다. 아래 공통 용어표는 이 상한의 예외를 만들지 않는다. Grok의 모델·추론 수준은 13.0과 13.1.2를 따르고 Codex의 고정값·상한을 적용하지 않는다. 추론 수준과 하위 에이전트 생성 깊이를 혼동하지 않는다. Grok TUI `/effort`가 `low`/`medium`/`high`/`xhigh`만 받는 세션에서는 그 외 값을 TUI 추천값으로 쓰지 않는다.

| 추론 수준 | 한글 표기 | 적용 기준 |
| --- | --- | --- |
| `none` | 없음 | 의미 판단이 없고 결과가 완전히 결정적인 기계적 변환에만 사용 |
| `low` | 낮음 | 저위험 반복 작업이나 latency 우선 작업에 사용 |
| `medium` | 중간 | 요구사항이 명확한 서브에이전트 작업의 기본 출발점 |
| `high` | 높음 | 추가 추론이 실제 품질 향상을 만들 것으로 예상되거나 검증 경계가 여러 개인 작업 |
| `xhigh` | 매우 높음 | 사용자 지정 Codex 메인값이자 서브에이전트 상한. 서브에는 복합 구현·교차 검증처럼 추가 근거가 있을 때 적용 |
...
1. Codex 메인의 `xhigh`는 사용자 지정값이므로 작업마다 다시 정당화하거나 하향하지 않는다. 서브에이전트는 작업 난이도와 반환 결과의 검증 가능성에 맞춰 조절한다.
2. 서브에이전트에 `high` 이상을 지정할 때는 추가 추론이 필요한 구체적인 판단·검증 경계를 적는다. 단지 우선순위가 P0이거나 파일 수가 많다는 이유만으로 상향하지 않는다.
3. Codex 서브에이전트는 8점이거나 위험 상향 사유가 있어도 `max`·`ultra`를 사용하지 않는다. `Sol` / `xhigh` 상한 안에서 감당하기 어렵거나 기존 담당자의 설정으로 부족하면 메인이 회수한다. 메인 모델·추론 수준을 자동 상향하지 않는다. Grok의 `max`는 8점 또는 중대한 위험 사유와 추가 탐색 근거가 있고 도구가 지원할 때 검토한다.
...
아래 조건은 VATester 총점과 별도로 사전 판단·모델 선정·메인 회수에 적용한다. Codex 서브에이전트의 상한은 `Sol` / `xhigh`를 유지한다.

1. Codex는 정확도·동등성·보안·데이터 손상 위험을 메인이 먼저 평가한다. 중요한 설계·판정은 메인 `gpt-6-astra` / `xhigh`가 담당하고, 확정된 안전 경계 안의 구현·검토는 그 위험을 다룰 역량이 있는 모델과 최소 `high`로 위임한다. 상한 내에서 적합한 담당자가 없으면 메인이 처리한다.
...
4. 위임 모델·추론 수준 선정도 최대 한 개·재위임 금지 규칙을 지켜야 한다. 높은 위험을 이유로 Codex 서브에이전트에 `Astra`·`max`·`ultra`를 사용하거나 두 번째 담당자를 추가하지 않는다.
...
추천 모델: gpt-5.6-terra
추론 수준: 높음 (high)
선정 근거: 영향도 1, 불확실성 1, 검증 난이도 1, 변경 범위 1로 총 4점이다. 확정된 명세에 따른 단일 모듈 보완이며 복수 결과를 대조해야 하므로 high를 선택한다. 위험 상향은 없고 메인은 반환된 변경과 검증 증거를 직접 확인한다.
...
1. P1 - Manual UI evidence의 미확인 화면 재검수
   ...
   추론 수준: 높음 (high)
   담당: 메인 직접
   선정 근거: 영향도 2, 불확실성 1, 검증 난이도 2, 변경 범위 1로 총 6점이다. 실제 UI 조작 evidence와 문서 상태를 교차 검증해야 하며 별도 상향 규칙은 적용하지 않았다. Codex 런타임이면 최종 판정은 메인 gpt-6-astra / xhigh가 직접 수행한다.
```
