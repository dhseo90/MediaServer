...
4. 13.2 VATester 점수와 13.3 추론 수준 의미는 두 패밀리가 공유한다. 점수 결과를 모델명으로 바꿀 때만 해당 패밀리 표를 쓴다.
5. 다른 패밀리 이름을 섞어 쓰지 않는다. Grok 작업에 `5.6 Sol`을 쓰거나 Codex 작업에 `grok-4.6`을 쓰지 않는다.
6. 사용자가 특정 모델을 명시하면 그 지시를 우선하고, 추천 모델과 실제 사용 모델을 분리해 적는다.

### 13.1.1 Codex (GPT-5.6) 공식 모델 기준

Codex 런타임에서만 사용한다. 모델 명칭과 추론 수준은 OpenAI GPT-5.6 모델 가이드를 기준으로 한다.
...
총점에 따른 기본 모델은 런타임 패밀리별로 아래와 같다.

| 총점 | Codex 기본 모델 | Grok 기본 모델 | 기본 해석 |
| --- | --- | --- | --- |
| 0~2점 | `5.6 Luna` | `grok-4.5` | 반복적이고 결정적이며 영향 범위가 작은 작업 |
| 3~5점 | `5.6 Terra` | `grok-4.6` | 간단하거나 중간 규모이며 지능과 비용의 균형이 필요한 작업 |
| 6~8점 | `5.6 Sol` | `grok-4.6` | 복잡도, 영향, 불확실성, 검증 비용이 큰 작업 |

해당 패밀리의 13.1 작업 종류 초기 후보와 점수 결과가 다르면 더 높은 역량의 모델을 선택한다. 단, 반복 작업이라는 이유만으로 정확도나 데이터 위험을 무시해 Codex `Luna` 또는 Grok `grok-4.5`로 낮추지 않는다.

### 13.3 추론 수준 기준

추론 수준은 `none`, `low`, `medium`, `high`, `xhigh`, `max`만 사용한다. 한글 보고에는 아래 대응을 함께 쓴다. Codex와 Grok이 같은 단계 이름을 쓰지만, 현재 모델이 지원하지 않는 단계는 추천하지 않는다. Grok TUI `/effort`가 `low`/`medium`/`high`/`xhigh`만 받는 세션에서는 `none`/`minimal`/`max`를 TUI 추천값으로 쓰지 않는다.
...
3. `max`는 8점이거나 13.4의 중대한 상향 조건이 있고, `xhigh`보다 추가 탐색과 검증이 필요한 이유를 명시할 수 있는 가장 어려운 품질 우선 작업에만 사용한다. 현재 모델이 `max`를 지원하지 않으면 `xhigh`를 쓰고 그 이유를 적는다.
...
1. 정확도, 동등성, 보안, 데이터 손상 위험이 실질적으로 존재하면 점수 결과보다 모델을 한 단계 상향하고 추론 수준은 최소 `high`로 한다. Grok에서 이미 `grok-4.6`이면 모델은 유지하고 추론 수준만 최소 `high`로 올린다.
2. auth/role/scope, 외부 쓰기, 비가역 데이터 손실, 공개 API/schema/event payload, RTSP/WebRTC media path, release correctness를 변경하거나 검증하면 Codex는 `5.6 Sol`, Grok은 `grok-4.6`을 사용하고 추론 수준은 최소 `high`로 한다.
3. 여러 계약 경계에 걸친 보안 또는 데이터 손상 가능성, 전면 구조 변경, 거짓 PASS가 릴리즈를 허용할 위험이 있으면 `xhigh`를 우선 검토한다.
4. `max`는 자동 상향값이 아니다. 가장 어려운 품질 우선 작업이라는 근거와 `xhigh`로 부족한 이유를 별도로 적어야 한다. 현재 모델이 `max`를 지원하지 않으면 `xhigh`에서 멈춘다.
5. 상향 규칙을 적용했으면 원래 점수 모델과 최종 상향 모델을 선정 근거에 함께 적는다.

### 13.5 모델 추천 보고 형식

개발, 검토, 테스트 설계, 로드맵/잔여 이슈 추천에서 모델 추천을 제시할 때는 아래 형식을 사용한다. 런타임 패밀리를 먼저 적고, 해당 패밀리의 모델명만 쓴다.
...
표로 제시할 때도 `런타임 패밀리`, `추천 모델`, `추론 수준`, `선정 근거` 열을 각각 둔다. 선정 근거에는 최소한 네 평가 점수, 총점, 상향 규칙 적용 여부를 포함한다.
...
1. P1 - Manual UI evidence의 미확인 화면 재검수
   ...
   추천 모델: grok-4.6
   추론 수준: 높음 (high)
   선정 근거: 영향도 2, 불확실성 1, 검증 난이도 2, 변경 범위 1로 총 6점이다. 실제 UI 조작 evidence와 문서 상태를 교차 검증해야 하며 별도 상향 규칙은 적용하지 않았다. Codex 런타임이면 같은 점수에서 5.6 Sol을 쓴다.
```
