...
9. 개발 작업에는 Superpowers 절차를 활용하되, 모델 선택과 위임은 1.3의 `아스트라 medium 기본 책임자 + 단일 개발 담당자` 원칙을 따른다.
...
아래와 같이 해석이나 추가 판단이 필요한 미해소 충돌은 즉시 중단하고 보고한다.
...
4. 사용자의 최신 지시와 기존 문서/과거 대화가 충돌하고, 변경 대상·범위 또는 안전 경계가 불명확한 경우

사용자의 최신 명시 지시로 변경 대상과 범위가 분명하거나 1.3처럼 예외와 우선순위가 이미 정해진 경우에는 그 지시와 기준을 적용한다. 이미 해소된 동일 충돌을 이유로 재승인을 반복 요청하지 않는다. 이 원칙은 명시 범위 밖 변경, 검증 생략, 거짓 완료 또는 승인 없는 커밋·푸시·릴리즈 실행을 허용하지 않는다.
...
에이전트에게 자의적 커밋 권한은 없다. 이전 대화, 과거 규칙, release close-out 관행, `/goal`, "릴리즈 준비", "마무리", "끝내" 같은 표현은 커밋 승인으로 해석하지 않는다. 최신 사용자 요청에 `커밋해`, `커밋 진행`, `commit`처럼 커밋 자체가 명시되지 않으면 커밋하지 않는다. 커밋 가능 상태 보고와 실제 커밋은 서로 다르다. 명시 승인이 없으면 커밋 가능하다고 판단해도 멈추고 기다리며, 해당 범위의 명시 승인이 있으면 5.1의 조건을 충족한 뒤 재승인 없이 커밋한다.

### 1.3 버전 개발과 단일 서브에이전트 운영

이 절은 2026-09-06의 `Astra 책임자 + 단일 개발 담당자` 방식에 2026-09-08 사용자가 승인한 `medium 기본·필요 시 사용자 상향` 원칙을 반영한다. 모델·위임에 관한 스킬의 일반 지침보다 이 절을 우선한다. 작업 시작 시 이 절과 모델 선정에 필요한 13장을 실제 파일에서 확인한다. 이는 프로젝트 운영 기준이며 Astra low/medium이 모든 작업에서 Sol xhigh보다 우수하다는 공식 성능 보장을 뜻하지 않는다.

1. 메인의 기본 모델은 `GPT-6 Astra` (`gpt-6-astra`), 기본 추론 수준은 `medium`이다. 실제 모델·추론 수준은 사용자가 선택하며 다른 값을 명시하면 그 지시를 따른다. 어려운 작업이나 높은 점수만으로 자동 상향하지 않는다. 현재 설정으로 해결하지 못하면 원인·시도·미해소 판단을 보고하고, 사용자가 필요에 따라 `high` 또는 `xhigh`로 올린다. 문서 변경만으로 실행 중 모델·추론 설정까지 변경됐다고 보고하지 않는다.
2. 메인은 로드맵·작업 범위·구조와 변경하지 않을 계약을 결정하고, 구현 전에 합격 기준과 회귀 확인 범위를 확정한다. 공개 API/schema와 시간·ID 계약, 녹화·보존·복구의 데이터 안전성, auth/role/scope, media/source lifecycle의 설계와 안전 경계, 복합적인 원인 분석, 최종 통합·완료 판정은 메인이 책임진다. 중요 기능이라는 이유만으로 모든 코드 수정을 메인에게 고정하지 않는다.
3. 확정된 범위의 구현·테스트 작성·승인된 검증 실행은 단일 서브에이전트에 위임하는 것을 기본으로 한다. 입력·출력·수정 범위·계약·완료 기준이 명확하고 메인이 결과를 검증할 수 있어야 하며, 이는 중요한 기능에도 동일하게 적용한다. 설계가 미확정된 채 구현을 떠넘기거나, 서브에이전트가 계약·검증 범위를 자의적으로 바꾸게 하지 않는다. 한정된 조사와 근거 수집도 같은 담당자에게 맡길 수 있다.
4. 메인 외 작업용 서브에이전트는 **최대 한 개**만 둔다. 메인과 그 한 개가 함께 일하는 것은 허용하지만, 별도 구현자·검토자를 동시에 추가하지 않는다. 사용 가능한 기존 에이전트를 먼저 확인하고 같은 에이전트를 순차 재사용한다. 이전 작업의 에이전트가 남아 있으면 새 에이전트를 추가하기 전에 상태와 담당 작업을 확인한다.
5. 서브에이전트는 하위 에이전트를 생성하거나 다른 에이전트에 재위임할 수 없다. 도구 이름이나 별도 작업 생성 기능을 이용해 이 제한을 우회하지 않는다. 위임 깊이는 `메인 → 서브에이전트` 한 단계다. 이는 모델의 추론 수준과 별개의 제한이다.
6. 메인은 위임 전에 13장의 기준으로 **서브에이전트의 모델과 추론 수준**을 정하고 도구가 지원하는 정확한 값으로 지정한다. 복합 개발의 기본 후보는 `gpt-6-astra` / `medium`이며 제한된 작업에는 Sol/Terra/Luna를 선택할 수 있다. 사용자 별도 지정이 없으면 `low` 또는 `medium` 안에서 선택하며 `high`·`xhigh`는 사용자 명시 지정 또는 승인 뒤에만 사용한다. 허용 모델 상한은 `gpt-6-astra`, 승인 가능한 추론 상한은 `xhigh`다. 같은 작업 묶음에서 설정을 유지하고 실행 중 변경 기능이 없으면 변경한 것처럼 보고하지 않는다. 기존 담당자의 설정으로 부족하면 메인이 회수하며 모델을 바꾸기 위해 두 번째 에이전트를 추가하지 않는다.
7. 위임 지시에는 담당 범위, 수정 가능한 파일, 필요한 계약, 합격 기준과 회귀 확인 범위, 승인된 검증, 모델·추론 수준, 하위 에이전트 금지, 결과 보고 항목을 명시한다. 반환 내용은 변경 파일·핵심 diff·조사 결론, 실제 실행 명령·결과·증거 위치, 실패·미실행·미해소 우려 중심으로 간결하게 보고하되 필수 보고 항목은 유지한다. 메인과 서브에이전트가 같은 파일을 동시에 수정하지 않도록 소유 범위를 나눈다.
8. 메인은 반환된 실제 diff와 검증 증거를 직접 확인하고 안전 경계를 검토한 뒤 통합한다. 요약 보고나 서브에이전트의 자체 PASS만으로 완료 처리하지 않는다. 보완은 같은 담당자에게 요청하되, 인계보다 직접 처리가 짧은 소규모 수정은 메인이 처리할 수 있다. 동일 원인이 재발하거나 여러 계약이 얽혀 구현 지시 자체를 재판단해야 하면 같은 수정을 반복 위임하지 않고 메인이 회수해 원인과 영향 범위를 확인한다. 이 예외는 3장·8장의 실패 보고·중단 규칙을 우회하거나 중단 뒤 수정·재실행의 새 권한을 만들지 않는다.
9. Superpowers의 설계, TDD, 원인 분석, 변경 검토와 완료 검증 절차는 작업에 맞게 활용한다. `메인은 직접 수정 금지`, `작업마다 새 에이전트 생성`, `별도 검토 에이전트 추가`, `메인 xhigh 고정`, `서브에이전트 Astra 금지` 규칙은 이 프로젝트에서 적용하지 않는다. 이 충돌은 본 절에 따라 해소하며 이미 승인된 설정을 반복 승인받지 않는다. 추론 수준을 낮춰도 계약·합격 기준·검증 범위는 줄이지 않는다.
10. 승인된 로드맵과 현재 단계의 개발은 기존 진행 문맥을 이어간다. 모델·위임 선택을 위해 승인된 설계부터 다시 조율하지 않는다. 이 운영 방식은 다른 단계의 자동 착수, 범위 밖 변경, 테스트 묶음 확대, 커밋·푸시·릴리즈 실행의 새 권한을 부여하지 않는다.
11. 인계는 전체 대화 복사 대신 관련 파일·계약·완료 기준 중심의 최소 문맥으로 구성하되, 필수 지침을 누락하지 않는다. 함수마다 잘게 쪼개지 않고 독립적으로 검증 가능한 기능 단위로 맡긴다. 메인은 반환 근거를 확인하되 같은 조사를 처음부터 반복하거나 전체 로그를 무조건 재독하지 않고, 변경·누락·실패·불확실한 부분을 중심으로 확인한다.
12. 같은 코드·환경·검증 범위에 대한 유효한 증거는 인계만을 이유로 재실행하지 않는다. 관련 변경, 증거 누락·만료, 실패 또는 미해소 회귀 우려가 있으면 영향 범위에 맞는 재검증을 판단하고 7장의 실행 승인 기준을 따른다. 토큰 절약을 이유로 합격 기준이나 검증 범위를 줄이지 않는다.
13. 지침 중복은 12.2의 source-of-truth와 참조 방식으로 관리한다. 필수 규칙은 유지하며 로딩 한도를 낮춰 지침을 잘라내지 않는다. 위임 자체가 토큰 절약을 보장한다고 보고하지 않으며, 절감 효과는 집계 가능한 실제 사용량이 있을 때만 판단한다.
...
3. 한 단계가 실패하면 그 즉시 중단한다. 단, 3.1.1의 조건을 모두 충족한 TDD의 예상된 RED는 단계 실패와 구분한다.
...
예상된 RED는 승인된 현재 단계의 구현 전에 미구현 요구사항을 확인하는 검증이며, 다음 조건을 모두 충족해야 한다.

1. 실행 전에 해당 focused test와 예상 실패 assertion·원인을 특정한다. 관찰된 임의의 실패를 사후에 RED로 바꾸지 않는다.
2. 실제 결과가 그 미구현 요구사항의 예상 assertion 실패와 일치한다. 빌드 오류, 명령·의존성 부재, 환경 오류 또는 다른 원인의 실패를 RED로 대체하지 않는다.
3. 기존 통과 baseline·회귀 테스트의 실패는 예상된 RED에 포함하지 않는다. 실제 실패에는 3.1과 8장의 중단 규칙을 적용하며, `/goal` 예외는 3.3을 따른다.
4. 예상된 RED의 명령·실패 원인·결과를 기록한 뒤 승인된 동일 단계 안에서 구현과 GREEN 검증을 이어간다. RED 자체를 PASS나 완료 evidence로 사용하지 않는다.
5. 동일 요구사항의 GREEN과 관련 회귀 검증을 통과하기 전에는 단계 완료, 해당 단계 커밋 또는 다음 단계로 넘어가지 않는다. 테스트 범위와 실행 승인은 7장의 기준을 유지한다.
...
4. 각 단계가 통과하고 필수 기록과 테스트 결과가 반영되면 `커밋 가능: 예/아니오`를 보고한다. 해당 범위의 명시 승인이 없을 때만 멈추고 기다린다. 이미 명시 승인된 범위이고 모든 커밋 조건이 충족되면 재승인 없이 커밋하고 결과를 보고한다.
...
3. 모든 단계가 통과하고 커밋이 완료되어도 사용자의 명시적 푸시 요청이 없으면 푸시하지 않는다. 명시적 요청이 있고 해당 범위의 푸시 조건이 충족되면 재승인 없이 푸시하고 결과를 보고한다.
...
1. 사용자가 실제로 원한 최종 산출물과 완료 범위를 명확히 식별했는가?
2. 그 산출물이 파일/코드/UI/설정/테스트 결과에 실제로 존재하는가?
3. 실제 수행한 검증이 그 산출물 자체를 확인하며, 검증 범위와 한계를 구분했는가?
4. 완료 조건에 해당하는 `공식 확인 필요`, `미확인`, `pending`, `review-required`, `not-approved`, `후속 확인 필요` 항목이 모두 해소되었는가?
5. 완료로 보고하는 상태와 roadmap/status 문서의 관련 완료 표기를 직접 evidence로 뒷받침했는가?
...
TDD의 예상된 RED는 3.1.1의 조건을 모두 충족한 경우에만 실제 실패와 구분한다. 아래 빌드·환경·회귀 실패를 예상된 RED로 대체하지 않는다.
...
모델 추천은 현재 대화를 수행하는 에이전트 런타임 패밀리를 따른다. Codex 기준만 쓰거나 Grok 기준만 쓰지 않고, 아래 규칙으로 하나를 고른다.

1. 현재 에이전트가 Grok이면 13.1.2 Grok 기준만 추천에 사용한다.
2. 현재 에이전트가 Codex이면 13.1.1 Codex 기준만 추천에 사용한다.
3. 런타임을 확인할 수 없으면 임의로 한 패밀리를 고르지 말고 `추천 모델 선택 불가`로 보고한다. 확인된 런타임, 추천 모델, 실제 사용 모델, 영향 범위를 분리해 적는다.
4. 13.2 VATester 점수와 13.3 추론 수준 의미는 두 패밀리가 공유한다. Codex에서는 1.3에 따라 메인의 설계·판정과 위임할 구현을 먼저 구분하고, 점수 기반 모델 조절은 단일 개발 담당자에게 적용한다. 메인 직접 수정 예외와 서브에이전트 상한은 점수보다 우선한다.
5. 다른 패밀리 이름을 섞어 쓰지 않는다. Grok 작업에 `5.6 Sol`을 쓰거나 Codex 작업에 `grok-4.6`을 쓰지 않는다.
6. 사용자가 특정 모델과 추론 수준을 명시하면 그 지시를 우선하고 추천과 실제 사용 설정을 분리해 적는다. Codex 메인의 버전 시작 기본값과 상향 결정은 1.3을 따른다.

### 13.1.1 Codex 메인과 서브에이전트 모델 기준

Codex 런타임에서만 사용한다. 역할·기본값·사용자 상향 승인·상한은 1.3을 따른다. 모델 이름과 지원 추론 수준은 현재 실행 도구가 노출하는 목록으로 확인하며 문서에 있다는 이유만으로 선택 가능하다고 보고하지 않는다.

- 공식 모델 참고: <https://developers.openai.com/api/docs/models/gpt-6-astra>
- `GPT-6 Astra` (`gpt-6-astra`): 메인 책임자와 복합 개발 서브에이전트의 기본 후보. 같은 모델을 사용해도 설계·안전 계약·통합·최종 판정은 메인이 책임짐
- `GPT-5.6 Sol` (`gpt-5.6-sol`): 확정된 계약에 따른 구현·조사의 대안. 실제 작업의 품질·사용량·시간 또는 가용성 근거로 선택하며 저비용을 무조건 보장하지 않음
- `GPT-5.6 Terra` (`gpt-5.6-terra`): 명세와 검증 기준이 분명한 제한된 구현·검토
- `GPT-5.6 Luna` (`gpt-5.6-luna`): 결과를 결정적으로 검증할 수 있는 정형 변환·반복 수집

위임 가능한 작업의 초기 후보는 아래와 같다.

1. 정형 변환이나 실행이 승인된 결정적 verifier의 결과 수집은 `gpt-5.6-luna`의 `low` 또는 `medium`을 검토한다. 단순 실행은 메인이 도구로 처리할 수 있으므로 서브에이전트를 억지로 만들지 않는다.
2. 범위가 작고 요구사항과 검증 방법이 분명한 개발은 `gpt-5.6-terra` / `medium`을 출발점으로 삼는다. 복잡한 교차 검증이 필요하면 Astra / medium 후보를 검토하되 기존 담당자의 역량 부족 시에는 1.3의 회수 원칙을 따른다.
3. 여러 모듈의 근거 대조나 의미 검토가 필요한 구현·조사는 `gpt-6-astra` / `medium`을 기본 후보로 삼고 `gpt-5.6-sol` / `medium`을 대안으로 검토한다. 복잡한 저장·권한·미디어 경계도 메인이 계약과 검증 기준을 먼저 확정한다. 미해소 판단에 추가 추론이 필요하면 근거를 보고하고 사용자 상향 결정을 기다린다.
4. 중요 기능도 메인이 계약·합격 기준·회귀 확인 범위를 확정하면 위임할 수 있다. 미확정 설계나 반복 실패를 모델 상향만으로 해결하지 않으며, 메인의 사전 판단·회수 기준은 1.3과 13.4를 따른다.
5. 초기 후보는 13.2의 점수와 13.4의 위험 판단으로 확정한다. 선택 불가나 기존 에이전트의 역량 부족 시 1.3의 메인 회수 원칙을 적용한다. 모델·추론 상한을 넘는 서브에이전트나 추가 검토자를 만들지 않는다.

### 13.1.2 Grok (Grok 4.x) 공식 모델 기준

Grok 런타임에서만 사용한다. 모델 명칭은 현재 Grok TUI가 실제로 노출하는 slug를 쓴다. `Sol`/`Terra`/`Luna` 같은 Codex 등급명을 Grok 모델처럼 만들지 않는다.

현재 확인된 기본 모델:

- `grok-4.6`: 현재 기본 flagship. 다중 모듈 분석, 구조 설계, 고위험 검토, 일반 개발의 기본값
- `grok-4.5`: 이전 세대. 반복, 대량 분류, 정형 변환, 이미 정해진 verifier 반복처럼 영향 범위가 작고 결정적인 작업에 우선

모델 목록은 `grok models`로 확인한다. 위 slug가 현재 세션에서 보이지 않으면 목록에 있는 동등 모델을 쓰고, 확인된 slug와 선택 사유를 적는다. 목록에 없는 이름을 만들지 않는다.

추론 수준은 Grok TUI `/effort`와 headless `--effort`가 받는 값을 사용한다.

- TUI `/effort` 기본 메뉴: `low`, `medium`, `high`, `xhigh`
- headless `--effort`는 위 값에 더해 `none`, `minimal`, `max`를 받을 수 있다. 현재 모델이 해당 단계를 지원할 때만 쓴다.

작업 종류에 따른 초기 후보는 아래와 같다.

1. 반복, 대량 분류, 정형 변환, 이미 정해진 verifier 반복 실행은 `grok-4.5`를 우선한다.
2. 범위가 작고 요구사항과 검증 방법이 분명한 간단한 개발 작업은 `grok-4.6`을 우선한다.
3. 위 두 종류로 명확히 분류되지 않는 기본 작업, 다중 모듈 분석, 구조 설계, 고위험 검토는 `grok-4.6`을 우선한다.
...
6. `Shift+Tab`은 모델 전환이 아니다. 모델은 `/model`, `/m`, 또는 스크롤백에서 `Ctrl+M`으로 고른다.
...
총점에 따른 후보는 아래와 같다. Codex 열은 **위임 가능한 작업의 서브에이전트 후보**이며 메인 모델을 변경하는 표가 아니다. 점수보다 1.3의 직접 처리·위임 경계가 우선한다.

| 총점 | Codex 서브에이전트 후보 | Grok 기본 모델 | 기본 해석 |
| --- | --- | --- | --- |
| 0~2점 | `5.6 Luna` | `grok-4.5` | 반복적이고 결정적이며 영향 범위가 작은 작업 |
| 3~5점 | `5.6 Terra` | `grok-4.6` | 간단하거나 중간 규모이며 지능과 비용의 균형이 필요한 작업 |
| 6~8점 | `GPT-6 Astra / medium` 기본, `5.6 Sol / medium` 대안; 계약 미확정·회수 사유가 있으면 메인 판단 선행 | `grok-4.6` | 복잡도, 영향, 불확실성, 검증 비용이 큰 작업 |

해당 패밀리의 13.1 작업 종류 초기 후보와 점수 결과가 다르면 필요한 판단 역량을 충족하는 쪽을 선택하고 근거를 적는다. 반복 작업이라는 이유로 정확도나 데이터 위험을 무시해 Codex `Luna` 또는 Grok `grok-4.5`로 낮추지 않는다. Codex 메인 직접 처리 항목에도 점수와 근거를 기록하되 점수만으로 사용자 설정을 바꾸지 않는다. 점수는 작업 분류이며 모델 간 벤치마크나 자동 추론 상향 규칙이 아니다.

### 13.3 추론 수준 기준

추론 수준은 현재 모델과 실행 도구가 함께 지원하는 값만 사용한다. Codex 기본값·사용자 상향 승인·상한은 1.3을 따른다. 아래 용어표는 자동 상향 권한을 만들지 않는다. Grok은 13.0과 13.1.2를 따르며 Codex의 기본값·상한을 적용하지 않는다. 추론 수준과 하위 에이전트 생성 깊이를 혼동하지 않는다. Grok TUI가 지원하지 않는 추론 수준을 TUI 추천값으로 쓰지 않는다.

| 추론 수준 | 한글 표기 | 적용 기준 |
| --- | --- | --- |
| `none` | 없음 | 의미 판단이 없고 결과가 완전히 결정적인 기계적 변환에만 사용 |
| `low` | 낮음 | 저위험 반복 작업이나 latency 우선 작업에 사용 |
| `medium` | 중간 | Codex 메인과 복합 개발 담당자의 기본 출발점 |
| `high` | 높음 | 현재 수준으로 해결하지 못한 판단에 추가 추론이 필요할 때 검토. Codex는 사용자 지정 또는 승인 후 적용 |
| `xhigh` | 매우 높음 | 복합 난제의 예외적 선택이자 Codex 승인 가능 상한. 사용자 지정 또는 승인 없이 적용하지 않음 |
| `max` | 최대 | Codex 서브에는 사용하지 않음. Grok은 지원되는 경우 가장 어려운 품질 우선 작업에서 검토 |
| `ultra` | 울트라 | Codex 서브에는 사용하지 않음. 다른 런타임에서도 도구 지원과 사용자 명시 지시, 인원·위임 깊이 제한을 모두 충족해야 검토 |

1. Codex는 medium 기본으로 진행하고 실제 사용자 선택을 존중한다. 추론 수준을 바꿔도 정확도·회귀 검증·완료 기준을 완화하지 않는다. AGENTS.md는 실행 중인 모델 설정을 직접 바꾸는 장치가 아니다.
2. Codex 메인 또는 서브에이전트의 high 이상을 제안할 때는 미해소 문제·시도한 방법·추가 추론이 필요한 판단을 적는다. P0·파일 수·높은 점수만으로 상향하지 않으며 사용자 명시 지정 또는 승인을 확인한다.
3. Codex는 8점이거나 위험 상향 사유가 있어도 `max`·`ultra`를 사용하지 않는다. 기존 담당자의 설정으로 부족하면 메인이 회수해 원인과 계약을 재검토하고 필요 시 사용자 상향 결정을 요청한다. Grok의 `max`는 8점 또는 중대한 위험 사유와 추가 탐색 근거가 있고 도구가 지원할 때 검토한다.
4. 위임된 반복 실행과 정형 evidence 수집은 `low` 또는 `medium`을 검토하고, 원인 분석이 중요 작업으로 확대되면 Codex 메인이 회수한다. 재사용 중인 에이전트의 설정을 변경할 수 없으면 작업마다 설정을 바꾼 것처럼 보고하지 않는다.

### 13.4 정확도 및 위험 상향 규칙

아래 조건은 VATester 총점과 별도로 사전 판단·모델 선정·메인 회수에 적용한다. Codex 기본값·사용자 상향 승인·상한은 1.3을 따르며 위험 자체가 자동 추론 상향을 허용하지 않는다.

1. Codex는 정확도·동등성·보안·데이터 손상 위험을 메인이 먼저 평가한다. 중요한 설계·판정은 메인이 담당하고 확정된 안전 경계 안의 구현·검토는 Astra / medium을 기본 후보로 위임할 수 있다. 위험이 크다는 이유로 최소 high를 강제하지 않으며 계약 미확정이나 근거 부족은 메인이 회수한다. 추가 추론이 필요하면 사용자에게 근거를 보고한다.
2. auth/role/scope, 외부 쓰기, 비가역 데이터 손실, 공개 API/schema/event payload, RTSP/WebRTC media path, release correctness는 메인이 변경 허용 범위·합격 기준·회귀 확인 범위를 먼저 확정하고 반환된 diff와 증거를 직접 검토한다. 구현은 단일 개발 담당자에게 맡길 수 있지만 계약 변경·위험 수용·최종 판정까지 위임하지 않는다. 외부 쓰기의 명시 승인 경계도 그대로 유지한다. Grok 런타임이면 `grok-4.6`과 최소 `high`를 사용한다.
3. 동일 원인 재발, 예상 밖의 교차 회귀, 미확정 계약이나 근거 부족으로 보안·데이터 안전성·PASS 판정을 다시 판단해야 하면 Codex 메인이 회수해 원인·영향 범위를 분석하고 구현 지시를 재검토한다. 같은 수정을 이유 없이 반복 위임하지 않는다. 실제 실패 뒤의 보고·중단·수정 재개는 3장·8장을 따른다.
4. 위임 모델·추론 수준 선정도 최대 한 개·재위임 금지 규칙을 지켜야 한다. Astra 서브에이전트는 허용하지만 높은 위험을 이유로 `max`·`ultra`를 사용하거나 두 번째 담당자를 추가하지 않는다.
5. 상향 또는 메인 회수를 적용했으면 원래 후보, 최종 담당자·모델·추론 수준과 이유를 함께 적는다. Grok에서 이미 `grok-4.6`이면 모델을 유지하고 필요한 추론 수준만 조절한다.

### 13.5 모델 추천 보고 형식

개발, 검토, 테스트 설계, 로드맵/잔여 이슈 추천에서 모델 추천을 제시할 때는 아래 형식을 사용한다. 런타임 패밀리와 `메인 직접` 또는 `단일 서브에이전트 위임`을 먼저 적는다. 추천과 실제 실행을 구분하고, 모델을 선택하지 않았거나 설정 변경을 확인하지 못했으면 그 상태를 적는다.
...
추천 모델: grok-4.6
추론 수준: 높음 (high)
선정 근거: 영향도 1, 불확실성 1, 검증 난이도 2, 변경 범위 1로 총 5점이다. 여러 스크립트와 Docker 빌드 경계를 검증하지만 모델 정확도 계약을 변경하지 않는 통합 작업이다. Grok 기본표는 grok-4.6이며 상향 규칙은 적용하지 않았다.
```
...
담당: 메인 직접
모델: gpt-6-astra
추론 수준: 중간 (medium), 기본 권고값. 실제 사용자 선택과 다르면 실제 설정을 별도 표기
선정 근거: 영향도 2, 불확실성 2, 검증 난이도 2, 변경 범위 2로 총 8점이다. 녹화 삭제·복구 계약과 여러 모듈의 종료 순서, 합격·회귀 기준을 판단하므로 메인이 설계와 최종 판정을 직접 처리한다. 확정된 구현은 단일 개발 담당자에게 위임할 수 있으며 점수에 따른 메인 모델 전환은 하지 않는다.

런타임 패밀리: Codex
담당: 단일 서브에이전트 위임
추천 모델: gpt-5.6-terra
추론 수준: 중간 (medium)
선정 근거: 영향도 1, 불확실성 1, 검증 난이도 1, 변경 범위 1로 총 4점이다. 확정된 명세에 따른 단일 모듈 보완으로 medium을 선택한다. 자동 상향은 없고 메인은 반환된 변경과 검증 증거를 직접 확인한다.
위임 제한: 기존 서브에이전트 한 개 재사용, 하위 에이전트 생성 금지
```

표로 제시할 때도 `런타임 패밀리`, `담당(메인/서브)`, `추천 모델`, `추론 수준`, `선정 근거` 열을 각각 둔다. 선정 근거에는 최소한 네 평가 점수, 총점, 상향 또는 메인 회수 적용 여부를 포함한다. 메인 직접 처리의 모델·추론 수준은 사용자 지정 유지로 적는다.
...
7. 권장 추론 수준
8. 모델과 추론 수준 선정 근거
9. 메인 직접 처리 또는 단일 서브에이전트 위임 여부와 그 이유
...
1. P1 - Manual UI evidence의 미확인 화면 재검수
   ...
   추론 수준: 높음 (high)
   담당: 메인 직접
   선정 근거: 영향도 2, 불확실성 1, 검증 난이도 2, 변경 범위 1로 총 6점이다. 실제 UI 조작 evidence와 문서 상태를 교차 검증해야 하며 별도 상향 규칙은 적용하지 않았다. Codex 런타임이면 최종 판정은 메인 gpt-6-astra(기본 medium)가 직접 수행하며 사용자 실제 선택을 따른다.
```
