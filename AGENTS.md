...
해당 패밀리의 13.1 작업 종류 초기 후보와 점수 결과가 다르면 필요한 판단 역량을 충족하는 쪽을 선택하고 근거를 적는다. 반복 작업이라는 이유만으로 정확도나 데이터 위험을 무시해 Codex `Luna` 또는 Grok `grok-4.5`로 낮추지 않는다. 메인 직접 처리 항목에도 점수와 근거를 기록하되 메인의 `Astra` / `xhigh` 선택을 낮추는 데 사용하지 않는다.

### 13.3 추론 수준 기준

추론 수준은 현재 모델과 실행 도구가 함께 지원하는 값만 사용한다. 메인은 사용자가 선택한 `xhigh`를 유지하고, 아래 기준은 주로 서브에이전트의 추론 수준 조절에 사용한다. 추론 수준과 하위 에이전트 생성 깊이를 혼동하지 않는다. Grok TUI `/effort`가 `low`/`medium`/`high`/`xhigh`만 받는 세션에서는 그 외 값을 TUI 추천값으로 쓰지 않는다.
...
1. 메인의 `xhigh`는 사용자 지정값이므로 작업마다 다시 정당화하거나 하향하지 않는다. 서브에이전트는 작업 난이도와 반환 결과의 검증 가능성에 맞춰 조절한다.
