...
3. 실패·미실행·미확인을 숨기지 않고 사실과 추정을 구분한다. 실패 뒤 단계는 중단한다(3.3 예외).
...
작업 시작 시 이 절과 13장을 실제 파일에서 확인한다. 모델·위임 스킬 일반 지침보다 이 절이 우선한다.
...
3. 확정된 구현·테스트 작성·승인된 실행과 한정 조사는 단일 담당자에게 위임하는 것이 기본이다. 중요 기능도 위임 가능하나 미확정 설계·정책 결정은 떠넘기지 않는다.
...
일반 실패는 즉시 중단하고 뒤 단계는 `건너뜀`이다. 실패 단계는 커밋하지 않고 기존 승인 커밋은 유지한다.
...
`/goal` 또는 goal option의 end-to-end 목표에서는 실패를 먼저 기록하고 같은 목표·로드맵 범위의 수정 가능한 원인을 고친 뒤
해당 기능 안정화와 영향 회귀를 재개할 수 있다. 실패가 해소되기 전 뒤 단계는 금지한다.
해결 불가·사용자 결정·새 권한이 필요하면 중단하고 뒤 단계는 건너뛴다. 최초 실패·수정·재검증을 함께 남긴다.
goal은 범위 밖 수정, 외부 변경, 미승인 테스트·커밋·푸시 권한을 만들지 않는다.
...
안정화 실패 후 30분/120분/UI로 넘어가지 않는다. 모든 묶음은 명시 실행 승인 범위만 수행한다.
...
일반 작업은 아래 실패에서 중단·보고하고 뒤 단계는 건너뛴다. 예상 RED는 3.1.1, goal 수정 재개는 3.3만 예외다.

- build·핵심 smoke·auth route guard 실패.
- viewer source URL/debug/raw JSON 노출.
- WebRTC DataChannel, Event POST, SSE/WS schema 변경 징후.
- RTSP/WebRTC media 회귀 또는 DataChannel 실패의 영상 경로 전파.
- cleanup/port cleanup 또는 git diff --check 실패.
- 명령 부재/실행 불가로 대체 판단이 필요한 경우.

실패 명령·지점·확인/추정 원인·변경 파일·기존 결과·미실행·재개 조건을 남긴다.
실패를 숨기거나 timeout/판정 완화로 PASS를 만들지 않는다.
...
각 이슈마다 모델·추론·선정 근거와 메인/서브 역할을 기록한다. 같은 모델이라도 근거를 생략하지 않는다.
...
영향도/불확실성/검증 난이도/변경 범위를 각각 0~2, 합계 0~8로 기록한다.
...
Codex 메인 직접 작업도 점수는 기록하되 1.3 역할·사용자 설정·상한이 우선이다. 점수는 모델 벤치마크가 아니다.

### 13.3 추론 수준 기준

none=결정적 기계 변환, low=저위험/지연 우선, medium=Codex 기본, high=추가 판단,
xhigh=예외 복합 판단이다. Codex high/xhigh는 사용자 지정/승인 필요, max/ultra는 사용하지 않는다.
Grok max는 지원되며 8점 또는 중대 위험·추가 탐색 근거가 있을 때 검토한다.
다른 런타임 ultra도 지원·사용자 명시·인원/깊이 제한을 모두 충족해야 한다.
P0·파일 수·점수만으로 상향하지 않고 미해소 문제·시도·추가 판단을 보고한다.
추론을 낮춰도 계약·회귀·완료 기준은 유지하며 실행 중 설정을 바꾼 것처럼 보고하지 않는다.

### 13.4 정확도 및 위험 상향 규칙

auth/scope·외부 쓰기·비가역 손실·공개 schema/event·media·release correctness는 메인이 안전 계약·검증 범위를 확정하고 실제 diff/evidence를 검토한다.
구현은 위임 가능하나 위험 수용·정책·최종 판정은 위임하지 않는다. Codex 위험은 자동 high 사유가 아니며 Grok은 grok-4.6/최소 high다.
반복 원인·교차 회귀·근거 부족이면 메인이 회수한다. 원래 후보·최종 담당/모델/추론·이유를 기록한다.
상향도 승인/인원/깊이 제한과 실패 중단 규칙을 우회하지 않는다.

### 13.5 모델 추천 보고 형식

`런타임 패밀리 | 담당(메인/서브) | 추천 모델 | 추론 수준 | 선정 근거`를 적는다.
선정 근거에는 네 점수·합계·상향/회수 여부, 실제 설정·추천의 차이를 포함한다.
후속 추천은 현재 버전·현재 스텝 안에서 실제 처리 가능한 항목만 허용한다.
각 항목은 제목·우선순위·이유·예상 검증·범위 근거·모델/추론·역할을 갖춘다.
