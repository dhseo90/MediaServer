...
9. 개발 작업에는 Superpowers 절차를 활용하되, 모델 선택과 위임은 1.3의 `아스트라 xhigh 메인 직접 처리 + 선택적 서브에이전트 한 개` 원칙을 따른다.
...
이 절은 사용자가 2026-09-05에 지정한 Codex 개발 방식이다. 모델·위임에 관한 스킬의 일반 지침보다 이 절을 우선한다. 자동 주입된 지침이 일부여도 작업 시작 시 이 절과 모델 선정에 필요한 13장을 실제 파일에서 확인한다.
...
2. 중요 작업은 메인이 직접 설계·구현·수정한다. 버전 로드맵과 구조 결정, 공개 API/schema와 시간·ID 계약, 녹화·보존·복구의 데이터 안전성, auth/role/scope, media/source lifecycle, 다중 모듈의 원인 분석과 최종 통합·완료 판정이 이에 해당한다.
3. 서브에이전트는 입력·출력·수정 범위·완료 기준이 명확하고, 반환된 결과를 메인이 검증해 수용할 수 있는 독립 작업에만 사용한다. 한정된 소스 조사, 결정적인 검증 실행과 결과 수집, 명세가 확정된 소규모 구현, 독립적인 변경 검토가 후보이며 위임은 필수가 아니다. 중요 작업의 독립 검토를 위임해도 최종 판단과 통합 책임은 메인에 남는다.
...
6. 메인은 위임 전에 13장의 기준으로 **서브에이전트의 모델과 추론 수준**을 정하고, 사용 가능한 도구가 지원하는 정확한 값으로 지정한다. 실행 중 설정을 바꾸는 기능이 없으면 실제 변경한 것처럼 보고하지 않는다. 기존 에이전트로 처리할 수 없는 난이도이면 메인이 회수해 직접 처리하며, 모델을 바꾸기 위해 두 번째 에이전트를 추가하지 않는다.
7. 위임 지시에는 담당 범위, 수정 가능한 파일, 필요한 계약, 완료 기준, 모델·추론 수준, 하위 에이전트 금지, 결과 보고 항목을 명시한다. 메인과 서브에이전트가 같은 파일을 동시에 수정하지 않도록 소유 범위를 나눈다.
8. 메인은 반환된 변경과 검증 증거를 직접 확인한 뒤 통합한다. 결과 보고만으로 완료 처리하지 않는다. 수정이 필요하면 같은 서브에이전트에 후속 작업을 주거나 메인이 직접 수정한다. 변경·실패·미해소 우려가 없는 동일 코드의 검증을 인계받았다는 이유만으로 반복하지 않는다.
...
4. 13.2 VATester 점수와 13.3 추론 수준 의미는 두 패밀리가 공유한다. Codex에서는 1.3에 따라 메인 직접 처리 여부를 먼저 결정하고, 점수 기반 모델 조절은 위임 가능한 서브에이전트 작업에 적용한다.
...
Codex 런타임에서만 사용한다. 메인 모델 유지 원칙은 1.3을 따르고, 아래 후보는 위임할 작업의 모델 선정에 사용한다. 모델 이름과 지원 추론 수준은 현재 실행 도구가 노출하는 목록으로 확인하며, 문서에 있다는 이유만으로 선택 가능하다고 보고하지 않는다.

- 공식 모델 참고: <https://developers.openai.com/api/docs/models/gpt-6-astra>
- `GPT-6 Astra` (`gpt-6-astra`): 메인 직접 개발·통합·최종 판단. 위임 가능한 독립 검토에도 필요하면 선택
- `GPT-5.6 Sol` (`gpt-5.6-sol`): 범위는 분리되지만 여러 근거를 대조해야 하는 조사·검토·구현 보조
...
3. 범위가 독립적이면서 여러 모듈의 근거 대조나 의미 검토가 필요한 보조 작업은 `gpt-5.6-sol`의 `high`를 검토한다. 독립 검토 자체의 난도가 더 높으면 `gpt-6-astra`의 `high` 또는 `xhigh`를 검토한다.
4. 구조 결정이나 중요 구현을 위임하기 위해 모델만 상향하지 않는다. 1.3의 중요 작업이면 메인 `gpt-6-astra` / `xhigh`가 직접 처리한다.
5. 초기 후보는 13.2의 점수와 13.4의 위험 판단으로 확정한다. 선택 불가 또는 기존 에이전트의 모델 변경 불가 시 1.3의 메인 회수 원칙을 적용한다.
...
| 6~8점 | `5.6 Sol` 또는 `6 Astra`; 중요 작업은 메인 직접 처리 | `grok-4.6` | 복잡도, 영향, 불확실성, 검증 비용이 큰 작업 |
...
추론 수준은 현재 모델과 실행 도구가 함께 지원하는 값만 사용한다. Codex 메인은 1.3의 사용자 지정 `xhigh` 유지 원칙을 따르며, Codex에서 아래 기준은 주로 서브에이전트의 추론 수준 조절에 사용한다. Grok의 모델·추론 수준은 13.0과 13.1.2를 따르고 Codex 메인의 고정값을 적용하지 않는다. 추론 수준과 하위 에이전트 생성 깊이를 혼동하지 않는다. Grok TUI `/effort`가 `low`/`medium`/`high`/`xhigh`만 받는 세션에서는 그 외 값을 TUI 추천값으로 쓰지 않는다.
...
| `xhigh` | 매우 높음 | 사용자가 지정한 Codex 메인 기본값. 서브에이전트에는 어려운 독립 검토처럼 추가 근거가 있을 때 적용 |
| `max` | 최대 | 가장 어려운 품질 우선 작업에만 사용하며 latency와 비용보다 정확성이 우선일 때 적용 |
| `ultra` | 울트라 | 현재 실행 도구에 노출되고 사용자가 명시한 경우에만 검토. 자동 위임을 포함하더라도 단일 서브에이전트·재위임 금지 규칙을 지킬 수 있어야 함 |
...
3. 서브에이전트의 `max`는 8점이거나 13.4의 중대한 상향 조건이 있고 `xhigh`보다 추가 탐색이 필요한 이유가 있을 때 검토한다. 지원하지 않는 값은 지정하지 않는다. 이 규칙으로 메인을 자동 상향하지 않는다.
4. 위임된 반복 실행과 정형 evidence 수집은 `low` 또는 `medium`을 검토하고, 원인 분석이 중요 작업으로 확대되면 메인이 회수한다. `ultra`의 자동 위임 동작이 1.3의 제한을 보장하지 못하면 선택하지 않는다.

### 13.4 정확도 및 위험 상향 규칙

아래 조건은 VATester 총점과 별도로 모델과 추론 수준을 상향한다.

1. Codex는 정확도·동등성·보안·데이터 손상 위험을 먼저 평가해 직접 처리 여부를 결정한다. 중요한 판단이면 메인 `gpt-6-astra` / `xhigh`가 담당한다. 독립적인 보조 검토만 위임할 때는 해당 위험을 검토할 역량이 있는 모델과 최소 `high`를 선택한다.
2. auth/role/scope, 외부 쓰기, 비가역 데이터 손실, 공개 API/schema/event payload, RTSP/WebRTC media path, release correctness의 설계·중요 구현·최종 판정은 메인이 직접 수행한다. 서브에이전트는 분리된 근거 수집이나 독립 검토를 보조할 수 있다. Grok 런타임이면 `grok-4.6`과 최소 `high`를 사용한다.
3. 여러 계약 경계에 걸친 보안 또는 데이터 손상 가능성, 전면 구조 변경, 거짓 PASS 위험이 있으면 Codex 메인이 회수하고 기존 `xhigh`에서 원인을 분석한다. 같은 낮은 모델에 실패한 작업을 이유 없이 반복 위임하지 않는다.
4. 위임 모델·추론 수준 상향도 최대 한 개·재위임 금지 규칙을 지켜야 한다. `max`나 `ultra`는 자동 상향값이 아니며 메인 선택을 바꾸는 근거로 사용하지 않는다.
...
선정 근거: 영향도 2, 불확실성 2, 검증 난이도 2, 변경 범위 2로 총 8점이다. 녹화 삭제·복구 계약과 여러 모듈의 종료 순서를 판단하므로 메인이 직접 처리한다. 점수에 따른 모델 전환은 하지 않는다.
