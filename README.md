...
최신 공개 버전 `2.7.0`은 `/ops/events` Incident Triage Board, Rule What-if Preview,
Operational Action Pack을 닫은 source-only GitHub Release입니다. 현재 source tree
`2.8.0`은 2.x 라인을 `2.8.0`과 `2.9.0`까지만 유지한다는 전제에서, 3.0의 대규모
