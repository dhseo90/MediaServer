...
현재 README 대표 이미지는 제품 shell 설명용입니다. 현재 source tree는 `v3.1.0`
Encoded Event Clip and Safe Sharing Expansion을, 최신 공개 GitHub Release는 `v3.1.0`
Encoded Event Clip and Safe Sharing Expansion을 가리키지만, 이 이미지를 v3.1.0 source
baseline 또는 v3.1.0 공개 릴리즈 증거로 쓰지 않습니다. 특히
...
## v3.1.0 S00 source baseline alignment
...
- v3.1.0 source tree와 v3.1.0 published source-only baseline을 분리 정렬하고, 직전
  v3.0.0 baseline은 historical reference로만 둡니다.
...
- image recapture, 직접 브라우저 검수, UI 풀테스트, 30분/120분, published metadata는
  `verify-v310-entry-baseline` PASS로 대체하지 않습니다.
