...
현재 README 대표 이미지는 제품 shell 설명용입니다. 현재 source tree는 `v3.7.0`
Site-Aware Operations and Safe Runbook Control Plane을 가리키고, 최신 공개 GitHub Release는
`v3.6.0` Operations Simulation and Safe Apply Readiness를 가리킵니다. 이 이미지를 v3.7.0 source baseline,
...
- v3.7.0 source roadmap과 v3.6.0 published source-only baseline을 분리하고,
  직전 v3.5.0 baseline은 historical reference로만 둡니다.
