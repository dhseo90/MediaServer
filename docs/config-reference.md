...
Auth on에서 직접 generic media 생성 route인 `POST /webrtc/session`, `POST /whep`, `POST /whip/publish`는 admin/operator `ops:read` 또는 `lab:read` 권한이 필요합니다. 직접 WebSocket metadata side-channel인 `/ws/va-metadata`는 Lab/custom-client 경로이므로 admin/operator 또는 `lab:read` 권한이 필요합니다. Auth off에서는 기존 개발/자동화 호환을 위해 계속 허용됩니다. Viewer/client 계정은 source locator를 직접 보내지 않고 `/client/api/views/{viewId}/webrtc/session` wrapper를 사용해야 합니다.
