...
| 64 | V390-REVIEW4-64 | Structure | 승인된 구조 안정화 slice 실행 | P0 | 진행 중 / Slice 1 완료 | Current `v3.9.0`에서 composition root를 먼저 분리했습니다. `src/main.cpp` 245→8줄, 신규 `media_server::application::RunMediaServerApplication`, shared SessionManager와 RTSP→HTTP start/HTTP→RTSP→EventStorage cleanup/auth-user CLI 불변을 build·compiled CLI·execution oracle로 확인했습니다. Historical REVIEW4-51 fixture/graph SHA는 보존하고 별도 current graph에서 production 150/C++ 75/owner 10, target 위반 25→22, SCC 8을 기록합니다. Slice 2 route/API handler부터 6 verifier/docs까지는 미진행이며 각 slice는 allowed file set, dependency direction, API/schema/media-path 불변, rollback point를 독립적으로 닫아야 합니다. | 5.6 Sol | 최대 (max) | 영향도 2, 불확실성 2, 검증 난이도 2, 변경 범위 2, 총 8점. 43,266-line server와 10,217-line UI를 포함한 전면 구조 변경으로 xhigh보다 추가 탐색과 회귀 검증이 필요한 최난도 작업 |
