...
| 65 | V390-REVIEW4-65 | Final Test | current HEAD 독립 acceptance와 evidence | P0 | latest UI 35/424 UI-036 narrative-token/structured-material boundary FAIL 전면 보정 / actual UI 재실행 전 checkpoint 대기 | 사용자 실행 인터페이스는 `./test_server_30min.sh`, `./test_server_120min.sh`, `./test_ui.sh`, `./test_release.sh` 네 무인자 launcher로 유지합니다. Source `1f39e863`의 standalone UI actual run은 UI-001~035 중 34건 PASS 뒤 UI-036에서 `/ops/rules`의 안전한 `write 없음` 설명을 실제 write material로 오인해 executed/pass/fail/not-run `35/34/1/389`로 중단됐고 Policy v4는 not-run, cleanup은 PASS했습니다. 전수 확인 결과 9개 negative-boundary family의 narrative token이 288개 core oracle DOM substring 금지로 잘못 투영됐습니다. Core oracle은 narrative token raw DOM ban을 전부 제거하고 `WritePerformed`, `registryWrite`, `providerCall`, `viewerClientExposure`, `sourceUrl` 같은 구체 상태를 `forbiddenMaterialTokens`로 분리합니다. `...=false`·`없음`·`비노출`은 허용하고 `...=true`·실제 credential/raw/source 값은 FAIL하며, write/mutation은 forbidden network와 authoritative before/after equality로 계속 검증합니다. 제품 C++/API/UI는 변경하지 않았습니다. Semantic candidate `917c7917895ff4bcd9b7fe801f797437eb1bac1ea5706385262cab89cbc22e9c`와 986행 approval은 불변입니다. 공식 generator는 canonical 424를 변경하지 않고 native 288개 core 행의 SHA/digest/provenance만 갱신했으며 non-hash product/workflow drift는 0입니다. 실제 UI/Policy v4/30분/120분/release acceptance는 이 correction에서 재실행하지 않습니다. | 5.6 Sol | 높음 (high) | 영향도 2, 불확실성 1, 검증 난이도 2, 변경 범위 1, 총 6점. 9개 negative-boundary family의 false-positive를 공통 모델에서 차단하는 P0 작업 |
