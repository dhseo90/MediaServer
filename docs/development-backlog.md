...
| 64 | V390-REVIEW4-64 | Structure | 승인된 구조 안정화 slice 실행 | P0 | 진행 중 / Slice 2 완료 | Slice 1 composition root에 이어 `/ops/api/actions/execution-deferral-decision`의 exact GET path/status/body/no-store owner를 `ops_action_execution_deferral`로 이동했습니다. Outer principal guard와 non-GET 404 fallback은 transport에 보존했고 build, action 8/0, v3.8 boundary 5/0, actual HTTP auth 401/auth-off GET 200 no-store/POST 404, execution oracle 8/0을 확인했습니다. Historical SHA는 보존하며 current graph는 production 152/C++ 76/owner 10, target 위반 25→21, SCC 8, server 43,186줄입니다. Slice 3~6은 미진행입니다. | 5.6 Sol | 최대 (max) | 영향도 2, 불확실성 2, 검증 난이도 2, 변경 범위 2, 총 8점. 43,186-line server와 10,217-line UI를 포함한 전면 구조 변경으로 xhigh보다 추가 탐색과 회귀 검증이 필요한 최난도 작업 |
