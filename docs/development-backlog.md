...
- 완료 범위: Source/View summary, active session/stream/tap summary, recent event summary, warning/stale summary, 고밀도 source/view tile, `all/attention/published/unassigned` focus, `view/source` 검색, attention/미배정 카운터, source health/active tap/recent event/evidence drill-down, 선택형 read-only media preview tile, 간단한 runtime/event timeline, 채널/이벤트/클라이언트/룰 화면으로 이어지는 action 링크, `#channel`/`#view`/`#q` hash 기반 deep-link를 `/ops/home`과 `/ops/live` 연계 화면에 표시합니다. `/ops/live`는 자동 media session을 열지 않고, preview도 수동 시작일 때만 하나의 PublishedView 세션을 사용합니다.
- 후속: 더 긴 scenario/runtime timeline, 장애 대응 action의 쓰기/복구 API, preview overlay mode 세분화와 attention 다중 preview 정책입니다.
