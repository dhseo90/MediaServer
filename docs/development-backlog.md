...
| 8 | v4.0.0 (8) stabilization and release readiness | P0 | 완료 | 네 테스트 영역 판정 기록. 30분은 `executed-pass`(3차 `./test_server_30min.sh`, source `b53e8af1`). UI는 `executed-pass`(`./test_ui.sh` exact 424/424, qualifier `uiFulltestPass=true`). 120분은 conditional-not-run. close-out은 dry-run. tag/GitHub Release/PR/published metadata는 미생성. `test/fixtures/v400_release_readiness.json`과 `scripts/internal/verify_v400_release_readiness.mjs`, `./server.sh verify-v400-release-readiness`를 추가. 이 스텝 완료와 이 명령 PASS는 출시 가능 판정이 아님 |
