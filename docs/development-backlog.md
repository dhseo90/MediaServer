...
| 65 | V390-REVIEW4-65 | Final Test | current HEAD 독립 acceptance와 evidence | P0 | actual UI infrastructure FAIL 원천 보정 / correction checkpoint 뒤 UI 재실행 대기 | 개별 재검증은 `./test_server_30min.sh`, `./test_server_120min.sh`, `./test_ui.sh`로 각각 OS temp 30분, OS temp 직접 승인 120분, repository-local transient exact 424+Policy v4만 실행합니다. 전체 acceptance는 `./test_release.sh`가 저장소 canonical output에서 안정화/feature gate/30분/UI/cleanup/report/final integrity를 실행하며 120분은 AGENTS 7.6.2 trigger가 있을 때만 자동 선택하고 없으면 `not-required`로 남깁니다. Source `911446e8` actual run은 feature gate 35/35와 real 30분을 통과한 뒤 manifest drift로 browser 전 0/424 FAIL했고, source `2bc7ad05` standalone UI run은 OS temp output과 repository-only Policy v4 root의 모순으로 UI-001~003 뒤 infrastructure FAIL했습니다. 후자는 case assertion이 아니며 UI-004 partial screenshot, producer 예외의 child summary 누락, orphan duplicate cleanup FAIL이 연쇄됐습니다. 현재 보정은 UI output을 `.media_server.test/v3.9.0/ui-acceptance-current`로 고정하고 invalid root를 preflight에서 거부합니다. Runner는 raw `CAPTURED`만 stage success로 인정하되 `uiFulltestPass=false`를 유지하고, bootstrap/producer/case 실패에 424행 ledger·failure phase·not-run·cleanup 경계를 기록하며 referenced partial artifact만 보존해 dedupe합니다. Contract PASS는 actual UI PASS가 아니며 correction checkpoint 뒤 `./test_ui.sh`를 정확히 1회 재실행해야 합니다. | 5.6 Terra | 높음 (high) | 영향도 2, 불확실성 1, 검증 난이도 2, 변경 범위 1, 총 6점. 테스트 인프라 신뢰 경계와 재현성 상향 적용 |
