...
- 검증: `./server.sh verify-auth-bootstrap`, `./server.sh verify-auth-users`, `./server.sh verify-auth-routes`를 추가해 setup/login/session, admin user management/invite/request 보존, root/ops/client/lab route guard, same-origin CORS/preflight를 자동 smoke로 확인합니다.
