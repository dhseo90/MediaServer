...
| 65 | V390-REVIEW4-65 | Final Test | current HEAD 독립 acceptance와 evidence | P0 | latest UI 8/424 UI-009 fresh-role setup isolation FAIL 원천 보정 / checkpoint commit·push 대기 | 개별 재검증은 `./test_server_30min.sh`, `./test_server_120min.sh`, `./test_ui.sh`로 각각 OS temp 30분, OS temp 직접 승인 120분, repository-local transient exact 424+Policy v4만 실행합니다. 전체 acceptance는 `./test_release.sh`가 저장소 canonical output에서 안정화/feature gate/30분/UI/cleanup/report/final integrity를 실행하며 120분은 AGENTS 7.6.2 trigger가 있을 때만 자동 선택하고 없으면 `not-required`로 남깁니다. Source `911446e8` actual run은 feature gate 35/35와 real 30분을 통과한 뒤 manifest drift로 browser 전 0/424 FAIL했고, source `2bc7ad05` standalone UI run은 OS temp output과 repository-only Policy v4 root의 모순으로 UI-001~003 뒤 infrastructure FAIL했습니다. Source `9c5410e8` standalone UI run은 root/cleanup 보정이 실제 PASS했지만 whole-file implementation binding drift로 manifest-validation에서 0/424 FAIL했습니다. Source `a762df04` standalone UI run은 UI-004 cleanup 최종 로그인이 auth users 파일을 다시 써서 4/424에서 infrastructure oracle FAIL했습니다. Source `28378c74` standalone UI run은 UI-001~008 중 7건 PASS 뒤 UI-009의 제품 UI assertion 전에 fresh-role setup `/login`이 `lastLoginAt`/`lastLoginIp`를 기록해 authoritative state drift로 8/424에서 중단됐습니다. 현재 원천 보정은 login 직전 users 파일만 snapshot하고 발급된 session/cookie와 whoami 결과를 유지하면서 login audit write만 성공·예외 모두 복원합니다. UI-009 전용 예외나 drift/read-only 완화는 없습니다. Contract PASS는 actual UI PASS가 아니며 checkpoint commit·push 뒤 새 commit에서 `./test_ui.sh`를 정확히 1회 재실행해야 합니다. | 5.6 Sol | 높음 (high) | 영향도 2, 불확실성 1, 검증 난이도 2, 변경 범위 1, 총 6점. 인증 mutation과 cleanup oracle 정확도 상향 적용 |
