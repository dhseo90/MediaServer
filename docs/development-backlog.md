...
| 65 | V390-REVIEW4-65 | Final Test | current HEAD 독립 acceptance와 evidence | P0 | latest UI 26/424 UI-027 credential label/value boundary FAIL 원천 보정 / actual UI 재실행 전 checkpoint 대기 | 사용자 실행 인터페이스는 `./test_server_30min.sh`, `./test_server_120min.sh`, `./test_ui.sh`, `./test_release.sh` 네 무인자 launcher로 유지합니다. Source `2cad7bb9`의 standalone UI actual run은 UI-001~026 중 25건 PASS 뒤 UI-027에서 `credential` 안전 경계 label을 실제 credential material로 오인해 executed/pass/fail/not-run `26/25/1/398`로 중단됐고 Policy v4는 not-run, cleanup은 PASS했습니다. 제품 `/ops/vlm`은 `credential env 준비 전`, `credential 비노출`처럼 저장·호출이 없음을 설명하며 실제 credential 값은 렌더링하지 않습니다. Oracle은 credential 계열 DOM을 단순 substring으로 보지 않고 key/value assignment 또는 credential 식별 form control의 비어 있지 않은 값으로만 material 노출로 판정합니다. `credential=sk-...`와 credential input 값은 계속 FAIL합니다. 구조 분리 뒤 stale VLM verifier도 UI markup을 `product_ui_server_pages.cpp`, API/route guard를 WebRTC source bundle에서 각각 확인하도록 소유 경계를 복구했습니다. Fresh semantic candidate `917c7917895ff4bcd9b7fe801f797437eb1bac1ea5706385262cab89cbc22e9c`는 feature/pass 변경 0, 978 carry-forward + UI-022/025/026/027/028/030/031/034 독립 검토로 986/986 approval을 구성했습니다. 실제 UI/Policy v4/30분/120분/release acceptance는 이 correction에서 재실행하지 않습니다. | 5.6 Sol | 높음 (high) | 영향도 2, 불확실성 1, 검증 난이도 2, 변경 범위 1, 총 6점. Credential redaction과 exact DOM oracle 정확도를 함께 보존하는 P0 작업 |
