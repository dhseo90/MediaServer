...
- 완료 범위: Source/View summary, active session/stream/tap summary, recent event summary, warning/stale summary, 고밀도 source/view tile, `all/attention/published/unassigned` focus, `view/source` 검색, attention/미배정 카운터, source health/active tap/recent event/evidence drill-down, 간단한 runtime/event timeline을 `/ops/home`과 `/ops/live`에 표시합니다. `/ops/live`는 자동 media session을 열지 않습니다.
- 후속: 실제 media preview tile, 더 긴 scenario/runtime timeline, 장애 대응 action을 한 화면에서 연결합니다.
