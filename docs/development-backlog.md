...
| 65 | V390-REVIEW4-65 | Final Test | current HEAD 독립 acceptance와 evidence | P0 | actual UI pre-execution FAIL 보정 / correction checkpoint 뒤 canonical 재실행 대기 | 개별 재검증은 `./test_server_30min.sh`, `./test_server_120min.sh`, `./test_ui.sh`로 각각 OS temp에서 30분만, 직접 승인된 120분만, exact 424+Policy v4만 실행합니다. 전체 acceptance는 `./test_release.sh`가 저장소 canonical output에서 안정화/feature gate/30분/UI/cleanup/report/final integrity를 실행하며 120분은 AGENTS 7.6.2 trigger가 있을 때만 자동 선택하고 없으면 `not-required`로 남깁니다. Source `911446e8` actual run `v390-test-acceptance-20260717153927-36826`은 feature gate 35/35와 real 30분 2,360.544초(118 PASS/0 FAIL/2 제외)를 통과한 뒤 `ui-exact-424` implementation binding drift로 browser case 실행 전 0/424에서 FAIL했습니다. UI environment/listener/temp-root cleanup은 실제 PASS였지만 child summary가 생성되지 않아 root cleanup은 FAIL했고 Policy v4·120분 판정/실행·final integrity는 not-run입니다. 공식 generator의 hash-only binding 갱신, 조기 native manifest contract gate와 structured pre-execution summary/cleanup 판정을 보강했으며 다음 canonical 실행은 build 뒤 current 36개 feature gate를 사용합니다. Canonical FAIL과 30분 PASS evidence는 그대로 보존하며 correction checkpoint 뒤 clean HEAD 전체 재실행이 남았습니다. | 5.6 Sol | 높음 (high) | 영향도 2, 불확실성 1, 검증 난이도 2, 변경 범위 1, 총 6점. release correctness 검증 상향 적용 |
