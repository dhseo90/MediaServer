...
- 후속: 더 긴 scenario/runtime timeline, 장애 대응 action의 쓰기/복구 API, attention 슬롯 우선순위 정교화입니다. preview 실동작 smoke는 selector/API 계약 smoke에 더해 `start/stop/restart`, `raw/va-overlay/va-rule` payload, dual-slot cleanup 검증까지 반영했습니다.
