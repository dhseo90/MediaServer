...
| 65 | V390-REVIEW4-65 | Final Test | current HEAD 독립 acceptance와 evidence | P0 | actual feature-gate FAIL 보정 완료 / correction checkpoint 뒤 canonical 재실행 대기 | 개별 재검증은 `./test_server_30min.sh`, `./test_server_120min.sh`, `./test_ui.sh`로 각각 OS temp에서 30분만, 직접 승인된 120분만, exact 424+Policy v4만 실행합니다. 전체 acceptance는 `./test_release.sh`가 저장소 canonical output에서 안정화/feature gate/30분/UI/cleanup/report/final integrity를 실행하며 120분은 AGENTS 7.6.2 trigger가 있을 때만 자동 선택하고 없으면 `not-required`로 남깁니다. `9d68f7d9` actual run은 build 뒤 `v390-deferred-product-owner-signoff`에서 unrelated event-rule UI 변경이 `product_ui_page_scripts.cpp` whole-file digest를 흔들어 FAIL했고, 30분·exact UI·Policy v4·120분·final integrity는 not-run, cleanup/report는 PASS입니다. 실패 evidence는 canonical root에 그대로 보존했습니다. External VLM/Re-ID의 UI evidence를 두 product UI 파일의 unique ordered locator·bounded SHA-256·region-local token으로 이관하고 whole-file fallback을 금지했습니다. Contract 7/7, owner 6/6, semantic 986/986, approval 986/986, migration 9/9, feature evidence/inventory/index/script/docs focused gate가 PASS했지만 actual acceptance는 재실행하지 않았습니다. Correction checkpoint commit/push 뒤 clean HEAD canonical 재실행이 남았습니다. | 5.6 Sol | 높음 (high) | 영향도 2, 불확실성 1, 검증 난이도 2, 변경 범위 1, 총 6점. release correctness 검증 상향 적용 |
