...
| 65 | V390-REVIEW4-65 | Final Test | current HEAD 독립 acceptance와 evidence | P0 | latest UI 32/424 UI-033 audience-label/exposure-material boundary FAIL 원천 보정 / actual UI 재실행 전 checkpoint 대기 | 사용자 실행 인터페이스는 `./test_server_30min.sh`, `./test_server_120min.sh`, `./test_ui.sh`, `./test_release.sh` 네 무인자 launcher로 유지합니다. Source `5edff738`의 standalone UI actual run은 UI-001~032 중 31건 PASS 뒤 UI-033에서 `/ops/vlm`의 안전한 `client/viewer` 경계 설명을 실제 client exposure material로 오인해 executed/pass/fail/not-run `32/31/1/392`로 중단됐고 Policy v4는 not-run, cleanup은 PASS했습니다. Core oracle은 `client-viewer-boundary`의 `client`/`viewer` audience label을 구조화 response key 검사에는 유지하되 Ops DOM 금지 text에서는 제외합니다. `viewerClientExposure` 같은 실제 노출 신호는 유지하며 `viewerClientExposure=false`는 안전 attestation, `viewerClientExposure=true`는 FAIL입니다. 제품 C++/API/UI는 변경하지 않았습니다. Fresh semantic candidate `917c7917895ff4bcd9b7fe801f797437eb1bac1ea5706385262cab89cbc22e9c`와 986행 approval은 불변입니다. 공식 generator는 canonical 424를 변경하지 않고 native 30행의 SHA/digest/provenance만 갱신했으며 non-hash product/workflow drift는 0입니다. 실제 UI/Policy v4/30분/120분/release acceptance는 이 correction에서 재실행하지 않습니다. | 5.6 Sol | 높음 (high) | 영향도 2, 불확실성 1, 검증 난이도 2, 변경 범위 1, 총 6점. Client/viewer privacy 경계와 exact DOM oracle 정확도를 함께 보존하는 P0 작업 |
