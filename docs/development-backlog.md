...
| 64 | V390-REVIEW4-64 | Structure | 승인된 구조 안정화 slice 실행 | P0 | 진행 중 / current continuation Slice 16 완료 | Historical ordered Slice 1~5와 current continuation 1~16을 완료했습니다. Slice 16은 dependency-free Source/View application DTO와 common result를 추가해 transport의 concrete registry witness를 2→1로 줄였습니다. Focused RED 0/4 뒤 5/0, build100%, ONVIF atomicity19/0, lifecycle PASS, public owner6/0, transport5/0, bundle6/0, physical6/0, structure15/0입니다. Full source-registry 중 발견한 WebRTC local-description/ICE teardown 경쟁의 libnice SIGABRT는 GStreamer 1.28+ close reply, external signaling 직렬화, weak callback/active drain 뒤에만 pipeline을 NULL로 전환해 수정했습니다. Close가 non-REPLIED이면 객체를 프로세스 수명 quarantine에 명시적으로 보존하고 이후 WebRTC Start를 restart-required로 차단해 unsafe unref, background retry, 추가 누적을 금지합니다. MEDIA-025 순서 oracle, MEDIA-026 4×8 accepted ICE/DELETE race, SRC-001~068 actual 계약 5회 연속을 통과했습니다. Split 뒤 stale해진 companion 11개는 제품 model/route PASS를 확인하고 exact function/UI owner에 재결속했습니다. Actual graph는 production178/C++87, edge17, Policy v1 위반3, SCC0, target2/internal separation true, mixed max10,160입니다. 남은 transport→analysis/core-media/domain 방향과 parked evidence가 open이므로 REVIEW4-64/65 완료가 아닙니다. | 5.6 Sol | 최대 (max) | 영향도 2, 불확실성 2, 검증 난이도 2, 변경 범위 2, 총 8점. Physical split 뒤에도 dependency 방향과 final evidence를 순차 폐쇄해야 하는 최난도 작업 |
