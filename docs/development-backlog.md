...
항목은 해당 source 기능과 local verifier 기준을 뜻하며, GitHub Release publish 완료를
뜻하지 않습니다.
...
- 이 문서의 `v2.5.0` 항목은 source tree와 문서 재정리 기준입니다.
...
- tag/push/GitHub Release, PR merge/main sync/next branch sync, `verify-release-metadata --published`는 별도 명시 승인과 실제 실행이 있기 전까지 완료로 쓰지 않습니다.

## 최신 공개 기준: v2.4.0 Source Release Baseline

v2.4.0은 source-only/live-only 제품 경계를 유지하면서 Operator Event Review & Action
Workflow를 닫은 최신 공개 릴리즈입니다. 이 기준은 v2.5.0의 시작 baseline이며,
