...
| 20 | v3.9.0 (20) stabilization and release readiness | P0 | UI PASS/full release pending | Verification commit `47582fea`에서 clean Static과 actual `./test_ui.sh` exact `424/424`, Policy v4 qualified `424/424`, UI final-integrity/cleanup이 PASS했습니다. 남은 release close-out은 `v3.9.0` 승격 후 `./test_release.sh`로 안정화→30분→exact UI→120분 trigger/run→cleanup/final integrity를 동일 source binding에서 완료하는 것입니다 |
| 21 | v3.9.0 (21) actual test acceptance bundle 실행 모드 | P0 | UI actual PASS/release actual pending | UI-only canonical acceptance는 `47582fea`에서 PASS했습니다. 전체 release acceptance는 아직 미실행이며 기존 `docs/release-artifacts/v3.9.0/test-acceptance-current-final` 실패 artifact를 완료 evidence로 사용하지 않습니다. `./test_release.sh` PASS와 저장소 보존 evidence 갱신 전에는 release-ready가 아닙니다 |
