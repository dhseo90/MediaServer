...
| 65 | V390-REVIEW4-65 | Final Test | current HEAD 독립 acceptance와 evidence | P0 | canonical evidence lifecycle 완료 / checkpoint commit·push 및 clean-HEAD actual 재실행 대기 | 개별 재검증은 `./test_server_30min.sh`, `./test_server_120min.sh`, `./test_ui.sh`로 각각 OS temp에서 30분만, 직접 승인된 120분만, exact 424+Policy v4만 실행합니다. 전체 acceptance는 `./test_release.sh`가 저장소 canonical output에서 안정화/feature gate/30분/UI/cleanup/report/final integrity를 실행하며 120분은 AGENTS 7.6.2 trigger가 있을 때만 자동 선택하고 없으면 `not-required`로 남깁니다. 모든 launcher는 인자를 exit 64로 거부하고 output·port·account·role secret·browser dependency를 내부 준비하며 첫 실패 뒤 later stage를 `not-run`으로 기록합니다. Final integrity는 canonical summary/report와 필수 child evidence를 직접 검증하고 final manifest는 temp evidence reference를 거부합니다. Launcher 13/13·acceptance 15/15·final-integrity 12/12 contract가 PASS했습니다. 기존 diagnostics 94,338파일·2,029,763,448B만 삭제하고 runs 및 최상위 failure evidence를 포함한 67파일·1,333,967B를 보존했습니다. 실제 30분·exact UI·120분은 재실행하지 않았으며 checkpoint commit/push가 필요합니다. | 5.6 Sol | 높음 (high) | 영향도 2, 불확실성 1, 검증 난이도 2, 변경 범위 1, 총 6점. release correctness 검증 상향 적용 |
