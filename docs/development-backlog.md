...
| 64 | V390-REVIEW4-64 | Structure | 승인된 구조 안정화 slice 실행 | P0 | 진행 중 / Slice 3 완료 | Slice 1 composition root와 Slice 2 action route owner에 이어 Slice 3에서 SourceRegistry/PublishedView read-model의 `http_auth`/`auth::Principal` dependency를 제거했습니다. Transport-owned authorizer가 기존 operator/RequireScope wildcard 의미를 보존하고 write/persistence/ONVIF paired transaction은 변경하지 않습니다. Build, source registry actual HTTP/SRC-001~068, lifecycle active→idle, ONVIF crash/restart 19 cases, backup handoff 6/0, companion 25/25, execution oracle 9/0, script 11/0을 통과했습니다. Current graph는 production 152/C++ 76/owner 10, target 위반 25→20, SCC 8, server 43,195줄입니다. Feature evidence 8,752 drift는 Slice 6 entry blocker이고 Slice 4~6은 미진행입니다. | 5.6 Sol | 최대 (max) | 영향도 2, 불확실성 2, 검증 난이도 2, 변경 범위 2, 총 8점. 43,195-line server와 10,217-line UI를 포함한 전면 구조 변경으로 xhigh보다 추가 탐색과 회귀 검증이 필요한 최난도 작업 |
