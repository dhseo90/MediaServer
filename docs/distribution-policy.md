...
| Local binary bundle | `media_server`와 문서만 묶음 | `verify-release-bundle-dry-run`, `verify-bundle-policy --bundle-dir <dir>` |
| Runtime 포함 bundle | GStreamer/FFmpeg/plugin까지 복사 | 별도 법무 검토, source offer, license text, attribution |
| Container image without runtime | 사용자가 runtime layer를 직접 선택 | README에 설치 의존성 명시 |
| Container image with runtime | image 안에 FFmpeg/GStreamer/plugin 포함 | image filesystem에 대해 `verify-bundle-policy` 실행 |
