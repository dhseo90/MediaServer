...
- The current source tree starts the `v2.6.0` roadmap from the published
  incident-memory baseline and keeps `/ops/events` productization local and
  Ops-only until each step has verifier/UI evidence.
