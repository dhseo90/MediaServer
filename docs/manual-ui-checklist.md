...
사용합니다. 최신 공개 release 기준은 `v3.9.1`이고 직전 공개 제품 baseline은
`v3.9.0`이며, 현재 release 목표는 `v3.9.1`입니다.
...
아래 행은 v3.4.0 Operations Continuity Drill Workspace Step 6~9 UI 풀테스트 기준입니다.
정적 verifier나 API 응답만으로 PASS 처리하지 않고, 인앱 브라우저에서 실제 Ops/Client route를
열어 `Ops Continuity Drill Workspace`, `Approval-Gated Recovery Checklist`, `Client-safe Maintenance Digest`, `Drill Evidence Export and Cleanup Manifest`, `Field Bridge Condition Gates`와 비노출 경계를 확인해야 합니다.
