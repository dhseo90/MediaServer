...
| --- | --- | --- | --- | --- | --- |
| B11-C01 | 삭제 영수증 strict codec/계약 반례 | 집중·문서 검사 | 변경 영향 판정 후 | 후속 별도 실행 | 비대상: 내부 저장 |
...
내부 검증 단위이며 기존 V410-S06-I27~I34 8개·31 action을 유지한다. 사전 정의·결과는 중앙 테스트 기록 B10을 따른다.

| ID | 확인 항목 | 안정화 | 30분 | 120분 | UI |
| --- | --- | --- | --- | --- | --- |
| B10-U01 | 임시 계정 메모리 driver | PASS: 비출력·handoff 미생성·mode 경계、B10 준비기 6/6 | 비대상: 테스트 준비 | 비대상: 테스트 준비 | 실제 로그인은 U04/U05 |
| B10-U02 | driver 실패·취소·정리 | PASS: 취소·관측 실패·정리、UA/SF 21/21·proxy 11/11 | 비대상: 테스트 준비 | 비대상: 테스트 준비 | 실제 브라우저 정리 별도 |
| B10-U03 | 조작·시각·네트워크 증거 | 예정: 입력·redaction·경로/hash·실패 보존 | 비대상: UI 수집 | 비대상: UI 수집 | 준비 PASS는 실제 UI 아님 |
| B10-U04 | I30 실제 영상 제어 | 비대상: 실제 UI | 기존 PASS 영향 판정 | 실제 재생 영향 판정 | 예정: 재생/정지/탐색·프레임/Range·crash |
| B10-U05 | I27~I34 실제 UI 전수 | 비대상: 실제 UI | 기존 PASS 영향 판정 | 녹화120분 선수 | 예정: 31 action·역할·4 viewport×2 theme |
| B10-L01 | 현행 녹화 장시간 | 기존 집중 결과 대조 | 기존 PASS 유지 판정 | 승인: UI 통과 후120분·자원·보존/복구/삭제 | 대체 불가 |

## S11 B-07 누적·실제 통합

| ID | 확인 항목 | 안정화 | 30분 | 120분 | UI |
| --- | --- | --- | --- | --- | --- |
| B07-S01 | 큰 상세1/16/32 누적 | 예정: 실제 파일·공개 B 전이·비용 | 고정 뒤 판정 | 녹화 자원 영향, 미실행 | 비대상: CLI |
| B07-S02 | 삭제1020/2049 누적 | 예정: 삭제·충돌·복구·저장량 | 고정 뒤 판정 | 녹화 자원 영향, 미실행 | 비대상: CLI |
| B07-S03 | 비용·관측 | 예정: 준비/제품/관측 분리·기존상한 | 고정 뒤 판정 | 녹화 관측, 미실행 | 비대상: CLI |
| B07-H01 | 실제 HTTP·보존 | 예정:4초·15초·용량·재기동 | 고정 뒤 판정 | 녹화 전용 후속 | 실제 UI 별도 |
| B07-I01 | 현행5단계 통합 | 예정: 실제2출력·hash·권한·재기동 | 고정 뒤 판정 | 녹화 전용 후속 | 실제 UI 별도 |
| B08-R01 | 정확한 이전 snapshot 회수 | 집중 PASS: target 권위·기존 파일 보존·재개방. 누적비용은 미판정 | 보존·복구 변경 영향 판정 | 녹화 누적 직접 영향 | 비대상: CLI |
| B08-R02 | 회수 중단·손상·reader 반례 | 집중 PASS: PREPARED/PUBLISH_INTENT·바뀐 파일·열린 FD·lock-free 관측 Busy/단일세대 | 보존·복구 변경 영향 판정 | 녹화 누적 직접 영향 | 비대상: CLI |
| B08-C01 | 누적 비용·진단 | 단기 PASS: 1/16/32와 1,020/2,049 규모·현재 snapshot 1개·관측 손실 분리. 실제 JS/HTTP는 B08-H01에서 확인 | 보존·복구 변경 영향 판정 | 녹화 120분 별도 | 비대상: CLI |
| B08-H01 | 실제 HTTP·보존 재검증 | 단기 73/73 PASS: 실제 30초·HTTP 최대1.323초·정리. 장시간 자원 추세와 영향 변경 뒤 재검증은 별도 | 보존 변경 영향 판정 | 녹화 120분 별도 | 실제 UI 별도 |
| B08-I01 | 현행5단계 실제 통합 | 최종 PASS: 35+40+10+46+27개, 두 기동 각각 새 2출력·HTTP/파일 hash·기존 출력 보존·정리. 첫 실패 auth400, 뒤 HTTP4초, 임시 journal ENOENT 이력은 B08 기록에 보존 | 보존 변경 영향 판정 | 녹화 120분 별도 | 실제 UI 별도 |
| B08-I02 | 검증기 SQLite 임시 journal 경쟁 | 집중 9/9·관측기 회귀 67/67 PASS: 정확한 임시 journal ENOENT만 live 비원자 관측에서 인정. 실제 통합 최종 결과는 B08-I01 별도 | 30분 관측 경계 영향 판정 | 녹화 120분 관측 영향 판정 | 비대상: 내부 검증기 |
| B08-Q01 | 단일 요청의 반복 기록 검증 | 집중 PASS: 첫 엄격 검증 뒤 동일 요청 전체 archive 읽기 1회·typed 값 보존 | 비대상: 내부 단기 | 녹화 재생 영향 판정 | 비대상: 내부 저장 |
| B08-Q02 | 재사용 수명·상한 | 집중 PASS: 새 요청·복사·재개방·append·회전에서 엄격 검증, 0 예산 fallback | 비대상: 내부 단기 | 녹화 재생 영향 판정 | 비대상: 내부 저장 |
| B08-Q03 | 변조·소유 반례 | 집중 PASS: 같은 크기/mtime 복원·다른 행·inode·link·PID·foreign 거부, FD 수명 | 비대상: 내부 단기 | 녹화 재생 영향 판정 | 비대상: 내부 저장 |
| B08-Q04 | Catalog 소비자·구성 교차 | 집중 PASS: media eligibility·hold와 지원·비지원 구성; 삭제 영향 회귀는 별도 | 비대상: 내부 단기 | 녹화 재생 영향 판정 | 비대상: 내부 저장 |
| B09-F01 | 최종 소스 결속·단기 | 예정: build/auth/media/environment/docs/inventory·증거 영향 | 30분 범위 재판정 | 120분 범위 재판정 | 실제 UI 범위 재판정 |

## S11 B-06 현행 검증 연결

| ID | 확인 항목 | 안정화 | 30분 | 120분 | UI |
| --- | --- | --- | --- | --- | --- |
| B06-V01 | 종료 Catalog 진단 | PASS: B/legacy·손상·원본 불변·안전한 분류 | 비대상: 도구 자체 | 녹화 복구 검사에 연결, 미실행 | 비대상: CLI |
| B06-V02 | 현행 UI seed | PASS: B 생성/재개방·기존 독립 기대값 | 비대상: 도구 자체 | 비대상: UI 준비 | 준비 자체검사; 실제 UI 미실행 |
| B06-V03 | live 원장 관측 | PASS: C++ codec·순서/손상/회전·실제 writer/삭제 | 고정 뒤 영향 판정 | 녹화 관측기에 연결, 미실행 | 비대상: CLI |
| B06-V04 | 용량·실행 연결 | PASS: 현행 파일 계상·hold cache·구형 소비자 분리 | 고정 뒤 영향 판정 | 녹화 자원 관측 연결, 미실행 | seed 연결만, 실제 UI 미실행 |
