...
| 항목 | 현재 상태 | 결론 |
| --- | --- | --- |
| 기능 ID 목록 | 525개 기능 ID를 `UI-*`, `AUTH-*`, `SRC-*`, `RULE-*`, `EVT-*`, `CLIENT-*`, `MEDIA-*`, `LAB-*`, `SAFE-*`, `OPS-*`로 분리 | 기준표 작성 완료 |
...
| `SAFE-001`~`SAFE-084` | safety/boundary verifier family | schema/media/auth/UI automation 불변 조건 |
