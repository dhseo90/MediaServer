...
| --- | --- | --- |
| `UI-001`~`UI-018`, `UI-022`~`UI-035` | `verify-auth-bootstrap`, `verify-auth-routes`, `verify-ops-client-ui`, `verify-ops-route-boundaries`, `verify-vlm-install-connection-ui`, `verify-vlm-profile-storage`, `verify-vlm-runtime-status-ui`, `verify-vlm-evaluation-result-workflow`, `verify-vlm-review-action-workflow`, `verify-vlm-ops-event-review-ui`, `verify-vlm-privacy-transfer-guard` | route/shell/404/VLM dry-run/profile/privacy/runtime status/evaluation result/event review/action guard UI 경계 |
...
| `EVT-001`~`EVT-035` | `verify-va-event-coverage-report`, `verify-va-events`, `verify-ops-event-review-inbox`, `verify-ops-event-records-scope`, `verify-ops-alert-delivery-integrations`, `verify-va-runtime-console`, `verify-vlm-event-evidence-extraction`, `verify-vlm-observation-sidecar`, `verify-vlm-event-explanation-hints`, `verify-vlm-review-action-workflow`, `verify-vlm-ops-event-review-ui`, `verify-vlm-summary-search-candidates`, `verify-vlm-rule-suggestion-candidates` | event log 육안 확인은 UI 풀테스트 |
...
| --- | --- | --- | --- |
| 기능/route 목록 freeze | `v2.0.0 Pre-Test Update List`, `UI-022`~`UI-035`, `/ops/vlm`, `/ops/events`, `/client/events`, client redaction route가 결과 템플릿에 있음 | 리스트/템플릿 수정 후 긴 테스트 시작 전 재검수 | 긴 테스트 미시작 상태이므로 30분/120분/UI 재시작 없음 |
