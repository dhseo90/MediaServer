...
| 항목 | 현재 상태 | 결론 |
| --- | --- | --- |
| 기능 ID 목록 | 513개 기능 ID를 `UI-*`, `AUTH-*`, `SRC-*`, `RULE-*`, `EVT-*`, `CLIENT-*`, `MEDIA-*`, `LAB-*`, `SAFE-*`, `OPS-*`로 분리 | 기준표 작성 완료 |
...
| `SAFE-001`~`SAFE-078` | safety/boundary verifier family | schema/media/auth/UI automation 불변 조건 |
| `OPS-035`~`OPS-048` | ops evidence/readiness verifier family | real operational backup, release publish, UI 풀테스트 evidence가 아님 |
