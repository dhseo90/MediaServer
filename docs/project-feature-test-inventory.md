...
실행 전 등록이다. 후보 PASS를 실제 게시/복구 PASS로 대체하지 않는다.

| ID | 확인 항목 | 안정화 | 30분 | 120분 | UI |
| --- | --- | --- | --- | --- | --- |
| B04-T01 | 정상 전환·권위 | 예정: 원본/B 동등·old owner·proof | 미실행: 고정 뒤 판정 | 미실행: 녹화 최종 | 비대상: 내부 저장 |
| B04-T02 | crash 내구 경계 | 예정: receipt/link/marker/manifest 중단 | 미실행: 고정 뒤 판정 | 미실행: 녹화 최종 | 비대상: 내부 저장 |
| B04-T03 | PREPARED 복구 | 예정: source/domain·inode/hash·부재 결박 | 미실행: 고정 뒤 판정 | 미실행: 녹화 최종 | 비대상: 내부 저장 |
| B04-T04 | 승격 소유 | 예정: nlink2·foreign/불명확 보존 | 미실행: 고정 뒤 판정 | 미실행: 녹화 최종 | 비대상: 내부 저장 |
| B04-T05 | 불확실 상태 | 예정: intent·손상·조회오류·모순 거부 | 미실행: 고정 뒤 판정 | 미실행: 녹화 최종 | 비대상: 내부 저장 |
| B04-T06 | 회전 연결 | 예정: checkpoint 중단·기존 비용/guard 유지 | 미실행: 고정 뒤 판정 | 미실행: 녹화 최종 | 비대상: 내부 저장 |

## S11 B-04 게시 영수증 값 계약

실행 전 등록 후 내부 값 codec74개·빌드 통과, 교체 descriptor/ID 호환 보완 후101개·manifest 영향 회귀 통과.
보완 이후 전체 서버 빌드는 게시 통합 소스 고정 뒤 판정한다. 실제 게시·복구·삭제의 검증 권위가 아니다.
