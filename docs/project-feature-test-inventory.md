...
이 표는 결론 표입니다. 각 기능이 코드에 있는지, 제품 UI에 있는지, 자동/수동 테스트가
있는지를 같은 기준으로 표시합니다. `없음`은 해당 축에 제품 기능으로 존재하지
않는다는 뜻이며, `부분`은 다른 UI가 간접 소비하거나 요약만 표시한다는 뜻입니다.

| 기능 | 코드상 로직 | UI 존재 | 테스트 | 결론 |
| --- | --- | --- | --- | --- |
| Server lifecycle/config | 있음: `server.sh`, `src/main.cpp`, `src/app_config.cpp` | 없음: CLI/log/env 중심 | 있음: `build`, `verify-server-start-modes`, `verify-release-metadata`, `verify-bundle-policy` | 코드 + 테스트 있음, 제품 UI 없음 |
| Source ingest: file/RTSP/HTTP-HLS/WHEP/Published WebRTC | 있음: `source_factory.*`, `/ops/api/sources`, `/ops/api/views`, `/whep` | 있음: `/ops/sources`, `/client/live` | 있음: `verify-codecs`, `verify-uri-longrun`, `verify-ops-source-lifecycle`, `verify-ops-client-ui` | 코드 + UI + 테스트 있음 |
| WHIP publish ingest | 있음: `/whip/publish`, `/whip/publish/session/{id}`, `webrtc_source_registry.*` | 부분: 직접 publish UI 없음. `/ops/sources`에서 Published WebRTC sourceId 연결만 제공 | 있음: `whip_publish_test.py`가 `verify-codecs`/`verify-webrtc-ice`에서 소비, auth denial은 `verify-auth-routes` | 코드 + 테스트 있음, 직접 제품 UI 없음 |
| RTSP output | 있음: `gstreamer_rtsp_server.*`, `rtsp_egress_session.*`, VA overlay route | 부분: URL copy만 UI 제공 | 있음: `verify-codecs`, `verify-va`, `verify-rtsp-va-overlay-policy` | 코드 + 테스트 있음, UI는 URL 노출/복사 수준 |
| WebRTC output/WHEP | 있음: `/webrtc/session`, `/whep`, client scoped WebRTC proxy, DataChannel metadata | 있음: `/client/live`, `/ops/rules` preview | 있음: `verify-webrtc-ice`, `verify-webrtc-va-metadata`, `verify-ops-client-ui`, `verify-client-tile-disconnect` | 코드 + UI + 테스트 있음 |
| Auth/session | 있음: `/setup`, `/login`, `/logout`, `/password/change`, `/invite/setup`, `/auth/whoami` | 있음: Auth shell | 있음: `verify-auth-bootstrap`, `verify-auth-users`, `verify-auth-routes` | 코드 + UI + 테스트 있음 |
| Role/scope | 있음: `http_auth.*`, route guard, admin/operator/viewer/integrator scopes | 부분: `/ops/users`에서 관리, route별 redirect/403으로 확인 | 있음: `verify-auth-users`, `verify-auth-routes`, `verify-ops-client-ui` | 코드 + UI 일부 + 테스트 있음 |
| Client access request | 있음: `/client/api/access-requests`, `/ops/api/access-requests/*` | 있음: `/client/request-access`, `/ops/users` pending request panel | 있음: `verify-auth-users`, `verify-auth-routes`, `verify-ops-click-e2e` | 코드 + UI + 테스트 있음 |
| Ops shell/nav | 있음: `/ops`, `/ops/home`, `/ops/dashboard`, `/ops/sources`, `/ops/rules`, `/ops/users`, `/ops/events` | 있음: Ops product shell. `/ops/events`는 direct diagnostic route | 있음: `verify-ops-client-ui`, `verify-ops-click-e2e`, `verify-ops-tables-layout`, `verify-ops-route-boundaries` | 코드 + UI + 테스트 있음 |
| Ops dashboard/runtime status | 있음: `/ops/api/runtime/status`, `/ops/api/source-health`, `/ops/api/diagnostics/log-tail` | 있음: `/ops/dashboard` | 있음: `verify-ops-root-cause-panel`, `verify-ops-operator-incident-timeline`, `verify-va-runtime-console` | 코드 + UI + 테스트 있음 |
| Source/channel management | 있음: source/view CRUD, bulk channel action, ONVIF import draft APIs | 있음: `/ops/sources` | 있음: `verify-ops-source-lifecycle`, `verify-ops-source-group-site-management`, `verify-ops-channel-bulk`, `verify-onvif-ops-sources-ui` | 코드 + UI + 테스트 있음 |
| Rule/profile/scenario management | 있음: `/ops/api/rules/catalog`, `/lab/analysis/rules`, `/lab/analysis/profiles`, `/lab/analysis/va-rules` | 있음: `/ops/rules` | 부분: `verify-rule-ui`, `verify-ops-rules-roundtrip`, `verify-ops-scenario-builder-ui`, `verify-ops-scenario-presets`는 UI/저장/프리셋 검증. 실제 영상 이벤트 발생 전수 테스트 아님 | 코드 + UI + 부분 테스트 있음 |
| User management | 있음: `/ops/api/users`, `/ops/api/invites`, `/ops/api/audit`, last-admin guard | 있음: `/ops/users` | 있음: `verify-auth-users`, `verify-ops-audit-trail`, `verify-ops-audit-persistence` | 코드 + UI + 테스트 있음 |
| Event review/evidence | 있음: `/ops/api/events/status`, `/ops/api/events/reviews`, `/lab/analysis/events/evidence*` | 있음: `/ops/events`, Dashboard event links | 있음: `verify-ops-event-records-scope`, `verify-ops-event-review-inbox`, `verify-event-post` | 코드 + UI + 테스트 있음. 실제 이벤트 전수 수동 evidence는 없음 |
| Alert delivery | 있음: `/ops/api/alerts/deliveries`, `/ops/api/alerts/deliveries/test` | 있음: `/ops/events` | 있음: `verify-ops-alert-delivery-integrations` | 코드 + UI + 테스트 있음 |
| Client live workspace | 있음: `/client/api/views`, live layout preference, scoped WebRTC APIs | 있음: `/client`, `/client/live` | 있음: `verify-client-live-workspace`, `verify-client-source-dock-events`, `verify-client-tile-disconnect`, `verify-client-tile-info-overlay-health`, `verify-client-saved-views-layout-presets` | 코드 + UI + 테스트 있음. 수동 다중 권한/다중 채널 evidence는 없음 |
| Client dashboard/events summary | 있음: `/client/api/views/{viewId}/dashboard`, `/client/api/views/{viewId}/events` | 있음: `/client/dashboard`, `/client/events` alias | 있음: `verify-client-dashboard-polish`, `verify-ops-client-ui` | 코드 + UI + 테스트 있음 |
| VA detector/runtime | 있음: dummy/YOLO detector, ONNX layout, adaptive fps/queue, redaction overlay, image/tap APIs | 부분: `/ops/rules` preview와 `/ops/dashboard` summary가 소비. full Lab UI 없음 | 있음: `verify-yolo-layouts`, `verify-adaptive`, `verify-image-analysis`, `verify-redaction`, `verify-va` | 코드 + UI 일부 + 테스트 있음 |
| Tracking/scenarios | 있음: `event_rule_engine.*`, `scenario_engine.*`, intrusion/line/dwell/loitering/zone/re-entry/wrong-direction scenarios | 부분: `/ops/rules` 설정, `/ops/dashboard` 품질 summary. 모든 scenario 실제 발생 UI evidence 없음 | 부분: `verify-analysis-state`, `verify-va-events`, `verify-va-replay`, `replay-va-metadata`는 fixture/replay 중심. 실제 영상 + 브라우저 UI 전수 이벤트 발생 테스트 아님 | 코드 + UI 일부 + 부분 테스트 있음. 실제 이벤트 전수 evidence 없음 |
| Runtime metadata: DataChannel/SSE/WS/state/metrics | 있음: `/lab/analysis/metadata/stream`, `/ws/va-metadata`, state/metrics dump | 부분: Ops dashboard summary만 소비. raw stream 제품 UI 없음 | 있음: `verify-webrtc-va-metadata`, `verify-va-metadata-sidechannel`, `verify-ws-metadata`, `verify-va-runtime-console` | 코드 + 테스트 있음, raw metadata 제품 UI 없음 |
| Event POST/storage/cleanup | 있음: event dispatcher, storage status, records, compaction, evidence bundle APIs | 부분: `/ops/events`와 Dashboard event panels가 소비 | 있음: `verify-event-post`, `verify-event-post-longrun`, `verify-ops-evidence-retention-cleanup` | 코드 + UI 일부 + 테스트 있음 |
| ONVIF support | 있음: `onvif_live_import.*`, `onvif_credential_provider.*`, `/ops/api/onvif/import-draft` | 있음: `/ops/sources` ONVIF source/import flow | 있음: `verify-onvif-*`, `verify-onvif-ops-sources-ui`, `verify-onvif-no-device-suite` | 코드 + UI + fixture/no-device 테스트 있음. 실장비 성공은 field gate |
| Integrator contract | 있음: Event POST/WebRTC/SSE/WS schema samples and redaction contract fixtures | 없음: 직접 제품 UI 없음 | 있음: `verify-integrator-contract-artifact`, metadata/event verifiers | 코드/fixture + 테스트 있음, 제품 UI 없음 |
| Release/public repo readiness | 있음: release docs/config/scripts, dependency notice/snapshot, bundle policy, actions security | 없음: 제품 웹 UI 대상 아님 | 있음: `verify-release-metadata`, `verify-actions-security`, `verify-public-repo-readiness`, `verify-release-closeout-helper` | 코드/문서/스크립트 + 테스트 있음, 제품 UI 없음 |
| Research boundaries: Re-ID/OC-SORT/BoT-SORT/DeepSORT/YouTube | 있음: analysis hooks, docs, fixtures, optional lab-only resolver | 부분: Re-ID/tracker option만 `/ops/rules`에서 보임. YouTube 제품 UI 없음 | 있음: `verify-reid-advanced-tracking`, `verify-oc-sort-benchmark-boundary`, `verify-bot-sort-deepsort-research-boundary` | 코드 + boundary 테스트 있음, 제품 default-on/UI 승격 없음 |
| Removed legacy product UI routes | 있음: `/lab`, `/lab/`, `/lab/rules`, `/lab/rules/`, `/lab/import`, `/lab/import/`, `/webrtc/test` 404-only guard | 없음: legacy HTML 없음. 200/HTML로 열리면 실패 | 있음: `verify-auth-routes`, `verify-ops-route-boundaries`, `verify-multichannel` skip notice | UI가 없어야 정상인 제거 검증 대상 |
| Support/release/config/sample artifacts | 있음: root docs, `.github`, `config`, `video` tracked artifact inventory | 부분: sample media는 `/ops/sources`, `/client/live`, `/ops/rules`가 간접 소비. 정책/config는 UI 없음 | 있음: `verify-project-inventory`, `verify-docs-links`, `verify-docs-ui-assets`, `verify-bundle-policy`, `verify-public-repo-readiness` | artifact + 테스트 있음. sample video 전체 이벤트 수동 evidence는 없음 |
...
| WHIP publish | external publisher ingest session/ICE/delete | `/whip/publish`, `/whip/publish/session/{id}` | 직접 제품 UI 없음. Published WebRTC source 연결은 `/ops/sources` | `verify-codecs`, `verify-webrtc-ice`, `whip_publish_test.py` |
...
| Rule/profile/scenario management | VA rules, event templates, profiles, scenario presets/builder, save validation, preview | `/ops/api/rules/catalog`, `/lab/analysis/rules`, `/lab/analysis/profiles`, `/lab/analysis/va-rules` | `/ops/rules` | UI/저장/프리셋: `verify-rule-ui`, `verify-ops-rules-roundtrip`, `verify-ops-scenario-builder-ui`, `verify-ops-scenario-presets`. 실제 이벤트 전수 테스트 아님 |
...
| Tracking/scenarios | object tracker, TrackStateManager, SceneContextBuilder, intrusion, line crossing, dwell, loitering, zone occupancy, re-entry, wrong direction, intrusion-after-line-crossing | `src/analysis/*scenario*`, `event_rule_engine.*`, `scenario_engine.*` | `/ops/rules` configuration, `/ops/dashboard` quality summary | fixture/replay 중심: `verify-analysis-state`, `verify-va-events`, `verify-va-replay`, `replay-va-metadata`. 실제 영상 + 브라우저 UI 전수 이벤트 발생 테스트 아님 |
...
- UI/Test 비교: `/client/live`, `/ops/rules` preview, RTSP URL copy와 direct WebRTC/WHEP/WHIP APIs가 소비합니다. `verify-codecs`, `verify-webrtc-ice`, `verify-webrtc-va-metadata`, `verify-rtsp-va-overlay-policy`, `whip_publish_test.py`가 일부 확인합니다.
