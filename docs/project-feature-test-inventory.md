...
| Rule/profile/scenario management | 있음: `/ops/api/rules/catalog`, `/lab/analysis/rules`, `/lab/analysis/profiles`, `/lab/analysis/va-rules` | 있음: `/ops/rules` | 있음: `verify-rule-ui`, `verify-ops-rules-roundtrip`, `verify-ops-scenario-builder-ui`, `verify-ops-scenario-presets` | 코드 + UI + 테스트 있음 |
...
| Tracking/scenarios | 있음: `event_rule_engine.*`, `scenario_engine.*`, intrusion/line/dwell/loitering/zone/re-entry/wrong-direction scenarios | 부분: `/ops/rules` 설정, `/ops/dashboard` 품질 summary. 모든 scenario 실제 발생 UI evidence 없음 | 있음: `verify-analysis-state`, `verify-va-events`, `verify-va-replay`, `replay-va-metadata` | 코드 + UI 일부 + fixture/replay 테스트 있음. 실제 이벤트 전수 수동 evidence 없음 |
