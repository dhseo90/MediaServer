...
| v2.2.0 S09 UI fulltest matrix / evidence | `UI-002`~`UI-004`, `UI-007`~`UI-010`, `UI-012`, `UI-014`~`UI-021`, `SAFE-018`~`SAFE-021`, [v220-ui-fulltest-matrix-evidence.md](./v220-ui-fulltest-matrix-evidence.md), `media-server.v220-ui-fulltest-matrix.v1` | `verify-v220-ui-fulltest-matrix-evidence`, `verify-manual-ui-evidence`, `verify-manual-ui-evidence-runner`, `verify-feature-inventory-coverage` | UI matrix/doc-only 변경이면 미실행. 버전 close-out 지시 시 30분 대상 | 미실행. 120분은 RC/high-risk/user approval 시 별도 판단 | S05~S08 route/control/interaction/responsive/theme/redaction/role guard를 `media-server.manual-ui-evidence-input.v1` evidence로 남김 | matrix 작성은 실행 evidence가 아니며 브라우저 UI 풀테스트 PASS, 30분/120분 PASS, published metadata PASS를 대체하지 않음 |
