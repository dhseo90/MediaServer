// 실제 원본 서비스 재현 전용. 제품 명령/등록 inventory가 아닌 보존 실행 source다.
...
const reportPath=path.join(path.dirname(self),'reproduction-home-guard.json');
...
let child, childExit, childEnv, before, tapId='', stage='S05-I27-G02', expectedRed=false;
let outcome='선행조건 미완료', failure='', rootOwned=false;
const report={schema:'media-server.s05.actual-identity-reproduction.v2',mode:'foreground',rows};
...
try{
  ...
  stage='S05-I27-L05';const created=await api('POST','/ops/api/sources',{sourceId:'9101',displayName:'S05 정체성 재현',kind:'file',file:'identity.mp4',enabled:true,recording:{enabled:true,continuousMaxBytes:268435456,eventMaxBytes:268435456,continuousMaxAgeMs:3600000,eventMaxAgeMs:3600000,revision:1}});
  ...
  const linked=report.events.every(e=>links.some(l=>l.event_id===e.eventId&&l.channel_id==='9101'));
  expectedRed=!linked;outcome=linked?'기대 RED 미재현: 연결 있음':'기대 RED: 실제 이벤트와 finalized numeric 채널 녹화가 있으나 link 없음';
  record(stage,linked,{expected:'동일 실제 영상의 numeric channel 9101 녹화 연결',eventChannelIds:report.events.map(e=>e.channelId),segmentChannelIds:segments.map(s=>s.channel_id),links,rejectionReason:'명시적 거부 로그가 없다면 원인 추정과 구분',fallbackPathExists:report.events.map(e=>e.fallbackPathExists)});
}catch(error){failure=redact(error.message);record(stage,false,{message:failure,classification:'예상 ID RED가 아닌 선행조건 실패'});}
finally{
  if(child&&!childExit){
    if(tapId){try{const d=await api('DELETE',`/lab/analysis/taps/${tapId}`);record('S05-I27-L12',true,{method:'DELETE',path:`/lab/analysis/taps/${tapId}`,status:d.status});}catch(error){record('S05-I27-L12',false,{error:redact(error.message)});}}
  ...
  report.outcome=outcome;report.failure=failure;report.expectedRed=expectedRed;report.processExit=childExit;report.elapsedMs=Date.now()-started;report.tokens={start:null,end:null,consumed:null,source:'계측값 미제공'};report.scriptSha256=digest(self);
  if(rootOwned){
  ...
  process.exitCode=failure||rows.some(r=>r.result==='fail'&&r.id!=='S05-I27-L11')?2:expectedRed?1:0;
}
