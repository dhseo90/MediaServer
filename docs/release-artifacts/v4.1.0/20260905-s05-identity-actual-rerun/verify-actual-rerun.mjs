...
import {eventHasExactCatalogLink} from '../20260904-s05-identity-fix/verify-actual-link-reader.mjs';

const self=fileURLToPath(import.meta.url);
const repo=path.resolve(path.dirname(self),'../../../..');
const root=process.argv[2];
const reportPath=path.join(path.dirname(self),'actual-foreground-rerun.json');
...
try{
  ...
  const linked=report.events.length>0&&originalIdentityPreserved&&report.events.every(e=>e.recordingLinkId&&eventHasExactCatalogLink(e,links,{sourceId:'9101',channelId:'9101'}));
