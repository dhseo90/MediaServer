...
| UI baseline 424 | 진행 대상 | 과거 424/424는 source `8fa98a99`에 결속; 이후 `/ops/events` 녹화 선택·저장 조회 변경 | `src/ingress/product_ui_page_scripts.cpp`, `V410-S06-I27~I34` | 승인; 현재 exact 전수 적격 재판정 필요 |
...
30분 PASS는 UI 또는 녹화 전용 120분 PASS가 아니다. 1번의 실제 UI와 2번 장시간·자원
검사는 아래에 각각 결과를 추가할 때까지 미실행이다. PR·병합·태그·Release는 이번
개발·푸시 승인에 포함되지 않는다.
