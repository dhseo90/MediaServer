...
## LP31 승인: 잔여 1~5 순차 마감

2026-09-23 사용자는 최신 UI 증거 보존·정리 → 녹화 실제 UI → 브라우저 미디어 → 공통 및 녹화 전용
120분 → 최종 기록·분할 커밋·푸시를 승인했다. 아래 LP29/LP30의 당시 120분 비범위는 역사적 상태다.
이번에도 PR·병합·태그·Release, 외부 서비스·실기기는 실행하지 않는다. 제품 계약·기존 timeout·합격 기준은 유지한다.
메인이 실행·증거·최종 판정을 담당하고 기존 단일 담당자는 후속 실행의 선수조건을 읽기 검토한다. 하위 위임은 금지한다.

| 테스트 카테고리 | 판정 | 직접 근거 | 근거 파일·행·기능 ID | 실행 승인 상태 |
| --- | --- | --- | --- | --- |
| 증거 보존·문서 검증 | 진행 대상 | 사용자 1·5번 | LP31-E01~03, AGENTS 7.1/7.8 | 이번 승인; 추가 대용량 보존 범위는 확인 중 |
| 녹화 실제 UI·브라우저 미디어 | 진행 대상 | 사용자 2·3번 | LP29-U03/04, I27~34의 기존 31개 조작, FINAL-browser-media | 이번 승인 |
| 공통·녹화120분 | 진행 대상 | 사용자 4번·S11 필수 | FINAL-120-common/recording, LP26-O01~05 | 이번 승인; 선수 UI 및 실행·판정·정리 준비 충족 후 각각 실행 |
| 기존30분·기본424 UI 재실행 | 미진행 | 현재 제품/바이너리 불변 및 이미 실행된 유효 결과 | LP29 실제30분, source8fa98a99 실제UI | 자동 재실행하지 않음; 향후 실제 diff 영향으로 재판정 |
| 외부 서비스·실기기·외부 release action | 미진행 | 사용자 제외 및 이번 범위 | AGENTS 4, release-policy | 제외/별도 승인 경계 |

| 제목 | 수행내용 | 수행 상세 내용(확인 방법) | 몇버전부터 들어갔는지 |
| --- | --- | --- | --- |
| LP31-E01 | 최신424·시각80 증거 보존 | 기존 실행 원출력의 source·exit·전수 행수·Policy·비밀 검사·archive 복원 hash를 대조. 새로운 UI 실행으로 세지 않음 | v4.1.0 |
| LP31-E02 | 직전 Policy 실패 보존 | source1ab의 실패 이유2개와 424/80 관측 결과를 원본에서 보존. 현행 PASS로 소급 승격하지 않음 | v4.1.0 |
| LP31-E03 | 소유 자료 정리·문서 정합 | PID/port 및 archive 확보 확인 후 해당 소유 자료만 정리, 크기/부재 기록. docs links/assets·diff check 순차 실행 | v4.1.0 |

녹화 UI는 기존 manual-ui-result-template의 8개 ID·31개 조작을 그대로 사용한다. 준비 fixture의 성공을
실제 조작 PASS로 사용하지 않는다. 120분은 관측 완료·자원 검토·정리를 구분하며 자동 false/reviewRequired를
임의 PASS로 바꾸지 않는다. 전용 token 집계가 없어 start/end/consumed는 미집계, 이유·실제 elapsed/source를 기록한다.

### LP31 실행 전 확인·보류 상태

2026-09-23 최신 source8fa98a99 실제 UI 원출력 4,565파일을 작업 소유 디렉터리 안에서 xz 압축하고,
별도 복원본의 전체 경로·바이트 수·SHA256이 원본과 동일함을 확인했다. 압축은16,666,968바이트,
SHA256은 `7082dad712c3b8dc445b61dc7c5970d6e4d3e55a2675e1ec26885cae598cb3ef`다.
복원용 디렉터리는 검증 후 삭제·부재를 확인했다. 원본 UI 결과는424 PASS·조작1,089행·시각80개·적격 PASS이며
이번 작업은 UI 재실행이 아니다. 직전 source1ab 최소 보존물26파일도 원본 해시/바이트가 일치하고,
실패 이유는 canonical-case-manifest-implementation-controlAction-drift 및
canonical-implementation-evidence-projection-drift 두 개로 유지했다. 원본 결과를 바꾸지 않았다.

사용자 권장 방향 확정에 따라 최신 전체 증거와 직전 실패의 최소 증거를 저장소에 이관했다.
[최신 전체 압축 증거](lp31-ui-final-full.tar.xz)와 [최신 구조화 전수 결과](lp31-ui-final-result.json),
[개별 결과표](lp31-ui-final-items.md), [직전 실패 최소 증거](lp30-ui-first-minimum.json.gz)와
[직전 실패 구조화 결과](lp30-ui-first-result.json)가 각각 존재한다. 전체 압축본의 저장소 SHA256은 위 복원 검사와 일치한다.
기록된 서버 PID22217 및
실행 PID20961/22074는 직접 조회에서 부재였으며 HTTP60422/RTSP60423/UDP64511도 점유 출력이 없었다.
프로세스 조회는 sandbox 제한 후 읽기 전용 권한으로 확인했고, PID 부재에 따른 exit1을 제품 실패로 세지 않았다.

후속 준비의 읽기 검토에서는 녹화 UI 준비기가 로컬 ICE 값을 지정하지 않는 것과 임시 wrapper의
준비 실패·spawn 오류·취소·로그 상한 정리 공백을 확인했다. 아직 서버/브라우저 실행은 하지 않았다.
이는 기존 제품 계약을 바꾸지 않는 격리 준비 범위에서 보완할 대상이다. 녹화120분 자원 판정은 구현계획의
장시간 측정 경계에 허용 범위 미확정으로 남아 있고 runner도 resourceTrendPass=false/reviewRequired=true를
유지한다. 공통120분에는 RSS/FD/thread 추세 수집이 없다. 새 수치를 임의로 확정하거나 관측 성공을 자원 PASS로
승격하지 않으며, 장시간 실행 전에 판정 기준 확정 또는 관측만 수행할 경계를 사용자에게 확인했다.

### LP31-E03 소유 중복 자료 정리

삭제 전에 두 소유 root의 realpath·디렉터리·UID501·0700을 직접 확인했다. 최신 전체 증거의 저장소 사본은
위 SHA256, 직전 최소 증거의 저장소 사본은 SHA256
`68ee7ed98f506e048420750629a47d376ef3f413e3b83883fde5471cc9303ffc`와 일치했다.
구조화 결과는 최신424 PASS·실제조작1,089행·시각80·복원일치·비밀검사 PASS, 직전 실패 이유2개·최소26파일을
확인했다. 녹화 UI·브라우저 미디어 실행용으로 남은 `/private/tmp/media-server-lp30-ui.wBzhhT`는 다른 단계가
사용하므로 지금 삭제하지 않았다.

| 경로 | 종류 | 삭제 전 크기 | 조치 | 삭제/보존 결과 | 근거 |
| --- | --- | --- | --- | --- | --- |
| `.media_server.test/v4.1.0/ui-acceptance-current` | 최신 실제UI 원본 root | 344,240KiB(할당) | 저장소 전체압축본 이관 후 삭제 | 부재 확인; 전체4,565파일 복원 가능 | SHA256·복원 일치 |
| `/private/tmp/media-server-lp30-ui.wBzhhT/ui-first-retained` | 직전 Policy 실패 원본 root | 344,032KiB(할당) | 실패 최소26파일·구조화 결과 이관 후 삭제 | 부재 확인; 전체 원본 복원은 불가 | 원본 최소자료 해시 일치·보존 선택 |
| `/private/tmp/media-server-lp30-ui.wBzhhT/output/`의 UI 최신/직전 중복8파일 | 이관 뒤 복제본·압축본 | 93,273,365B(파일 합계) | 정확한8파일만 삭제 | 전체8파일 부재 확인 | 이관본 해시 일치 |
| `/private/tmp/media-server-lp30-ui.wBzhhT`의 나머지 소유 출력 | 후속 검증 준비·실행 자료 | 24,444KiB(정리 뒤 할당) | 후속 단계에서 계속 사용 | 아직 보존 중, 최종 cleanup 대상 | 녹화 UI·browser wrapper 소유 |

최신 UI 실행 자체의 서버/runtime/port cleanup PASS 외에 종료된 실행 PID20961/22074/22217 부재와
TCP60422/60423·UDP64511 비점유를 대조했다. 정리 이력은 삭제된 약0.8GB의 원본 전체를 다시
살릴 수 있다는 뜻이 아니다. 최신 전체압축본 및 직전 최소증거만 저장소에서 복원할 수 있다.

현재 1번의 최신 UI 증거 보존과 해당 중복 원본 정리는 완료했다. 녹화 UI·브라우저미디어·두120분·외부
release는 아직 미실행이다. 커밋·푸시 수행 여부는 실제 Git 기록을 따른다. 자원 판정 허용 범위는
기존 미확정 상태를 유지하며, 관측 성공만으로 안정성 PASS를 주장하지 않는다.
