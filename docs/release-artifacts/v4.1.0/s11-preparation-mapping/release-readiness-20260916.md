...
이 문서의 아래 날짜별 표는 당시 판정 이력이다. [2026-09-24 최신 전수 대조](release-readiness-20260924.md)를 현재 기준으로 사용한다.

## 2026-09-23 S11 녹화 120분 실패 후 현재 전수 대조

이 절은 2026-09-23 당시 판정이다. 아래 LP29 이하 절도 당시 실행의 역사 기록이다. 삭제 영상의 상세 메타데이터 보존·압축 계약이 당시 미결정이어서 제품 수정/장시간 재실행/후속 UI·릴리즈 검증을 보류했다. 수치 상향이나 fixture 축소를 성공 판정으로 사용하지 않는다. 이후 [짧은 구성별 진단](../s11-recording-ui-20260923/recording-root-diagnostic.md)은 자체검사 5/5·7/7 및 실제 앱 71/71을 확인했으나 120분 실패를 해결하거나 최종 전수 증거를 대체하지 않는다.

### 지시 전수

| 번호 | 사용자 지시 | 처리 상태 | 결과 | 근거 |
| --- | --- | --- | --- | --- |
| 1 | 승인된 단계 순차 실행·실패 시 같은 범위에서 수정 후 계속 | 부분 완료·계약 결정 대기 | I30 수정·30분·공통120분 PASS, 녹화120분 FAIL 뒤 후속 UI 보류 | 중앙 기록·녹화120분 원출력 |
| 2 | 분할 커밋·조건 충족 시 푸시 | 부분 완료 | 제품 `c37949e31`·`9643b6e70` 커밋, 실패 단계 기록 미커밋·푸시 미수행 | Git 직접 확인 |
| 3 | 종합·릴리즈 잔여 재산정 | 수행 | 아래 전수표의 직접 확인/규칙/제안 구분 | 이 절 |
| 4 | 외부 서비스·실기기 제외 | 유지 | 실행하지 않았고 PASS 아님 | 사용자 명시 지시 |

### 기준

| 항목 | 기준 값 | 직접 확인 결과 | 근거 |
| --- | --- | --- | --- |
| branch/HEAD | `v4.1.0` 개발 브랜치 | `9643b6e700ee8c26469b5be640bf79e3eecb1465`, local tracking 대비 ahead 8·behind 0; 원격 최신 상태는 재조회하지 않음 | Git read-only |
| 버전 | VERSION/CMake 4.1.0 | 둘 다 4.1.0 | 파일 직접 대조 |
| 실행 바이너리 | 최종 I30 제품 | SHA256 `beca63166d0db227adbac8036149947b22516127d5d213b02f231046f9c9a35e`; 공통120 동일 바이너리 | I30·공통120 기록 |
| release action | 각각 별도 승인 | PR/merge/tag/Release 미수행 | AGENTS 4장·Git 상태 |

### roadmap 대조

| roadmap 항목 | 문서상 상태 | 직접 확인 상태 | 불일치 여부 | 근거 |
| --- | --- | --- | --- | --- |
| S00~S08 | 과거 구현·검증 이력 | 이번 변경과 별도로 보존, 전체 재실행은 아님 | 없음 | 중앙 기록·변경 범위 |
| S09 | 종료·S10/S11로 대체 | 실패·수정 이력 유지 | 없음 | roadmap |
| S10 | 구현·코드 고정 이력 | 신규 I30 MP4 제품 diff가 생겨 최종 source 재결속 필요 | 이전 고정 증거 일부 재판정 | Git·I30 결과 |
| S11 | 30분·UI·120분 완료 조건 | 30분 PASS, 공통120분 재실행 PASS, 녹화120분 FAIL, 새 화면 전체 적격 미확인 | 전체 완료 아님 | 중앙 기록·전수 요약 |

### 구현 대조

| 확인 대상 | 실제 파일·route·함수·API·UI·verifier | 확인 결과 | 근거 |
| --- | --- | --- | --- |
| I30 신규 이벤트 영상 | managed DerivedJob/HTTP `/ops/api/recordings/media/*`·`/ops/events` | fMP4 관련 회귀·실제 브라우저 두 출력 재생 확인; 새 screenshot/trace 미보존 | I30 재검증 |
| 30분 | `verify-predev --soak-minutes 30` | 2,415초·20회·109 PASS·0 FAIL·외부 TURN 1 제외 | predev-30-summary |
| 공통120분 | `verify-predev --soak-minutes 120` | 첫 RTSP probe 실패 이력, 재실행 7,859초·80회·409 PASS·0 FAIL·외부 TURN 1 제외 | common-120 두 실행 기록 |
| 녹화120분 | `verify-v410-recording-longrun --duration-minutes 120` | 2,258.793초·3,123 PASS·1 FAIL, `observation-root-cap`; 재기동 단계 미도달 | recording-120-attempt1 원출력 |
| 증가 경계 | `retention_coordinator.cpp`, `recording_journal.cpp`, `recording_catalog.cpp` | 영상 quota는 media만 계산; deleted mutation/checkpoint/SQLite 이력은 계속 남음 | 코드 직접 대조·한정 독립 감사 |
| 최종 UI | 424 baseline·녹화31 action·Policy v4 | 이전 baseline 적격·I30 단기 브라우저 관측은 존재, 새 소스 전체 적격은 없음 | UI 보존 증거·I30 기록 |

### 근거 분류

| 항목 | 근거 유형 | 근거 | 릴리즈 영향 |
| --- | --- | --- | --- |
| FAIL 뒤 후속 보류·30분/UI blocker·120분 판정 | AGENTS 직접 규칙 | 3·7·8장 | 녹화120분 미해결 상태에서 S11 완료 불가 |
| 30분·공통120분·녹화120분 수치 | 프로젝트 직접 확인 | 각 구조화 요약·원출력·정리 | PASS와 FAIL 분리 |
| metadata 증가 구조 | 프로젝트 직접 확인 | quota 계산·checkpoint 압축 대상·SQLite/메모리 상태 | 상한 증액만으로 해결 불가 |
| 삭제 이력의 보존 기간·압축 표현 | 추론/제안 | ID 충돌 거부·tombstone·복구 계약의 정책 선택 필요 | 사용자 판단 전 제품 스키마 변경 금지 |

### 테스트 필요성 판정

| 테스트 카테고리 | 판정 | 직접 근거 | 근거 파일·행·기능 ID | 실행 승인 상태 |
| --- | --- | --- | --- | --- |
| 안정화 | 조건부 진행 | 저장 계약 변경 시 영향을 받는 catalog/복구/보존·빌드 | `V410-S03/S04/S10`, AGENTS 7.6.2 | 현재 변경 범위 미확정, 재실행 보류 |
| 30분 | 조건부 진행 | 현재 소스 실행 PASS, 후속 저장 변경의 무효 범위는 diff 후 판정 | predev-30-summary·AGENTS 7.6.2 | 기존 실행 승인 완료, 무조건 재실행 아님 |
| 120분 공통 | 조건부 진행 | 동일 바이너리 재실행 PASS, 후속 코드 변경 영향 판정 필요 | predev-120-pass-summary·AGENTS 7.6.2 | 기존 승인 실행 완료, 영향 밖이면 승계 가능 |
| 120분 녹화 전용 | 진행 대상 | 녹화·보존·복구 직접 변경과 기존 FAIL | recording-120-attempt1·`V410-S04/S10` | 실행 승인 있었으나 현재 실패 단계, 수정 전 재실행 금지 |
| UI 풀테스트 | 진행 대상 | 신규 MP4와 I31 UI 변경, 새 source screenshot/trace 미보존 | I30 재검증·`V410-S06-I27~34`, AGENTS 7.6.3 | 사용자 승인, 녹화120분 후 영향 범위 판정 |
| 외부/실기기 | 미진행 | 사용자 명시 제외 | 사용자 지시 | 제외, PASS 불가 |

### 개발 순서

| 순서 | 우선순위 | 잔여 이슈 | 해야 할 일 | 성격 | 근거 유형 | release action 전·후 |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | P0 | 삭제 메타데이터 수명 계약 | ID 재사용 거부·pin/hold·이벤트 참조·삭제 증명·미디어 없는 복구를 지키면서 상세 기록 압축/만료 경계 선택 | 설계 결정 | 직접 확인+제안 | 전 |
| 2 | P0 | journal·SQLite·메모리 보완 | 선택한 버전 계약을 writer/checkpoint/재투영·runtime에 적용, 기존 데이터 반례 보존 | 제품 개발 | 제안 | 전 |
| 3 | P0 | 집중·영향 회귀 | 삭제/중복 ID·손상·재기동·SQLite fallback·보존/참조·용량/RSS를 독립 검증 | 검증 | AGENTS+제안 | 전 |
| 4 | P0 | 녹화120분 재실행 | 동일 120분·정리·자원 추세를 판정; 상한/timeout만 완화하지 않음 | 장시간 검증 | 기존 FAIL | 전 |
| 5 | P0 | 변경 UI 증거 적격 | I30/I31와 영향 화면의 실제 play/pause/seek·screenshot/trace·Policy v4 결속 | UI 검증 | 기존 미확인 | 전 |
| 6 | P0 | S11 최종 정합 | 증거 유지/무효 분류, 문서·버전·빌드·인증·cleanup·커밋·푸시 가능성 판정 | 마감 | AGENTS | 전 |
| 7 | 별도 승인 | PR/merge/tag/Release | 완료 증거 후 각 외부 action 별도 사용자 승인 | 릴리즈 실행 | AGENTS 직접 규칙 | 후 |

### 미해소

| 항목 | 상태 | 사유 | 완료 evidence 사용 가능 여부 | 다음 조건 |
| --- | --- | --- | --- | --- |
| 녹화120분 | FAIL | 448MiB 격리 root 상한, metadata 증가와 fixture/영상 합산 | 불가 | 저장 계약·제품 보완 후 동일 장시간 재검증 |
| 새 화면 전체 Policy v4 | 미확인 | I30 단기 브라우저 관측의 screenshot/trace 미보존 | 불가 | 실제 UI 영향 범위와 적격 증거 |
| 공통120분 최초 시간초과 | 미재현·원인 미확정 | 동일 코드 재실행은 통과했으나 최초 원인 불명 | 최초 실패는 PASS 불가, 재실행 증거는 유효 | 최종 영향 판정에 이력 유지 |
| 외부 서비스·실기기 | 사용자 제외 | endpoint/실기기 검증 안 함 | 불가 | 이번 release 필수로 주장하지 않음 |
| 최종 커밋·푸시·release action | 미수행 | 실패 단계와 미커밋 기록·외부 승인 경계 | 불가 | 위 P0 통과 후 재판정 |
