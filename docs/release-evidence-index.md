...
| V390-REVIEW4-65 current HEAD independent acceptance | 사용자 canonical `./test_release.sh`, 네 root launcher, tracked long VA seed fixture, release canonical `test-acceptance-current-final`의 `summary.json`/`report.md`, per-stage logs/summaries, exact-424 screenshots/traces, Policy v4 qualification, duration/cleanup measurement | [release-test-records.md](./release-test-records.md) `V390-REVIEW4-65 구현 파일·함수·개별 verifier 결과 맵`, `REVIEW4-65 clean-checkout VA media fixture`, `REVIEW4-65 UI manifest drift actual failure/correction`, `REVIEW4-65 UI artifact/lifecycle invariant correction`, `REVIEW4-62/65 canonical evidence lifecycle` | Current actual은 feature gate 35/35와 real 30분 PASS 뒤 UI pre-execution 0/424 FAIL, 이후 standalone UI는 artifact-root infrastructure에서 FAIL했으며 Policy v4·120분·final integrity not-run. Correction contract는 actual PASS를 대체하지 않음 | 30분·120분 launcher는 OS temp, `./test_ui.sh`는 저장소 내부 transient `.media_server.test/v3.9.0/ui-acceptance-current`, `./test_release.sh`는 canonical output을 사용합니다. Historical actual 34/34와 35/35 기록은 보존하며 current flow는 build→36 gate→30분→UI→AGENTS 7.6.2 조건부 120분→cleanup→report→final integrity입니다. UI preflight는 저장소 밖 artifact root를 environment bootstrap 전에 거부합니다. Exact runner는 raw `CAPTURED`를 최종 `uiFulltestPass`와 분리하고 bootstrap/producer/case 실패에도 exact ledger를 남기며 부분 artifact를 prune/dedupe합니다. Correction candidate `ec38b1be...`는 983 carry-forward+3 independent review로 986/986 approval을 구성했고 feature evidence 986/986·project inventory 17/17을 통과했습니다. Contract/fixture는 actual evidence를 대체하지 않습니다 |
