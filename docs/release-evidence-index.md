...
| v3.9.0 Step 25 / V390-ADD1-11 current UI automation coverage matrix | `./server.sh verify-v390-ui-automation-coverage --output-dir <path>`, `./server.sh verify-v390-ui-automation-coverage-contract`, [v390-ui-automation-coverage-matrix.md](./v390-ui-automation-coverage-matrix.md), [project-feature-test-inventory.md](./project-feature-test-inventory.md) exact `UI-001`~`UI-115`, `OPS-169`/`SAFE-202`, `test/fixtures/v390_ui_automation_coverage_policy.json`, actual native visible-DOM summary | [release-test-records.md](./release-test-records.md) `V390-ADD1-11 UI automation coverage matrix final` | [release-test-records.md](./release-test-records.md) `V390-ADD1-11 UI 풀테스트`, matrix의 `unsupported-manual` 106개 | full automation coverage, UI 풀테스트 직접 조작, unsupported case PASS, 30분/120분, published metadata, release action evidence. Matrix gate는 `mapped-with-explicit-gaps`/partial automation evidence만 확인합니다 |
