...
| v3.9.0 current source/readiness reconciliation | current inventory 986개, current acceptance source, Step 20 Development 15~18 및 Review2-21/35 companion verifier, current review approved/closed 상태 | [release-test-records.md](./release-test-records.md) v3.9.0 `v390 current user approval closure reconciliation`, `V390-REVIEW4-61 current acceptance policy supersession`; canonical `./server.sh verify-v390-test-acceptance-bundle --output-dir docs/release-artifacts/v3.9.0/test-acceptance-current-final --ui-http-base <loopback-url> --ui-role-state-map <roles.json> --ui-server-log <server.log> --ui-server-pid <pid> --ui-rtsp-port <port> --ui-temporary-root <media_server_v390_ui-*>` | historical source `8fe583d8`의 974개/26-command evidence와 current final 안정화·30분·120분·UI 미실행 기록 | Historical 결과를 current PASS로 재사용하지 않습니다. clean worktree current source에서 안정화→feature gate→30분→exact 424 Policy v4 UI→AGENTS 7.6.2 판정상 필요한 120분→raw cleanup/final integrity evidence가 필요합니다. Trigger 없는 `--run-120`은 거부합니다 |
