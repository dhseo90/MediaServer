...
| v3.9.0 current source/readiness reconciliation | current inventory 986개, current acceptance 32-command, Step 20 Development 15~18 및 Review2-21/35 companion verifier, current review approved/closed 상태 | [release-test-records.md](./release-test-records.md) v3.9.0 `v390 current user approval closure reconciliation`, `v390 current source/readiness reconciliation final`; canonical `./server.sh verify-v390-test-acceptance-bundle --output-dir docs/release-artifacts/v3.9.0/test-acceptance-current-final --run-120` | historical source `8fe583d8`의 974개/26-command evidence와 current final 안정화·30분·120분·UI 미실행 기록 | historical 결과를 current PASS로 재사용하지 않습니다. clean worktree의 current canonical source에서 안정화→32개 feature gate→30분→UI automation/replay→120분→Policy v4 전체 UI→cleanup/evidence 순서로 새 evidence가 필요합니다 |
