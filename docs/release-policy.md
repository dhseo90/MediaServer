...
- UI visual release baseline artifact는 승인된 release/RC 화면 상태를 다음 candidate와 비교하는 approved comparator입니다. public release asset으로 기본 업로드하지 않으며, release note에는 accepted baseline run, baseline diff, 수동 비노출 검토 결과 링크만 남깁니다.
