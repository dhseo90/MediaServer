...
| v300 release published metadata | `./server.sh verify-release-metadata --published` | GitHub Release publish 이후 외부 상태 확인용입니다. 아직 PR/main/tag/GitHub Release 단계가 진행되지 않아 실행하지 않았고 완료 evidence로 사용하지 않음 |
| v300 release PR/main/tag/GitHub Release | PR 생성/갱신, main merge, annotated tag 생성/push, GitHub Release 생성/갱신, published metadata 재검증 | AGENTS.md상 각 외부 릴리즈 작업은 별도 명시 승인이 필요합니다. 이번 문서/검증 run은 local release candidate 검증이며 외부 상태 변경 완료 evidence가 아님 |
