...
| v290 S04 30분/120분 longrun | `verify-predev --soak-minutes 30`, `verify-predev --soak-minutes 120`, `verify-va-runtime-console-longrun --duration-minutes 120` | 장시간 테스트 실행 승인 없음. S04 records gate PASS로 대체하지 않음 |
...
| v290 S05 UI 풀테스트 | 인앱 브라우저 route/control/action 직접 조작 | S05는 UI 풀테스트 기준 freeze입니다. 실제 UI 직접 조작 실행 승인/증거 없음. S05 verifier PASS로 대체하지 않음 |
| v290 S05 30분/120분 longrun | `verify-predev --soak-minutes 30`, `verify-predev --soak-minutes 120`, `verify-va-runtime-console-longrun --duration-minutes 120` | 장시간 테스트 실행 승인 없음. S05 criteria gate PASS로 대체하지 않음 |
...
| v290 S09 UI 풀테스트 | 인앱 브라우저 route/control/action 직접 조작, viewport/theme 전수 확인 | S09는 owner release readiness local gate입니다. 실제 UI 직접 조작 실행 승인/증거 없음. S09 local readiness PASS로 대체하지 않음 |
| v290 S09 30분/120분 longrun | `verify-predev --soak-minutes 30`, `verify-predev --soak-minutes 120`, `verify-va-runtime-console-longrun --duration-minutes 120` | 장시간 테스트 실행 승인 없음. S09 local readiness PASS로 대체하지 않음 |
| v290 S09 published metadata | `./server.sh verify-release-metadata --published` | GitHub Release publish 이후 외부 상태 확인용입니다. S09 local readiness PASS로 대체하지 않음 |
| v290 S09 PR/main/tag/GitHub Release | PR 생성/merge, main sync, release tag 생성, GitHub Release 생성/갱신 | 사용자가 이번 단계에서 명시 승인한 범위는 커밋/푸시까지입니다. release action은 별도 승인과 evidence가 필요하며 S09 local readiness PASS로 대체하지 않음 |
