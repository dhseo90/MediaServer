...
| v4.0.0 (8) | `./server.sh verify-v400-release-readiness` | 네 테스트 영역 판정과 close-out dry-run을 기록합니다. 현재 30분과 UI는 `executed-pass`, 120분은 `conditional-not-run`. 출시 가능 판정, tag/GitHub Release, UI 풀테스트, 30분/120분, published metadata, release action evidence가 아닙니다 |
