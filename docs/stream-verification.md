...
- 로컬 샘플 파일, 로컬 RTSP test source, WebRTC publish, 로컬 HTTP URI를 기본 기준으로 반복 검증한다.
- LAN IP 외부 클라이언트 접근성, 제3자 RTSP upstream advisory, WebRTC ICE, 외부 HTTP/HLS URI는 `--external` 또는 선택 검증에서 확인한다.
...
최신 후속 검증 결과:
- `2026-04-29`: 12번 2시간 이상 soak를 제외한 UI/검증 항목을 재검증했다. `./server.sh verify-analysis-state` 통과 `4/0`으로 TrackStateManager Active/Lost/Terminated, observation ring buffer, trajectory cap, channel 분리, TrackHealth, Appearance NoOp, SceneContextBuilder zone/line/dwell/vaRule scoping, EventManager lifecycle/cooldown/cleanup, IntrusionDwellScenario Candidate/Observing/Confirmed/Ended/재진입, Scenario/Event cleanup을 mock metadata로 확인했다. `./server.sh build` 통과, `./server.sh verify-rule-ui --http-base http://127.0.0.1:8081` 통과 `5/0/0`, `./server.sh verify-lab-layout --http-base http://127.0.0.1:8081` 통과 `10/0`, `MEDIA_SERVER_LISTEN_PORT=8555 MEDIA_SERVER_HTTP_LISTEN_PORT=8081 ./server.sh test --no-start --include-rules --include-rule-ui --include-va-events --include-image-analysis --include-redaction` 통과 `19/0/8`, 상세 로그 `.media_server.test/20260429-075051`. event POST 활성 서버에서 `./server.sh verify-event-post --mode schema` 통과 `7/0/0`, summary `/tmp/media_server_evtpost-1777417273-95760_summary.json`, `./server.sh verify-event-post --mode recovery` 통과 `11/0/0`, summary `/tmp/media_server_evtpost-1777417285-95929_summary.json`. 다채널은 live streaming 1/2/4/8 client 단계가 각각 `3/0`으로 통과했고, VA overlay는 4 client `5/0`, 8 client `5/0`으로 통과했다. summary는 `/tmp/media_server_multichannel-1777417321-96207_summary.json`, `/tmp/media_server_multichannel-1777417345-96616_summary.json`, `/tmp/media_server_multichannel-1777417375-97068_summary.json`, `/tmp/media_server_multichannel-1777417399-97582_summary.json`, `/tmp/media_server_multichannel-1777417429-98267_summary.json`, `/tmp/media_server_multichannel-1777417472-99212_summary.json`. 검증 후 runtime은 `activeSessions=0`, `resourceActiveStreams=0`, `activeAnalysisTaps=0`으로 cleanup 상태를 확인했다.
- `2026-04-28`: `/lab/rules`를 영상 분석 관리 화면으로 개편했다. 최상단 `영상 분석 설정`/`영상 분석 보기` 탭을 두고, 설정 탭은 숫자 기반 `vaRule` ID와 source/profile/event/scenario/geometry binding을 저장한다. 보기 탭은 Live Streaming, `va=1` overlay, `vaRule=<id>` 모드를 분리하고, `vaRule` 모드의 URL은 `file/url/source` override 없이 `?vaRule=<id>`만 생성한다. `./server.sh build` 통과, `cmake -S . -B build-release-gst-onnx -DCMAKE_BUILD_TYPE=Release -DMEDIA_SERVER_USE_GSTREAMER=ON -DMEDIA_SERVER_USE_ONNXRUNTIME=ON -DMEDIA_SERVER_ONNXRUNTIME_ROOT=/opt/homebrew/opt/onnxruntime` 후 `cmake --build build-release-gst-onnx` 통과, `./server.sh verify-rule-ui --http-base http://127.0.0.1:8081` 통과 `5/0/0`, `./server.sh verify-lab-layout --http-base http://127.0.0.1:8081` 통과 `10/0`. `MEDIA_SERVER_LISTEN_PORT=8555 MEDIA_SERVER_HTTP_LISTEN_PORT=8081 ./server.sh test --no-start --include-rules --include-rule-ui --include-va-events --include-image-analysis --include-redaction` 통과 `19/0/8`, `./server.sh verify-multichannel --http-base http://127.0.0.1:8081 --include-va --repeat 1 --single-clients 2 --clients-per-source 2 --hold-ms 5000` 통과 `9/0/0`, `./server.sh verify-predev --skip-build --soak-minutes 30 --heartbeat-interval 30` 통과 `68/0/2`, duration `2483s`, summary `/tmp/media_server_predev-1777389156-17375_summary.json`, report `/tmp/media_server_predev-1777389156-17375_report.md`, report html `/tmp/media_server_predev-1777389156-17375_report.html`. 최종 cleanup 후 `8080/8081/8554/8555` listener는 없었다.
- `2026-04-28`: Lab resize 안정화를 위해 stream/정적 이미지 분석을 bounded 2열 기준으로 정리하고 `./server.sh verify-lab-layout`을 추가했다. 390/768/1180/1365/1600 폭에서 stream과 image-analysis 모두 가로 overflow 없이 통과 `10/0`, summary `/var/folders/k0/qhmr6zdx11q0_41wfx4dsd200000gn/T/media_server_lab-layout-1777370197934-39090_summary.json`. 정적 이미지 분석과 VA 고급 튜닝의 숫자 입력은 min/max/default가 보이는 range slider로 바꿨다. `verify-redaction`은 runtime idle precheck를 즉시 실패가 아니라 최대 35초 대기 후 판정하도록 보강했다. `./server.sh verify-predev --skip-build --soak-minutes 30 --heartbeat-interval 30` 통과 `68/0/2`, duration `2512s`, summary `/tmp/media_server_predev-1777373074-81416_summary.json`, report `/tmp/media_server_predev-1777373074-81416_report.md`, report html `/tmp/media_server_predev-1777373074-81416_report.html`. 최종 cleanup 후 `8080/8081/8554/8555` listener는 없었다.
...
- `2026-04-27`: 후속 개발로 `/lab`과 `/webrtc/test`에 `/lab/runtime/status` 기반 런타임 상태 패널과 다채널 WebRTC 수동 테스트 패널을 추가했다. 같은 화면에서 active session/stream/tap, WHIP publish source의 video/audio readiness, subscriber count를 확인할 수 있다. 브라우저 확인에서 `/lab` title, runtime panel, 다채널 버튼 DOM, runtime text `session 0 · stream 0 · tap 0`을 확인했다.
- `2026-04-27`: `./server.sh summarize-reports`를 추가해 `/tmp/media_server_*summary*.json` 검증 결과를 Markdown 표로 모을 수 있게 했다. `verify-uri-longrun`은 실패 시 DNS, HTTP status, playlist parse, pad-not-linked, timeout, connection 계열 후보를 `failureClassifications`로 남긴다. 샘플 출력은 `/tmp/media_server_verification_report.md`로 생성 확인했다.
- `2026-04-27`: `./server.sh verify-event-post --mode recovery`를 추가했다. event POST worker가 초반 실패 후 복구되는 endpoint에서 `failedCount`와 `sentCount`를 모두 갱신하는지 확인한다. `MEDIA_SERVER_ANALYSIS_EVENT_POST_ENABLED=1 ./server.sh verify-event-post --mode recovery` 통과 `11/0/0`, `sentCount=3`, `failedCount=3`, `/flaky` 수신 `8`건을 확인했다.
- `2026-04-27`: 새 UI/검증 보강 후 `MEDIA_SERVER_LISTEN_PORT=8555 MEDIA_SERVER_HTTP_LISTEN_PORT=8081 ./server.sh test --no-start --include-rules --include-rule-ui --include-va-events --include-image-analysis` 통과 `19/0/4`. 이어 `./server.sh verify-multichannel --include-va --repeat 1 --single-clients 2 --clients-per-source 2 --hold-ms 5000` 통과 `8/0`, summary `/tmp/media_server_multichannel-1777245265-56877_summary.json`.
...
- `/lab/rules`는 영상 분석 관리 호환 route다. 화면 내부에서 `영상 분석 설정`과 `영상 분석 보기` 탭을 제공하며, 보기 탭은 원본 live, `va=1` overlay, `vaRule=<id>` 저장 설정 모드의 URL을 구분해서 표시한다.
...
1. 외부 RTSP source 심화 재검증
   - `./server.sh test` 기본 기준은 로컬 재현성을 우선하므로 LAN IP 외부 클라이언트 접근성을 포함하지 않는다.
   - LAN IP 외부 클라이언트 접근성은 `./server.sh test --external` 또는 `./server.sh test --stable`에서 hard gate로 확인한다.
   - 제3자 RTSP upstream 후보는 remote 응답 지연과 upstream 상태 영향이 커서 `--stable`/`--external`에서는 advisory로 본다.
...
- 이번 체크에서 우선 보는 범위
  - `./server.sh test` 무옵션은 `--basic`과 같으며, 로컬 재현성을 우선해 LAN IP/외부망 probe를 제외한다.
  - LAN IP 기준 외부 클라이언트 접근성은 `--external` 또는 `--stable` 모드에서 확인한다.
  - 제3자 RTSP upstream reachability는 `--external` 또는 `--stable` 모드에서 advisory로 확인한다.
...
- 권장 실행 순서
  1. `./server.sh install`
  2. `./server.sh test`
  3. 로컬 풀 범위가 필요하면 `./server.sh test --full`
  4. LAN/외부망 범위가 필요하면 `./server.sh test --external`
  ...
  8. Lab 반응형 UI를 건드렸다면 `./server.sh verify-lab-layout`
  9. 필요 시 `./server.sh verify-uri-longrun`
  10. 필요 시 `./server.sh verify-route-profiles`
  11. 필요 시 `./server.sh verify-tracker-stability`
  12. 필요 시 `./server.sh verify-yolo-layouts`
  13. 필요 시 `./server.sh verify-adaptive`
  14. `./server.sh stop`
- 판단 규칙
