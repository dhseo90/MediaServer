...
| v4.0.0 (8) | `./server.sh verify-v400-release-readiness` | concrete release-note candidate와 네 테스트 영역 판정을 확인합니다. 이전 30분/UI PASS는 `historical-pass-fresh-rerun-pending`, fresh 30분/UI는 미실행·필수, 120분은 `conditional-not-run`입니다. 출시 가능 판정, tag/GitHub Release, actual UI/장시간, published metadata, release action evidence가 아닙니다 |
