...
| Step | Command | Scope |
| --- | --- | --- |
| v3.4.0 (1) | `./server.sh verify-v340-entry-baseline`, `./server.sh verify-release-metadata`, `./server.sh verify-docs-links`, `./server.sh verify-docs-ui-assets` | source `3.4.0`, latest published `v3.3.0`, current roadmap `v3.4.0 Operations Continuity Drill Workspace` 정렬. v3.4 기능 구현, UI 풀테스트, 30분/120분, published metadata, tag, push, GitHub Release evidence가 아님 |
