...
사용자 승인: 재판정한 1~3번 개발 및 후속 재검토·마무리·커밋·푸시 요청.
이번 산출물은 기준·비교 설계·안전 계약이며 제품 변경(4번), 비교 본실행, 실제 앱, 장시간/UI로 확대하지 않는다.
Git은 이번 문서와 기존 누적 커밋/미해결 변경을 구분해 AGENTS.md 5장으로 판정하고 실제 결과를 중앙 기록에 남긴다.
AGENTS.md 1.3/3/7/8을 따른다.
미확정 저장 안전 설계는 메인이 직접 맡고 추가 작업 에이전트는 만들지 않는다.
...
이 추가 축도 이번에는 설계 등록만 하며 runner 구현/실행 시 개별 전이와 명령·상한을 연결한다.
...
- 실행 안전 RSS 상한은 제품 SLO나 기존 512MiB 결과와 별개다. 호스트 여유·동시 사용·프로세스 트리·중단/정리 방법과
  함께 **본실행 전에** 선정하고 승인 범위를 확인한다. 아직 미선정이므로 비교 본실행 준비 완료로 보고하지 않는다.
  도구 자체검증은 작은 격리 입력으로 준비할 수 있으나 이 설계가 대용량 실행 권한을 만들지는 않는다.

각 군 입력 manifest/hash·제품/계측/컴파일 hash·환경·한정 명령·예상 결과·실행 상세를 등록한 뒤 실행한다.
현재 구체 명령/runner는 아직 구현되지 않았다. 존재하지 않는 명령을 실행 가능하다고 보고하지 않는다.
...
이번에는 `git diff --check`, `MEDIA_SERVER_SKIP_LOCAL_ENV=1 ./server.sh verify-docs-links`,
`MEDIA_SERVER_SKIP_LOCAL_ENV=1 ./server.sh verify-docs-ui-assets`만 실행한다. 제품 빌드/검사 결과를 새로 만들지 않는다.
