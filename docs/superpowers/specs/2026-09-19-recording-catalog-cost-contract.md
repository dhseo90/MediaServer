...
5. 관측기는 검증된 불변 파일과 새 append를 구분한다. 재사용은 같은 root/store, 파일 identity,
   ...
   구현은 실행 수명에 묶인 단일 native 세션과 bounded strict-parser 캐시를 사용한다.
   매 호출 파일 원문/SHA와 chain의 ID·순서·충돌 조건은 다시 확인한다. 같은 snapshot/manifest의
   해석값과 동일 hash의 불변 shard만 재사용하며, 새/비적합 값은 전체 parser로 복귀한다.
   세션은 root inode와 파일 inode에 결박하고 요청별 timeout/출력 제한 및 종료 대기를 갖는다.
   캐시 논리 예산은 snapshot8MiB·identity16MiB·inode 기록8MiB이며 RSS 상한이 아니다.
   inode 기록을 회수하면 parser 재사용도 비우고 전체 읽기/검증으로 복귀한다. 이력 전체의 의미
   검사·최소 prefix 대조 비용은 남으므로 단기 실측 전 장시간 비용 해결을 주장하지 않는다.
