...
이 절은 제품 구현 완료가 아니며, 아래 대안의 영속 형식·손상 검출 시점 변경은 아직 선택하지 않았다.
...
`TryAutomaticCheckpointNoop`도 전체 물리 행을 읽어 raw SHA를 대조한다. 변경 후보는
`ReadCheckpointRecords` → `CompactRecords` → 의미 재적용/투영 대조 → `CommitCheckpoint`로 처리하고,
원자 교체 시 현재 원장 전체를 다시 쓴다. 따라서 다음 두 주장은 구분해야 한다.

1. **호환 처리 개선 가능:** 한 행씩 원문을 확인하고 필요한 행만 파싱·변환하면 일시 상세 실체화와
   중복 CPU 비용을 줄일 여지가 있다. 복구의 strict scratch 결과를 재사용하는 것도 별도 검토 대상이다.
2. **전체 누적 비용 제거는 아님:** 전체 읽기·전체 파일 교체를 유지하면서 과거 이력 길이와 무관한
   비용을 보장할 수는 없다. cache 입장 상한 변경, no-op 주기 확대, 검사 생략으로 해결됐다고 하지 않는다.

이는 코드 경계에서 도출한 하한이며 모든 장비에서 HTTP 실패가 반드시 난다는 측정 결론은 아니다.
...
| --- | --- | --- |
| A: 현재 형식 안의 스트리밍 처리 | JSONL 바이트/복구/전체 원문 검증 계약 유지. 검증된 scratch를 수용하고 물리 행을 제한된 수명으로 처리 | 호환 비용 개선이지 누적 읽기·재작성 비용 제거가 아님. 동일/변경 후보와 모든 strict fallback을 포함해 판정 |
...
권장 후보는 B지만 아직 구현 승격하지 않는다. 별도 증거 저장소는 새 Git 저장소를 뜻하지 않는다.
