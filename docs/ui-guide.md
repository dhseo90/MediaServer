...
현재 v2.2.0까지의 UI 변경과 `v2.2.0 Responsive UI Foundation`의 UI 정리는 아래
inventory를 기준으로 합니다.
...
| 현재 대표 이미지 | 2026-05-23 캡처 자산을 최신 공개 release v2.2.0의 대표 shell 설명 이미지로 유지. Client Live는 source tree, dock event feed, workspace preset, tile action/VA overlay 구조를 포함 |
