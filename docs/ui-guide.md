...
현재 소스 버전은 `2.7.0`입니다. 최신 공개 GitHub Release는 `v2.7.0` source-only
release이며, 이 문서는 현재 source tree의 UI 구조와 v2.7.0 roadmap 경계를 설명합니다.
...
현재 v2.7.0까지의 UI 변경과 `v2.2.0 Responsive UI Foundation`,
...
Incident Command Loop 전용 triage/scorecard/action/what-if UI는 실행 단계에서
