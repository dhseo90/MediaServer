...
현재 소스 버전은 `3.9.1`입니다. 최신 공개 GitHub Release는 `v3.9.0` source-only
release이며, 최신 published baseline은 `v3.9.0 Feature Completion, Structure Stabilization, and Test Model Preparation`입니다.
이 문서는 현재 source tree의 UI 구조와 v3.9.1 Release Correctness and Public Repository Hygiene
patch roadmap, v3.9.0 published baseline 경계를 설명합니다.
