...
통과해도 인앱 브라우저 UI 풀테스트, 30분 soak, 120분 longrun, field smoke가 PASS가
됐다는 뜻이 아닙니다. 실행하지 않은 항목은 결과 문서에서 `미실행`으로 남깁니다.
...
| manual UI checklist | [manual-ui-checklist.md](./manual-ui-checklist.md)가 F02~F06 route/control/action과 긴 테스트 preflight를 새 로드맵 기준으로 확인하게 합니다. |
...
| evidence verifier | `verify-v220-ui-evidence-closeout`이 이 연결을 정적 검증하고, `verify-manual-ui-evidence`가 기존 manual UI result 구조를 계속 검증합니다. |

## F02~F06 Close-out Preflight
...
- 실기기/외부 endpoint/credential이 필요한 ONVIF/WHEP/WHIP/VLM provider field smoke는 사용자가 제외하거나 실행 지시를 준 상태를 별도 기록합니다.
...
- 제외 기록: 사용자가 명시 제외한 실기기/외부 credential/scope 밖 항목
...
위 명령은 close-out 준비 기준을 확인합니다. 인앱 브라우저 UI 풀테스트와 30분/120분
장시간 테스트는 별도 실행 결과가 있을 때만 PASS로 기록합니다.
