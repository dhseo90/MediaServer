...
| V390-CAND-003 | `docs/vlm-rule-suggestion-candidates.md`, `/ops/api/vlm/rule-suggestion-drafts` route | VLM rule suggestion workflow is draft-only/manual-save; no automatic rule registry write occurs | Decide whether to complete an Ops-only review-to-draft bridge and evidence trail while preserving manual save boundaries | If approved, UI/API create a reviewable draft with provenance, explicit manual-save wording, no auto-apply, and verifier proves rule registry is unchanged until operator save | conditional | not-run | not-run | conditional | candidate-development | rule write/manual-approval boundary | Direct evidence: `docs/vlm-rule-suggestion-candidates.md:78`, `:151`, `src/ingress/webrtc_http_server.cpp:29693`. |
