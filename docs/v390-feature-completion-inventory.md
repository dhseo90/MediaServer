...
| V390-CAND-010 | Re-ID appearance assist code/UI | Re-ID assist can be selected but default/fallback behavior remains no-op unless model/config/provenance is provided | Closed with `explicit-opt-in-provenance-gated-assist`: Re-ID assist remains explicit opt-in association assist, model-backed readiness requires config/checksum/provenance, and no-op fallback is visible when gates are incomplete | `verify-v390-conditional-field-ai-decisions` proves `/ops/api/analysis/reid-assist-decision`, `/ops` Re-ID Assist Decision UI, AppConfig-derived readiness booleans, tracker-none forces off boundary, no model-backed execution, no embedding/crop/model path serialization, and docs/inventory wiring are present | required | not-run | not-run | conditional | closed-with-evidence | model/runtime bundle and privacy boundary | Closed by v3.9.0 (18). Evidence: `OpsV390ReidAssistDecisionJson`, `/ops/api/analysis/reid-assist-decision`, `renderV390ReidAssistDecision`, `UI-115`/`LAB-125`/`SAFE-210`/`OPS-177`. Model-backed execution, identity search, UI fulltest, and longrun were not run. |
