...
| V390-CAND-004 | `docs/vlm-evaluation-result-workflow.md`, `docs/vlm-profile-storage.md`, `/ops/vlm` | evaluation candidates can populate profile draft fields; saving/activation/runtime/provider calls remain separate | Decide whether to add an explicit passed-evaluation promotion/save/activation guard flow | If approved, passed candidates can be intentionally promoted through operator save/activation checks, invalid states are rejected, and runtime/provider calls remain default-off until separately approved | conditional | not-run | conditional | conditional | candidate-development | VLM default-off/provider-call boundary | Direct evidence: `docs/vlm-evaluation-result-workflow.md:20-22`, `:47`, `docs/vlm-profile-storage.md:65-75`. |
| V390-CAND-005 | `docs/ops-backup-recovery.md`, source reliability handoff route/UI | source reliability handoff is an operator input, not production restore/cutover or automatic recovery evidence | Decide whether to add a staging restore validation checklist/result artifact in the Ops handoff flow | If approved, handoff captures registry, PublishedView, source health, and viewer-scope validation results while still excluding production cutover and automatic recovery claims | conditional | not-run | conditional | conditional | candidate-development | backup/restore evidence boundary | Direct evidence: `docs/ops-backup-recovery.md:121-130`, `:137-155`, `:157-163`. |
