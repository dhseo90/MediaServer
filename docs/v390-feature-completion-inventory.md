...
| V390-CAND-002 | `src/ingress/onvif_live_import.cpp`, `src/ingress/product_ui_ops_sources_script.cpp`, `src/ingress/webrtc_http_server.cpp` | ONVIF live import validates/form-applies a draft and reports `notSaved:true`; actual source/view persistence is a separate operator flow | Decide whether approved ONVIF probe/import should persist source/view in one operator-gated flow | If approved, import-to-source/view persistence is scope-guarded, auditable, rollback-aware, and covered by route/UI verifier plus source registry evidence | conditional | conditional | conditional | conditional | candidate-development | source registry write semantics | Direct evidence: `src/ingress/onvif_live_import.cpp:1474`, `src/ingress/product_ui_ops_sources_script.cpp:1067`, `src/ingress/webrtc_http_server.cpp:39596`. |
