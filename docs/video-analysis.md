...
- `/ops/events`의 evidence column은 snapshot 파일과 clip manifest가 있는 경우 `/lab/analysis/events/evidence?download=1` 개별 다운로드와 `/lab/analysis/events/evidence/bundle` zip bundle 다운로드를 제공합니다. Bundle에는 `manifest.json`, 선택 snapshot, clip manifest, clip frame 파일이 포함되며 export는 Ops audit trail에 `export-bundle`로 남깁니다. bundle 링크는 `expiresAtMs` 기반 24시간 만료 정책을 사용하고, 만료된 링크는 서버가 거절합니다. evidence 원본 파일 삭제는 모든 role에서 차단되며, 삭제 가능한 대상은 비파괴 compaction snapshot 파일로 제한합니다. 이 bundle은 짧은 이벤트 증거 묶음이며 장기 영상 archive가 아닙니다.
