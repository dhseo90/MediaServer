...
const ownedHeader=path.join(root,'include/recording/recording_catalog.h');
if(fs.existsSync(ownedHeader)){
 const existing=fs.readFileSync(ownedHeader,'utf8');
 if(existing!==exact(header,'private:','public: // LP17 owned-copy only'))throw Error('owned-header-profile');
}else fs.writeFileSync(ownedHeader,exact(header,'private:','public: // LP26-O10 owned test copy'));
