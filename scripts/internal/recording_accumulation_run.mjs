...
function run(args,timeoutMs){return new Promise((resolve,reject)=>{
  ...
  child.on('close',(exit,signal)=>{clearTimeout(timer);clearTimeout(stageTimer);clearTimeout(force);clearInterval(memory);
    ...
    const elapsedMs=performance.now()-begin,processDiagnostic=accumulationProcessDiagnostic({mode:args[0],selectedCase:args.find(value=>['16','1020','2048','2049'].includes(value))??selected??null,status:exit,signal,stop:stopped,elapsedMs,groupClosed});processes.push(processDiagnostic);
