...
import {parsePhases,summarizeSpawnDiagnostic,validatedPhaseReceipt,diagnosticRootIdentity,sourceManifest,accumulationParentFailure,accumulationProcessDiagnostic,preserveDiagnosticReceipt,preserveAccumulationReceipt,receiptTree,ownedRoot,snapshotTree,copyVerified,cleanupAllowed,cleanupProfile,exact,instrumentCatalog,instrumentProbe,safeOutput,targetHash,selectReference} from './recording_archive_diagnostic_profile.mjs';
...
test('O28-R04 accumulation receipt는 최초 실패·미종료 group·고정 진단을 결속',()=>owned(root=>{const {receipt}=receiptFixture(root),manifest=receipt.manifest.slice(0,5),process=accumulationProcessDiagnostic({mode:'--catalog',selectedCase:'16',status:0,stop:'time-cap',elapsedMs:3.6,groupClosed:false}),value={schema:'media-server.recording-accumulation-receipt.v1',profile:'accumulation-probe',outcome:'fail',selectedCase:'16',parentFailureCode:'time-cap',groupClosed:false,manifestUnchanged:true,processes:[process],firstFailureIndex:0,command:{name:'recording-accumulation-run',args:['16']},storeManifest:receipt.sourceTree,manifest,rawPathsPublished:false},file=path.join(root,'accumulation.json');assert.deepEqual(process,{mode:'--catalog',selectedCase:'16',status:0,signal:null,stop:'time-cap',elapsedMs:4,groupClosed:false});assert(preserveAccumulationReceipt({evidencePath:file,receipt:value}).preserved);for(const changed of [{...value,selectedCase:'2048'},{...value,groupClosed:true},{...value,firstFailureIndex:null},{...value,processes:[{...process,detail:'secret'}]}])assert.throws(()=>preserveAccumulationReceipt({evidencePath:path.join(root,crypto.randomUUID()+'.json'),receipt:changed}));}));
