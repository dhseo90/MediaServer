...
node "$lp_script/recording_catalog_comparison_instrument.cjs" "$lp_repo" "$lp_root"
node - "$lp_repo" "$lp_root" <<'NODE'
'use strict';
const fs=require('fs'),path=require('path'),cp=require('child_process'),crypto=require('crypto');
const [repo,out]=process.argv.slice(2),build=path.join(repo,'build-gst-onnx');
...
for(const [file,target] of [['recording_catalog_comparison_probe.cpp','comparison'],['recording_catalog_comparison_job_probe.cpp','comparison-job']]){
