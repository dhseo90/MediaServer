...
for(const name of ['recording_catalog.cpp','recording_journal.cpp']){
 ...
 if(name==='recording_catalog.cpp'){
  ...
  s+='\nnamespace lp17 {\nOwned Journal(const recording::RecordingJournal& j){Owned n;if(j.managed_state_){Mutations(n,j.managed_state_->records);for(const auto& v:j.managed_state_->identities){String(n,v.first);String(n,v.second);}}return n;}\nvoid Provenance(const recording::RecordingJournal& j){if(!j.managed_state_)return;std::size_t i=0;for(const auto& m:j.managed_state_->records)std::cout<<"[lp17] {\\"kind\\":\\"mutation\\",\\"index\\":"<<i++<<",\\"mutationId\\":\\""<<m.mutation_id<<"\\",\\"occurredAtMs\\":"<<m.occurred_at_ms<<",\\"type\\":"<<static_cast<int>(m.mutation_type)<<"}\\n";}\n}\n';
 }
