...
for(const file of ['recording_catalog.cpp','recording_journal.cpp','recording_contracts.cpp','recording_checkpoint_validation.h']){
 ...
 if(file==='recording_catalog.cpp'){
  ...
  s=replace(s,'for(const auto& m:candidate)if(!m||!after->ApplyMutationLocked(*m,false,error,nullptr,m,&source_bindings_))return false;','if(!fc::Measure("checkpoint.candidateSemantic",[&]{for(const auto& m:candidate)if(!m||!after->ApplyMutationLocked(*m,false,error,nullptr,m,&source_bindings_))return false;return true;}))return false;');
