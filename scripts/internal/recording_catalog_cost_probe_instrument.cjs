...
for(const file of ['recording_catalog.cpp','recording_journal.cpp','recording_contracts.cpp','recording_checkpoint_validation.h']){
 ...
 if(file==='recording_catalog.cpp'){
  ...
  s=replace(s,'if(!journal_.ReadCheckpointRecords(this,&original,error,&read_snapshot,&views))return false;','if(!fc::Measure("checkpoint.ReadCheckpointRecords",[&]{return journal_.ReadCheckpointRecords(this,&original,error,&read_snapshot,&views);}))return false;');
  s=replace(s,'for(std::size_t i=first;i<original.size();++i)\n            if(!before->ApplyMutationLocked(*original[i],false,error,nullptr,original[i],&source_bindings_,&derived_jobs_,proof,views[i]))return false;','if(!fc::Measure("checkpoint.originalSemantic",[&]{for(std::size_t i=first;i<original.size();++i)if(!before->ApplyMutationLocked(*original[i],false,error,nullptr,original[i],&source_bindings_,&derived_jobs_,proof,views[i]))return false;return true;}))return false;');
  ...
  s=replace(s,'for(std::size_t i=0;i<candidate.size();++i){const auto& m=candidate[i];\n            if(!m||!after->ApplyMutationLocked(*m,false,error,nullptr,m,&source_bindings_,&derived_jobs_,proof,candidate_views[i]))return false;}','if(!fc::Measure("checkpoint.candidateSemantic",[&]{for(std::size_t i=0;i<candidate.size();++i){const auto& m=candidate[i];if(!m||!after->ApplyMutationLocked(*m,false,error,nullptr,m,&source_bindings_,&derived_jobs_,proof,candidate_views[i]))return false;}return true;}))return false;');
