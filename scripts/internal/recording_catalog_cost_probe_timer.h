...
struct Metric {std::uint64_t count{},inclusive{},exclusive{},maximum{};};
...
struct Scope {
 ...
 ~Scope(){if(active){const auto elapsed=static_cast<std::uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(Clock::now()-begin).count());auto& m=metrics[key];++m.count;m.inclusive+=elapsed;m.exclusive+=elapsed-children;if(elapsed>m.maximum)m.maximum=elapsed;current=parent;if(parent)parent->children+=elapsed;}}
};
...
inline std::map<std::string,Metric> Take(){if(current)throw std::runtime_error("active timer on take");auto out=std::move(metrics);metrics.clear();return out;}
