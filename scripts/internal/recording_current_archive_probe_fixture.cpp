...
int main(int argc,char** argv){
 ...
 try{
  ...
  const auto index=std::stoul(mode);if(index>reasons.size())return 2;
  ...
  if(index<reasons.size()&&!store.catalog.FailDerivedJobAfterCleanup(job.job_id,job.attempt_id,reasons[index],20,&error))throw std::runtime_error("fixture-terminal-transition");
  std::optional<recording::DerivedJobRecordV1> saved;
  if(!store.catalog.FindDerivedJob(job.job_id,&saved,&error)||!saved||saved->state!=(index<reasons.size()?recording::DerivedJobState::Failed:recording::DerivedJobState::Intent))throw std::runtime_error("fixture-readback");
