...
test('S11-O25-01 bounded group members do not consume the top-level item budget',async()=>{
  const group=groupedOutput('one',Array.from({length:4111},(_,i)=>`member-${i}`));
  const result=await allTimelinePages(async()=>({total:0,unplacedTotal:1,offset:0,limit:1,items:[],unplacedItems:[group]}),{limit:1});
  assert.equal(result.unplacedItems.length,1);
  assert.equal(result.unplacedItems[0].members.length,4111);
});
test('S11-O25-02 group members above the separate bound are rejected before consumption',async()=>{
  const group=groupedOutput('one',Array.from({length:8193},(_,i)=>`member-${i}`));
  const observed=[];let consumed=0;
  await assert.rejects(allTimelinePages(async()=>({total:0,unplacedTotal:1,offset:0,limit:1,items:[],unplacedItems:[group]}),
    {limit:1,observe:event=>observed.push(event),consumePage:()=>{consumed++;}}),
  error=>error.code==='page-bound-leaf-limit'&&observed.at(-1)?.details?.leaves===8193);
...
for(const [name,groups,options,code] of [
  ...
  ['leaf cap',[groupedOutput('one',['m1','m2','m3'])],{maxLeaves:2},'page-bound-leaf-limit'],
