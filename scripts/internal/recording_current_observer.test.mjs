...
import {parsePhases,summarizeSpawnDiagnostic,validatedPhaseReceipt,snapshotTree,copyVerified,receiptTree,diagnosticRootIdentity,sourceManifest,preserveDiagnosticReceipt} from './recording_archive_diagnostic_profile.mjs';
...
try{
  ...
  check('O28-D03 small native snapshot exposes external and internal phases without changing JSON',()=>{const value=snapshot(),trace=parsePhases(value.stderr),names=new Set(trace.rows.map(row=>row.phase));
    console.log('[native-phase-diagnostic] '+JSON.stringify({phases:[...names].sort(),rowCount:trace.rows.length,traceLoss:trace.loss,outputBytes:Buffer.byteLength(value.stdout??'')+Buffer.byteLength(value.stderr??''),rawOutputPublished:false}));
    for(const phase of ['journal-open','catalog-open','open','open-replay','open-preflight','open-apply','sqlite-open','rebuild','rebuild-replay','rebuild-clear','rebuild-project','query','media','digest','output','destruct','catalog-destruct'])assert(names.has(phase),phase);
    assert.equal(trace.complete,true);assert.equal(trace.loss,false);assert.equal(summarizeSpawnDiagnostic(value).traceStatus,'complete');const json=JSON.parse(value.stdout);assert.equal(json.catalogRecovered,true);assert.equal(Object.keys(json).sort().join(','),'available,catalogRecovered,deleted,digest,segments');});
  check('O28-R04 actual native two verified copies preserve equal receipts before cleanup',()=>{const parent=fs.realpathSync(os.tmpdir()),repo=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'../..'),sourceTree=snapshotTree(path.dirname(original)),outputs=[],receipts=[];
    const manifestEntries=[{name:'catalog-source',file:path.join(repo,'src/recording/recording_catalog.cpp')},{name:'catalog-instrumented',file:path.join(root,'catalog-instrumented.cpp')},{name:'runtime-archive',file:path.join(repo,'build-gst-onnx/libmedia_server_runtime.a')},{name:'native-source',file:new URL('./recording_current_observer_native.cpp',import.meta.url).pathname},{name:'trace-header',file:new URL('./recording_archive_phase_trace.h',import.meta.url).pathname},{name:'native-binary',file:binary}],manifest=sourceManifest(manifestEntries);
    for(let i=0;i<2;i++){const copyRoot=fs.realpathSync(fs.mkdtempSync(path.join(parent,'media-server-current-observer-copy-')));fs.chmodSync(copyRoot,0o700);const identity=diagnosticRootIdentity(copyRoot,parent,'current-observer-snapshot'),copy=path.join(copyRoot,'recordings'),copyTree=copyVerified(path.dirname(original),copy,sourceTree),value=snapshotAt(copy),child=summarizeSpawnDiagnostic(value,{elapsedMs:0}),receipt={schema:'media-server.recording-diagnostic-receipt.v1',profile:'current-observer-snapshot',child,trace:validatedPhaseReceipt(value.stderr),command:{name:'recording-current-observer-native',args:['--snapshot','owned-copy'],timeoutMs:15000,maxBuffer:16384},sourceTree:receiptTree(sourceTree),copyTree:receiptTree(copyTree),originalUnchanged:snapshotTree(path.dirname(original)).sha256===sourceTree.sha256,manifestUnchanged:JSON.stringify(sourceManifest(manifestEntries))===JSON.stringify(manifest),groupClosed:value.status===0&&!value.signal&&!value.error,limits:{treeBytes:448*1024*1024,treeEntries:4096},manifest,rawPathsPublished:false},evidence=path.join(root,`r04-native-${i}.json`);preserveDiagnosticReceipt({evidencePath:evidence,receipt});outputs.push(value.stdout);receipts.push(JSON.parse(fs.readFileSync(evidence)).receipt);removeDiagnosticRoot(copyRoot,identity,true);assert(!fs.existsSync(copyRoot));}
    assert.equal(outputs[0],outputs[1]);for(const key of ['sourceTree','copyTree','manifest','command','limits'])assert.deepEqual(receipts[0][key],receipts[1][key]);assert.deepEqual(receipts[0].trace.rows.map(x=>[x.kind,x.phase]),receipts[1].trace.rows.map(x=>[x.kind,x.phase]));assert.equal(before,hash(fs.readFileSync(original)));});
  check('O28-R03 actual native failure preserves receipt and roots until verified cleanup',()=>{const parent=fs.realpathSync(os.tmpdir()),repo=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'../..'),base=snapshotTree(path.dirname(original)),sourceRoot=fs.realpathSync(fs.mkdtempSync(path.join(parent,'media-server-current-observer-copy-'))),copyRoot=fs.realpathSync(fs.mkdtempSync(path.join(parent,'media-server-current-observer-copy-')));fs.chmodSync(sourceRoot,0o700);fs.chmodSync(copyRoot,0o700);const sourceIdentity=diagnosticRootIdentity(sourceRoot,parent,'current-observer-snapshot'),copyIdentity=diagnosticRootIdentity(copyRoot,parent,'current-observer-snapshot'),source=path.join(sourceRoot,'recordings');copyVerified(path.dirname(original),source,base);fs.appendFileSync(path.join(source,'recording-v2-mutations.jsonl'),'{bad}\n');const sourceTree=snapshotTree(source),copy=path.join(copyRoot,'recordings'),copyTree=copyVerified(source,copy,sourceTree),value=snapshotAt(copy),child=summarizeSpawnDiagnostic(value,{elapsedMs:0}),manifestEntries=[{name:'catalog-source',file:path.join(repo,'src/recording/recording_catalog.cpp')},{name:'catalog-instrumented',file:path.join(root,'catalog-instrumented.cpp')},{name:'runtime-archive',file:path.join(repo,'build-gst-onnx/libmedia_server_runtime.a')},{name:'native-source',file:new URL('./recording_current_observer_native.cpp',import.meta.url).pathname},{name:'trace-header',file:new URL('./recording_archive_phase_trace.h',import.meta.url).pathname},{name:'native-binary',file:binary}],manifest=sourceManifest(manifestEntries),receipt={schema:'media-server.recording-diagnostic-receipt.v1',profile:'current-observer-snapshot',child,trace:validatedPhaseReceipt(value.stderr),command:{name:'recording-current-observer-native',args:['--snapshot','owned-copy'],timeoutMs:15000,maxBuffer:16384},sourceTree:receiptTree(sourceTree),copyTree:receiptTree(copyTree),originalUnchanged:snapshotTree(source).sha256===sourceTree.sha256,manifestUnchanged:JSON.stringify(sourceManifest(manifestEntries))===JSON.stringify(manifest),groupClosed:value.status===1&&!value.signal&&!value.error,limits:{treeBytes:448*1024*1024,treeEntries:4096},manifest,rawPathsPublished:false},evidence=path.join(root,'r03-native-failure.json');assert.equal(value.status,1);assert(['catalog-open','journal-open','recovery-errors'].includes(child.failureCode));preserveDiagnosticReceipt({evidencePath:evidence,receipt});assert(fs.existsSync(sourceRoot)&&fs.existsSync(copyRoot));removeDiagnosticRoot(copyRoot,copyIdentity,true);removeDiagnosticRoot(sourceRoot,sourceIdentity,true);assert(!fs.existsSync(sourceRoot)&&!fs.existsSync(copyRoot));assert.equal(before,hash(fs.readFileSync(original)));});
