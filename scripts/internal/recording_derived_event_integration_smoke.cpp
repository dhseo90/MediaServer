...
int DiagnosticMode(const std::filesystem::path& root,bool no_crypto){
    ...
    try{
        ...
        }
        {
            ...
            recording::DerivedEventWorkerOptions queued_options;queued_options.wait_ms=20;queued_options.retry_ms=10;queued_options.max_attempts=2;std::size_t queued_provider_calls=0;std::vector<recording::DerivedEventAttemptDiagnostic> queued_attempts;
            queued_options.latest_evidence=[&](const auto& reference){if(reference.reference_id==queued.reference_id)++queued_provider_calls;return recording::DerivedEventEvidenceUpdate{"probe-channel","probe-channel",full};};queued_options.diagnostic=[&](const auto& reference,const auto& value){if(reference.reference_id==queued.reference_id)queued_attempts.push_back(value);};
            ...
            const bool submitted=reached&&worker.Submit(queued,empty,&error);std::this_thread::sleep_for(std::chrono::milliseconds(40));{std::lock_guard lock(gate_mu);released=true;}gate_cv.notify_all();
            ...
            check(submitted&&!barrier_timeout&&queued_provider_calls==1&&queued_attempts.size()==1&&queued_attempts[0].attempt==1&&queued_attempts[0].wait_ms==20&&queued_attempts[0].attempt_limit==2&&queued_attempts[0].deadline_exhausted&&!queued_attempts[0].attempt_exhausted&&queued_attempts[0].selection_complete&&queued_attempts[0].elapsed_ms>=20&&current.jobs.size()==1&&current.jobs[0].job.ready&&current.jobs[0].job.ready->request_fully_satisfied&&current.jobs[0].outputs.size()==2,"LP09-Q01 expired queued first evaluation consumes current evidence without deadline extension");
