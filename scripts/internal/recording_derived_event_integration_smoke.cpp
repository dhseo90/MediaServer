...
int DiagnosticMode(const std::filesystem::path& root,bool no_crypto){
    ...
    try{
        ...
        options.latest_evidence=[&](const auto&){return recording::DerivedEventEvidenceUpdate{"probe-channel","probe-channel",full};};const auto updated=run(options,empty,&live);
