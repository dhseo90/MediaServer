...
bool BindingOracle(Store& store,const recording::DerivedJobIntentV1& intent){
 ...
 for(const auto name:{"payload","type","entity","owner","prior"}){
  ...
  if(test=="prior")store.catalog.derived_jobs_.at(intent.job_id).state=recording::DerivedJobState::Ready;
  const bool rejected=!store.catalog.ApplyDerivedJobMutationLocked(changed,&error,true,&prepared);
  if(test=="prior")store.catalog.derived_jobs_.at(intent.job_id).state=recording::DerivedJobState::Intent;
 ...
 store.catalog.accepted_segment_state_mutations_[mutation.mutation_id]=recording::SerializeRecordingMutationV1(mutation);
