...
lp_mode="${2:-envelope}"
if [[ "$lp_mode" != envelope && "$lp_mode" != job && "$lp_mode" != content && "$lp_mode" != envelope-cost && "$lp_mode" != context && "$lp_mode" != transition-comparison && "$lp_mode" != intent-comparison && "$lp_mode" != journal-location && "$lp_mode" != journal-location-crypto-off ]];then exit 2;fi
...
let journal=source;
if((mode==='journal-location'||mode==='journal-location-crypto-off')&&source.includes('RecordingJournal::AcquireLocatedRecord(')){
...
if((mode==='journal-location'||mode==='journal-location-crypto-off'))console.log('[instrument] location_journal_original_sha256='+crypto.createHash('sha256').update(source).digest('hex')+' location_journal_instrumented_sha256='+crypto.createHash('sha256').update(journalCopy).digest('hex'));
...
if((mode==='journal-location'||mode==='journal-location-crypto-off')){
...
if [[ "$lp_mode" == journal-location || "$lp_mode" == journal-location-crypto-off ]];then read -r lp_location_flag < "$lp_root/location_flags" || [[ -n "$lp_location_flag" ]];lp_flags+=("$lp_location_flag");lp_sources=("$lp_script/recording_journal_location_smoke.cpp");fi
