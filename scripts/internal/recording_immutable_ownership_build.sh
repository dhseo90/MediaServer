...
lp_mode="${2:-envelope}"
if [[ "$lp_mode" != envelope && "$lp_mode" != job && "$lp_mode" != content && "$lp_mode" != envelope-cost && "$lp_mode" != context && "$lp_mode" != transition-comparison && "$lp_mode" != intent-comparison ]];then exit 2;fi
...
if(mode==='envelope-cost'){
 ...
 journal=exact(journal,'std::string JournalBytes(const RecordingMutationHandles& records) {','std::string JournalBytes(const RecordingMutationHandles& records) { ++envelope_cost_probe::journal_bytes_calls;');
