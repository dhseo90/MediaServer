...
template<class T> std::string BindingBytes(const T& value){const auto* p=BindingAddress(value);Need(p!=nullptr);return SerializeRecordingSourceBindingV1(*p);}
...
void BindingPoolSafety(Store& store,const RecordingMutationV1& order,const RecordingMutationV1& mutation,const RecordingSegmentV2& segment,const RecordingSourceBindingV1& binding){
 ...
 {const auto original=store.catalog.FindSourceBindingOwnedLocked(segment.segment_id);const auto shadow=store.catalog.checkpoint_cache_->shadow->FindSourceBindingOwnedLocked(segment.segment_id);ownership_probe::ResetBindingPoolCounts();Need(store.catalog.Checkpoint(&error));
  Check(ownership_probe::BindingPoolLookups()==0&&ownership_probe::BindingPoolComparisons()==0&&store.catalog.FindSourceBindingOwnedLocked(segment.segment_id)==original&&store.catalog.checkpoint_cache_->shadow->FindSourceBindingOwnedLocked(segment.segment_id)==shadow,"LP18-O10 no-op checkpoint preserves handles without binding comparisons");}
...
void BindingSharing(const std::filesystem::path& root){
 ...
 {Store s(root);segment=PrepareBinding(s);binding.segment_id=segment.segment_id;binding.source_id=segment.source_id;binding.channel_id=segment.channel_id;binding.store_id=segment.store_id;binding.media_epoch_id=segment.media_epoch_id;binding.source_generation="typed-generation";binding.generation_order=7;binding.track_id="video/0";binding.samples={{1,10},{3,20}};binding.last_accepted_ordinal=3;canonical=SerializeRecordingSourceBindingV1(binding);
  ...
  Check(SerializeRecordingSourceBindingV1(*s.catalog.FindSourceBinding(segment.segment_id))==canonical&&BindingBytes(s.catalog.source_bindings_.at(segment.segment_id))==canonical,"LP18-O11 public binding value mutation remains isolated");
  ...
  Check(BindingBytes(s.catalog.source_bindings_.at(segment.segment_id))==canonical,"LP18-O11 source snapshot binding mutation remains isolated");
  const auto disk=Bytes(s.journal.path());auto changed=binding;changed.samples[0].pts_ns=11;bool inserted=true;
  Check(!s.catalog.RecoverBoundSegmentV2(segment,changed,(root/"typed.mp4").string(),&inserted,&error)&&!inserted&&Bytes(s.journal.path())==disk&&BindingBytes(s.catalog.source_bindings_.at(segment.segment_id))==canonical,"LP18-O12 same ID changed binding rejected without mutation");
  ...
  Need(s.catalog.Checkpoint(&error));Need(s.catalog.checkpoint_cache_&&s.catalog.checkpoint_cache_->shadow);
  Check(BindingAddress(s.catalog.source_bindings_.at(segment.segment_id))==BindingAddress(s.catalog.checkpoint_cache_->shadow->source_bindings_.at(segment.segment_id)),"LP18-O10 checkpoint shadow shares current live binding");
  ...
  Check(valid&&full.ProjectionSignatureLocked()==s.catalog.checkpoint_cache_->shadow->ProjectionSignatureLocked()&&BindingBytes(full.source_bindings_.at(segment.segment_id))==canonical,"LP18-O13 independent full replay binding projection preserved");
#if LP18_BINDING_SHARED
  ...
  Check(!s.catalog.FindSourceBinding(segment.segment_id)&&s.catalog.SegmentLifecycleV2(segment.segment_id)==RecordingLifecycle::Deleted&&BindingBytes(s.catalog.source_bindings_.at(segment.segment_id))==canonical,"LP18-O12 deleted source hidden with internal binding preserved");
  Need(s.catalog.Checkpoint(&error));Need(s.catalog.checkpoint_cache_&&s.catalog.checkpoint_cache_->shadow);
  Check(BindingAddress(s.catalog.source_bindings_.at(segment.segment_id))==BindingAddress(s.catalog.checkpoint_cache_->shadow->source_bindings_.at(segment.segment_id)),"LP18-O10 deleted checkpoint retains shared binding evidence");
 }
 for(bool sql:{true,false}){RecordingJournal journal(RecordingJournal::ManagedOptions{root,"probe-store"});Need(journal.Open(&error));auto options=Store::Options(root);options.prefer_sqlite=sql;RecordingCatalog catalog(journal,options);Need(catalog.Open(&error));const std::string mode=sql?"sqlite":"fallback";
  Check(catalog.catalog_mode_==(sql?"sqlite-primary":"jsonl-fallback")&&!catalog.FindSourceBinding(segment.segment_id)&&catalog.SegmentLifecycleV2(segment.segment_id)==RecordingLifecycle::Deleted&&BindingBytes(catalog.source_bindings_.at(segment.segment_id))==canonical&&(!sql||SqlBinding(catalog)==canonical),"LP18-O13 reopen deleted canonical binding preserved "+mode);
  Need(catalog.Checkpoint(&error));Need(catalog.checkpoint_cache_&&catalog.checkpoint_cache_->shadow);
  Check(BindingAddress(catalog.source_bindings_.at(segment.segment_id))==BindingAddress(catalog.checkpoint_cache_->shadow->source_bindings_.at(segment.segment_id)),"LP18-O10 reopened checkpoint shares live binding "+mode);
 }
}
}
