...
void Safety(RecordingCatalog& live,RecordingJournal& journal,const std::filesystem::path& root,const RecordingJournalReplayResult& history,const RecordingMutationV1& transition){
 ...
 {RecordingCatalog target(journal,Options(root));seed(target);const auto payload=transition.payload_json;RecordingCatalog::PreparedDerivedMutation prepared(&target,payload);Need(target.ApplyDerivedJobMutationLocked(transition,&error,false,&prepared));const auto original=target.derived_jobs_.at(id);target.derived_jobs_.at(id)=std::make_shared<const DerivedJobRecordV1>(*original);
  Check(!target.ApplyDerivedJobMutationLocked(transition,&error,true,&prepared)&&target.derived_jobs_.at(id)!=original&&SerializeDerivedJobRecord(*target.derived_jobs_.at(id))==SerializeDerivedJobRecord(*original),"LP18-J02 equal-content replacement invalidates Prepared prior ownership");}
 {auto owner=std::make_unique<RecordingCatalog>(journal,Options(root));seed(*owner);Need(owner->ApplyMutationLocked(transition,false,&error));RecordingCatalog target(journal,Options(root));seed(target);auto retained=owner->derived_jobs_.at(id);std::weak_ptr<const DerivedJobRecordV1> weak=retained;Need(target.ApplyMutationLocked(transition,false,&error,nullptr,{},nullptr,&owner->derived_jobs_));const bool alias=target.derived_jobs_.at(id)==retained;owner.reset();retained.reset();Check(alias&&!weak.expired()&&SerializeDerivedJobRecord(*target.derived_jobs_.at(id))==canonical,"LP18-J01 shared job survives pool owner destruction");}
 {const auto current=live.derived_jobs_.at(id);const auto shadow=live.checkpoint_cache_->shadow->derived_jobs_.at(id);ownership_probe::ResetJobPoolCounts();Need(live.Checkpoint(&error));Check(ownership_probe::JobPoolLookups()==0&&ownership_probe::JobPoolComparisons()==0&&live.derived_jobs_.at(id)==current&&live.checkpoint_cache_->shadow->derived_jobs_.at(id)==shadow,"LP18-J01 no-op checkpoint preserves job handles without pool comparisons");}
 {RecordingCatalog target(journal,Options(root));seed(target);ownership_probe::ResetJobPoolCounts();Need(target.ApplyMutationLocked(transition,false,&error,nullptr,{},nullptr,&live.derived_jobs_));Check(ownership_probe::JobPoolLookups()==1&&ownership_probe::JobPoolComparisons()==1&&target.derived_jobs_.at(id)==live.derived_jobs_.at(id),"LP18-J01 newly applied job compares only matching current pool entry");ownership_probe::ResetJobPoolCounts();Need(target.ApplyMutationLocked(transition,false,&error,nullptr,{},nullptr,&live.derived_jobs_));Check(ownership_probe::JobPoolLookups()==0&&ownership_probe::JobPoolComparisons()==0,"LP18-J01 duplicate job mutation performs no pool comparison");}
 {RecordingCatalog target(journal,Options(root));for(const auto& m:history.mutations)Need(target.ApplyMutationLocked(m,false,&error,nullptr,{},nullptr,&live.derived_jobs_));Check(target.derived_jobs_.at(id)!=live.derived_jobs_.at(id)&&target.derived_jobs_.at(id)->state==DerivedJobState::Intent,"LP18-J03 latest terminal pool cannot replace historical Intent");auto invalid=transition;invalid.mutation_type=RecordingMutationType::DerivedJobComplete;Check(!target.ApplyMutationLocked(invalid,false,&error,nullptr,{},nullptr,&live.derived_jobs_)&&target.derived_jobs_.at(id)->state==DerivedJobState::Intent,"LP18-J03 matching job pool cannot bypass strict state validation");}
 {const auto saved=live.derived_jobs_.at(id);live.derived_jobs_.at(id).reset();std::optional<DerivedJobRecordV1> one;std::vector<DerivedJobRecordV1> all,active;bool more=false;const bool denied=!live.FindDerivedJob(id,&one,&error)&&!one&&!live.SnapshotDerivedJobs(&all,&error)&&all.empty()&&!live.SnapshotActiveDerivedJobs(1,&active,&more,&error)&&active.empty()&&!more&&!live.RetentionSnapshot().authoritative&&live.DerivedJobProtectsLocked("segment");live.derived_jobs_.at(id)=saved;Check(denied,"LP18-J03 null resident job rejects snapshots and retention authority");}
}
#endif
void Run(const std::filesystem::path& root){
 ...
 {RecordingJournal journal(RecordingJournal::ManagedOptions{root,"store"});Need(journal.Open(&error));RecordingCatalog catalog(journal,Options(root));Need(catalog.Open(&error));RecordingOrderReservationV1 order;Need(journal.ReserveRecordingOrder("store","order","segment","channel",&order,&error));Need(order.sequence==1);std::filesystem::create_directories(root/"channel");Need(WriteContainedFileDurably(root,root/"channel/segment.mp4","0123456789ab",&error));Need(catalog.FinalizeBoundSegmentV2(input.source.segment,*input.source.binding,(root/"channel/segment.mp4").string(),&error));bool inserted=false;Need(catalog.BeginDerivedJobIntent(input.intent,&inserted,&error));Need(inserted);
  const auto initial=SerializeDerivedJobRecord(*Job(catalog.derived_jobs_.at(id)));DerivedJobRecordV1 parsed;Check(ParseDerivedJobRecord(initial,&parsed,&error)&&SerializeDerivedJobRecord(parsed)==initial&&catalog.DerivedJobProtectsLocked("segment"),"LP18-J03 initial canonical and active source protection preserved");
  ...
  Check(SerializeDerivedJobRecord(*Job(catalog.derived_jobs_.at(id)))==initial,"LP18-J03 FindDerivedJob returns independent value");
  ...
  Check(rejected&&SerializeDerivedJobRecord(*Job(catalog.derived_jobs_.at(id)))==initial&&Bytes(journal.path())==before,"LP18-J03 foreign Prepared owner rejected without transition");
  Need(catalog.AppendAndApplyLocked(mutation,&error,&prepared));expected=SerializeDerivedJobRecord(*Job(catalog.derived_jobs_.at(id)));
  Check(candidate_address==reinterpret_cast<std::uintptr_t>(Job(catalog.derived_jobs_.at(id)))&&expected==payload,"LP18-J02 validated record is published as the same owned object");
  ...
  Check(!catalog.ApplyDerivedJobMutationLocked(mutation,&error,true,&prepared)&&SerializeDerivedJobRecord(*Job(catalog.derived_jobs_.at(id)))==expected,"LP18-J03 consumed Prepared cannot apply twice");
  ...
  Check(!catalog.UpdateDerivedJob(&owner,invalid,&error)&&Bytes(journal.path())==failed_bytes&&SerializeDerivedJobRecord(*Job(catalog.derived_jobs_.at(id)))==expected,"LP18-J03 invalid terminal transition preserves state and bytes");catalog.UnbindDerivedService(&owner);
  ...
  Check(Job(catalog.derived_jobs_.at(id))==Job(catalog.checkpoint_cache_->shadow->derived_jobs_.at(id))&&SerializeDerivedJobRecord(*Job(catalog.checkpoint_cache_->shadow->derived_jobs_.at(id)))==expected,"LP18-J01 checkpoint shares current live job object");
#if LP18_JOB_SHARED
