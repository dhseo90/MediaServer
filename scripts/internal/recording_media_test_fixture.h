...
Encoded Encode(int frames, bool bframes, bool fractional,int width=160,int height=90) {
    const std::string rate = fractional ? "30000/1001" : "10/1";
    ...
    Pipeline pipe("videotestsrc num-buffers=" + std::to_string(frames) +
        " pattern=ball ! video/x-raw,width="+std::to_string(width)+",height="+std::to_string(height)+",framerate=" + rate + " ! " + encoder +
