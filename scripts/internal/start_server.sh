...
print_existing_server() {
  ...
  echo "local ops: http://$(client_host "${MEDIA_SERVER_HTTP_LISTEN_ADDRESS}"):${MEDIA_SERVER_HTTP_LISTEN_PORT}/ops/home"
...
if [[ "${MEDIA_SERVER_SKIP_BUILD}" != "1" ]]; then
  echo "[2/3] configure: ${BUILD_DIR} (youtube=${MEDIA_SERVER_ENABLE_YOUTUBE_SOURCE})"
...
echo "log: ${LOG_FILE}"
echo "local ops: http://${LOCAL_HTTP_HOST}:${MEDIA_SERVER_HTTP_LISTEN_PORT}/ops/home"
...
echo "external bind: http://${MEDIA_SERVER_HTTP_LISTEN_ADDRESS}:${MEDIA_SERVER_HTTP_LISTEN_PORT}/ops/home"
echo "stop: ./server.sh stop"
