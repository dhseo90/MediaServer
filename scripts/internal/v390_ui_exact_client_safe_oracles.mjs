...
const entries = [
  ...
  exactSpec("CLIENT-022", "caption/status가 viewport를 가리지 않는 a11y 상태", {
    route: "/client/live", role: "viewer", visibleControl: "[data-live-status]", fixtures: ["assigned-view", "live-status-sample"],
    apiAssertions: [api("GET", "/client/api/views", [200], { requiredTokens: ["assignedViewId"] })],
    domAssertions: [dom("[data-live-status]", "ariaLive", "equals", "polite"), dom("[data-live-status]", "overlaps", "equals", false), dom("video", "captionVisible", "equals", true)],
    semanticKeys: ["a11y-status", "captionVisible"], forbiddenFields: viewerRawFields, forbiddenNetwork: noWriteNetwork, forbiddenStateMutations: protocolMutations,
  ...
  hiddenBoundary("SAFE-018", "client API/UI debug/source/raw 정보 비노출", { route: "/client/live", role: "viewer", forbiddenSelectors: ["[data-debug]", "[data-source-url]", "pre.raw-json"], forbiddenFields: ["sourceUrl", "rawJson", "debugCounters"], semanticKeys: ["client-redaction", "debug-source-raw-absent"], apiPath: "/client/api/views" }),
  ...
  hiddenBoundary("SAFE-031", "viewer/client VLM internal material 비노출", { route: "/client/live", role: "viewer", forbiddenSelectors: ["[data-vlm-model]", "[data-vlm-review-card]"], forbiddenFields: ["modelId", "prompt", "provider", "rawProviderResponse"], semanticKeys: ["vlm-ops-only", "viewer-vlm-absent"], apiPath: "/client/api/views" }),
  ...
  opsBoundary("SAFE-041", "incident/action write는 Ops review/audit에만 저장", "incidentWorkflowBoundary", { route: "/ops/users", role: "admin", apiPath: "/ops/api/audit", fixtures: ["ops-review-record"], requiredTokens: ["reviewId", "auditActionRef"], forbiddenFields: ["rawNote", ...viewerRawFields], forbiddenStateMutations: [...protocolMutations, "eventRecord"] }),
  ...
  exactSpec("SAFE-052", "VLM summary candidate review는 manual Ops wrapper/no-call", {
    route: "/ops/events", role: "operator", visibleControl: "#opsIncidentSearchInput", action: { kind: "fill", target: "#opsIncidentSearchInput", value: "{candidateId}" }, fixtures: ["vlm-summary-candidate"],
    apiAssertions: [api("GET", "/ops/api/events", [200], { requiredTokens: ["candidateId", "manualReviewRequired"] })], domAssertions: [dom("[data-vlm-summary-candidate]", "text", "includes", "{candidateId}"), dom("#opsIncidentSearchInput", "value", "equals", "{candidateId}")],
    semanticKeys: ["vlm-summary-candidate", "manualReviewRequired"], forbiddenFields: viewerRawFields, forbiddenNetwork: ["external:vlm-provider", ...noWriteNetwork],
  ...
  exactSpec("SAFE-053", "incident-to-rule은 seeded suggestion에서 draft-only handoff", {
    route: "/ops/events", role: "operator", visibleControl: "[data-incident-rule-draft-route]", action: { kind: "activate", target: "[data-incident-rule-draft-route]" }, fixtures: ["incident-record", "rule-suggestion"],
    apiAssertions: [api("GET", "/ops/api/events", [200], { requiredTokens: ["incidentId", "ruleSuggestionId", "/ops/rules"] })], domAssertions: [dom("[data-incident-rule-draft-route]", "href", "startsWith", "/ops/rules"), dom("[data-rule-suggestion]", "text", "includes", "ruleSuggestionId")],
  ...
  opsBoundary("SAFE-060", "operational action pack은 manual workflow links만 표시", "operationalActionPack", { route: "/ops/events", selector: "#dashRuntimeOpsList", requiredTokens: ["manualWorkflowLink", "externalDeliveryPerformed=false"], forbiddenNetwork: ["external:webhook", ...noWriteNetwork] }),
  exactSpec("SAFE-061", "rule what-if는 seeded incident/suggestion condition preview만 표시", {
    ...
    apiAssertions: [api("GET", "/ops/api/events", [200], { requiredTokens: ["incidentId", "conditionPreview"] })], domAssertions: [dom("#opsEventRulePresetSelect", "value", "equals", "road"), dom("[data-rule-what-if-preview]", "text", "includes", "conditionPreview")],
  ...
  opsBoundary("SAFE-129", "continuity drill workspace read-only blocked/ready", "continuityDrillWorkspace", { route: "/ops/sources", selector: "[data-continuity-drill-workspace]", fixtures: ["drill-package"], requiredTokens: ["validationStatus", "blocked", "ready"], forbiddenFields: ["sourceUrl", "rawLocator", "credential"] }),
