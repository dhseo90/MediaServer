...
Checks:
  ...
  - allowed action은 GitHub 공식 Node 24 baseline action 또는 SHA-pinned action만 허용
...
for (const file of files) {
  ...
  for (const line of text.split(/\n/)) {
    ...
    if (uses && !isAllowedAction(uses[1])) {
      failures.push(`${relative}: action must be actions/checkout@v5, actions/upload-artifact@v6, SHA pinned, or local: ${uses[1]}`);
    }
  }
  if (/ACTIONS_ALLOW_USE_UNSECURE_NODE_VERSION\s*:\s*true/.test(text)) {
    failures.push(`${relative}: Node 20 opt-out env is not allowed`);
  }
}

verifyAnnotationPolicyDocs();
verifyNode24PolicyDocs();
...
function isAllowedAction(value) {
  const officialAction = /^(actions\/[A-Za-z0-9_.-]+)@(v\d+)$/.exec(value);
  const allowedOfficial = {
    "actions/checkout": new Set(["v5"]),
    "actions/upload-artifact": new Set(["v6"]),
  };
  const officialVersionTag = Boolean(officialAction && allowedOfficial[officialAction[1]]?.has(officialAction[2]));
...
function verifyNode24PolicyDocs() {
  ...
  for (const phrase of required) {
    if (!policyDocs.includes(phrase)) {
      failures.push(`Node 24 action policy docs missing phrase: ${phrase}`);
    }
  }
}
