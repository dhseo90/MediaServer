#!/usr/bin/env node
// 파일 용도: Client Live action reduction baseline이 반복 text button UI로 되돌아가지 않는지 검증한다.
...
check(
  ...
  script.includes('class="icon-button tile-action-primary"') &&
    script.includes('aria-label="타일 ${tile.index + 1} 재생"') &&
    script.includes('<span data-role="tile-playback-icon" aria-hidden="true">▶</span>') &&
...
check(
  "keyboard shortcuts cover start reconnect stop",
    ...
    script.includes("toggleLiveTilePlayback(tile.index)") &&
...
check(
  "ops/client UI smoke tracks the action contract",
  uiSmoke.includes('data-testid="client-live-action-reduction"') &&
    uiSmoke.includes('data-action="toggle-playback"') &&
...
if (failures.length > 0) {
  console.log("");
  console.log("== Client action reduction 실패 ==");
...
console.log("");
console.log("== Client action reduction 통과 ==");
