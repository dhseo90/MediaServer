...
const stabilityVerifierByPrefix = {
  ...
  LAB: ["verify-analysis-state", "verify-va-metadata-sidechannel", "verify-ws-metadata", "verify-image-analysis", "verify-vlm-pc-capability", "verify-vlm-recommendation-engine"],
  SAFE: ["verify-integrator-contract-artifact", "verify-auth-routes", "verify-ops-client-ui", "verify-ui-blocking-dialog-policy", "verify-event-post", "verify-webrtc-va-metadata", "verify-ws-metadata"],
};

check("inventory row count is stable", () => {
  assert(rows.length === 319, `expected 319 feature rows, found ${rows.length}`);
