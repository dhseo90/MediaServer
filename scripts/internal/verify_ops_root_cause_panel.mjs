...
check("ops dashboard exposes root cause panel", () => {
  ...
  const required = [
    ...
    'data-testid="ops-runtime-operations-console"',
    ...
    'id="dashIncidentTimelineShare"',
    "현재 인시던트 필터 링크 복사",
    'id="dashIncidentTimeline"',
    "제목, 출처, incident/cid 검색",
    "소스 수명주기, 지연, 재연결, 권한/설정 상태와 다음 조치",
    "문제 원인, EventRecord, source health, 로그 단서",
    'id="dashRuntimeOpsBadges"',
    'id="dashRuntimeOpsText"',
    'id="dashRuntimeOpsList"',
    "런타임 운영 판독",
    "scenario timeline, TrackHealth, recent EventRecord",
  ];
...
check("ops dashboard script interprets runtime root causes", () => {
  ...
  const required = [
    ...
    "dashboardRuntimeOpsEventRecords",
    "dashboardRuntimeOpsEventFailures",
    "dashboardRuntimeOpsTrackHealth",
    "dashboardRuntimeOpsHighWater",
    "dashboardRuntimeOpsNextAction",
    "trackingIssueMetric",
    ...
    "renderDashboardRuntimeOperations",
    "renderDashboardRuntimeOpsEmpty",
    "renderDashboardRuntimeOpsError",
    "dashRuntimeOpsBadges",
    "dashRuntimeOpsText",
    "dashRuntimeOpsList",
    "TrackHealth 이슈",
    "recent EventRecord",
    "high-water",
    "runtime/status, state-dump, metrics, EventRecord",
