...
projection_root="$(mktemp -d "$projection_parent/media-server-catalog-generation-scratch.XXXXXX")"
...
if(p.dirname(root)!==parent||!/^media-server-catalog-generation-scratch\.[A-Za-z0-9]+$/.test(p.basename(root))||
...
projection_sources=(
  ...
  "$projection_script/recording_catalog_generation_scratch_smoke.cpp"
)
echo '[scope] private B Catalog scratch; public Open/Append/SQLite blocked; no RAM/completion claim'
date -u '+[start] %Y-%m-%dT%H:%M:%SZ'
shasum -a 256 "${projection_sources[@]}" "$projection_repo/include/recording/recording_catalog_generation_projection.h" "$projection_repo/include/recording/recording_journal.h" "$projection_script/verify_recording_catalog_generation_scratch.sh"
shasum -a 256 "$projection_repo/include/recording/recording_catalog.h" "$projection_repo/include/recording/recording_generation_recovery_session.h" "$projection_script/recording_catalog_generation_projection_smoke.cpp" "$projection_script/recording_journal_generation_readonly_smoke.cpp"
