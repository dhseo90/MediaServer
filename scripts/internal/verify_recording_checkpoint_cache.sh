...
let source=exact(original,'bool RecordingCatalog::CheckpointLocked(bool recover_only,std::string* error) {','bool RecordingCatalog::CheckpointLocked(bool recover_only,std::string* error) { cache_probe::Scope cache_scope;');
source=exact(source,'    const auto original=journal_.Replay();','    auto original=journal_.Replay();if(cache_probe::bad_suffix)original.mutations.push_back(*cache_probe::bad_suffix);');
...
source=exact(source,'    const auto first=reuse?cached->prefix.size():0;','    const auto first=reuse?cached->prefix.size():0; cache_scope.first=first;cache_scope.records=original.mutations.size();');
