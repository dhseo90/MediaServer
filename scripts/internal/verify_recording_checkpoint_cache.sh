...
source=exact(source,'    if(!journal_.ReadCheckpointRecords(this,&original,error,&read_snapshot))return false;','    if(!journal_.ReadCheckpointRecords(this,&original,error,&read_snapshot))return false;if(cache_probe::bad_suffix)original.push_back(std::make_shared<const RecordingMutationV1>(*cache_probe::bad_suffix));');
