...
const readCurrent='        if(!measured(__LINE__,[&]{return journal_.ReadCheckpointRecords(this,&original,error,&read_snapshot,&views);})){ // read\n            if(!journal_.OwnsCatalog(this))derived_job_state_authoritative_=false;\n            return false;\n        }';
...
source=exact(source,'std::vector<std::string> RecordingCatalog::ProjectionSignatureLocked() const {','std::vector<std::string> RecordingCatalog::ProjectionSignatureLocked() const {\n    if(cache_probe::depth&&cache_probe::projection_exception){const auto mode=cache_probe::projection_exception;cache_probe::projection_exception=0;++cache_probe::projection_exceptions;if(mode==2)derived_job_state_authoritative_=false;throw std::runtime_error("owned-projection-injection");}');
...
source=exact(source,'    if(!measured(__LINE__,[&]{return journal_.CommitCheckpoint(this,candidate,recover_only,error,read_snapshot);}))return false; // commit','    if(cache_probe::fail_commit||!measured(__LINE__,[&]{return journal_.CommitCheckpoint(this,candidate,recover_only,error,read_snapshot);}))return false; // commit');
...
inline unsigned projection_exception=0,projection_exceptions=0;
...
const child=spawn(path.join(root,'check'),[root],{stdio:['ignore','pipe','pipe'],detached:true});
let limit=false,bytes=0,force,stdout='';
...
child.stdout.on('data',b=>{if(stdout.length<4*1024*1024)stdout+=b.toString();});
function size(p){const s=fs.lstatSync(p);return s.isDirectory()?fs.readdirSync(p).reduce((n,x)=>n+size(path.join(p,x)),0):s.size;}
...
child.on('close',(code,signal)=>{clearInterval(disk);clearTimeout(timer);clearTimeout(force);const matches=[...stdout.matchAll(/^\[peak-rss\] bytes=(\d+) source=getrusage-self$/gm)];const peakRssBytes=matches.length===1?Number(matches[0][1]):null;const rssPass=Number.isSafeInteger(peakRssBytes)&&peakRssBytes>0&&peakRssBytes<=512*1024*1024;console.log((rssPass?'[pass] ':'[fail] ')+'LP15-C04 peakRSS bytes='+peakRssBytes+' cap=536870912');console.log('[bounded] '+JSON.stringify({code,signal,limit,outputBytes:bytes,runtimeCapSeconds:60,diskCapBytes:536870912,peakRssBytes,peakSource:'getrusage-self'}));process.exitCode=limit||!rssPass?2:code===0?0:code===1?1:2;});
NODE
