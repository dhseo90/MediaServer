...
"${CXX:-c++}" -std=c++17 -Wall -Wextra -Werror -pthread -I"$ROOT_DIR/include" "${FLAGS[@]}" \
  ...
  "$SCRIPT_DIR/recording_consumer_connection_smoke.cpp" \
  ...
  "$ROOT_DIR/src/domain/strict_json.cpp" "${LIBS[@]}" -lz -o "$BUILD_DIR/check"
