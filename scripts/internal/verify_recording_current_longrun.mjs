...
import {CurrentRecordingObserver,CurrentLongrunProgress,CurrentObservationBudget,summarizeCurrentSamples,closedJournalComplete,disabledChannelsExact} from './recording_current_observer.mjs';
...
function budget(){if(cancelled)throw Error('observation-cancelled');if(performance.now()>deadline)throw Error('observation-deadline');if(size(root)>=448*1024*1024)throw Error('observation-root-cap');if(processes.some(p=>p.overflow))throw Error('private-log-cap');}
...
try{
  ...
  check(generated.status===0&&!generated.error&&!generated.signal&&fs.statSync(path.join(root,'input/retention.mp4')).size<96*1024*1024,'LP26-O05 original bounded retention fixture');
...
}catch(error){failed++;console.error('[fail] current observation: '+(error?.message?.match(/^[a-zA-Z0-9 .:-]+$/)?error.message:'redacted-error'));}
finally{
