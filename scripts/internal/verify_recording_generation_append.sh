#!/usr/bin/env bash
# B 명시 opt-in 증분 쓰기 focused. 실서버/운영 자료에 접근하지 않는다.
set -euo pipefail
task_script="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
task_repo="$(cd "$task_script/../.." && pwd)"
task_parent="$(cd "${TMPDIR:-/tmp}" && pwd -P)"
task_root="$(mktemp -d "$task_parent/media-server-generation-append.XXXXXX")"
...
if(p.dirname(root)!==parent||!/^media-server-generation-append\.[A-Za-z0-9]+$/.test(p.basename(root))||!s.isDirectory()||s.isSymbolicLink()||s.uid!==process.getuid()||s.uid!==expected.uid||s.dev!==expected.dev||s.ino!==expected.ino||f.realpathSync(root)!==root)throw Error("cleanup ownership");
...
task_sources=(
 ...
 src/recording/recording_generation_manifest.cpp src/recording/recording_generation_cold_mutation.cpp
 ...
 scripts/internal/recording_generation_append_smoke.cpp
)
cd "$task_repo"
...
shasum -a 256 "${task_sources[@]}" include/recording/recording_catalog.h include/recording/recording_journal.h include/recording/recording_generation_recovery_session.h \
 scripts/internal/recording_catalog_generation_scratch_smoke.cpp scripts/internal/recording_catalog_generation_projection_smoke.cpp scripts/internal/recording_journal_generation_readonly_smoke.cpp scripts/internal/recording_generation_append_sql_cases.inc "$task_script/verify_recording_generation_append.sh"
for task_config in '1 1 1' '1 0 1' '0 1 1' '1 1 0'; do
 ...
 "${CXX:-c++}" -std=c++17 -Wall -Wextra -Werror -pthread -Iinclude \
  ...
  "${task_cflags[@]}" "${task_sources[@]}" "${task_libs[@]}" -lz -o "$task_root/append-$task_crypto-$task_sqlite-$task_backend"
 "$task_root/append-$task_crypto-$task_sqlite-$task_backend" "$task_root/fixtures-$task_crypto-$task_sqlite-$task_backend"
done
