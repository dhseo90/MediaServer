#!/usr/bin/env bash
# 파일 용도: 녹화 세대 consumer 참조 smoke의 빌드·실행·정리를 수행한다.
# B 소비자 보호·예약 focused. 실서버/운영 자료에 접근하지 않는다.
set -euo pipefail
task_script="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
task_repo="$(cd "$task_script/../.." && pwd)"
task_parent="$(cd "${TMPDIR:-/tmp}" && pwd -P)"
task_root="$(mktemp -d "$task_parent/media-server-generation-consumers.XXXXXX")"
...
if(p.dirname(root)!==parent||!/^media-server-generation-consumers\.[A-Za-z0-9]+$/.test(p.basename(root))||!s.isDirectory()||s.isSymbolicLink()||s.uid!==process.getuid()||s.uid!==expected.uid||s.dev!==expected.dev||s.ino!==expected.ino||f.realpathSync(root)!==root)throw Error("cleanup ownership");
...
task_sources=(
 ...
 scripts/internal/recording_generation_consumers_smoke.cpp
)
cd "$task_repo"
date -u '+[start] %Y-%m-%dT%H:%M:%SZ'
uname -srm
"${CXX:-c++}" --version | head -1
shasum -a 256 "${task_sources[@]}" include/recording/recording_catalog.h include/recording/recording_journal.h include/recording/recording_generation_recovery_session.h src/recording/gstreamer_segment_writer.cpp src/recording/recording_derived_job_service.cpp \
 scripts/internal/recording_catalog_generation_scratch_smoke.cpp scripts/internal/recording_catalog_generation_projection_smoke.cpp scripts/internal/recording_journal_generation_readonly_smoke.cpp "$task_script/verify_recording_generation_consumers.sh"
for task_config in '1 1 1' '1 0 1' '0 1 1' '1 1 0'; do
 ...
 "${CXX:-c++}" -std=c++17 -Wall -Wextra -Werror -pthread -Iinclude \
  ...
  "${task_cflags[@]}" "${task_sources[@]}" "${task_libs[@]}" -lz -o "$task_root/consumers-$task_crypto-$task_sqlite-$task_backend"
 "$task_root/consumers-$task_crypto-$task_sqlite-$task_backend" "$task_root/fixtures-$task_crypto-$task_sqlite-$task_backend"
done
