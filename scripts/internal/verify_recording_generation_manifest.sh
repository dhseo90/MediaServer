#!/usr/bin/env bash
# 파일 용도: B안 세대 manifest의 독립 형식·게시 반례를 소유 임시 빌드에서 확인한다.
set -euo pipefail

b02_script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
b02_repo="$(cd "$b02_script_dir/../.." && pwd)"
b02_temp_parent="$(cd "${TMPDIR:-/tmp}" && pwd -P)"
b02_root="$(mktemp -d "$b02_temp_parent/media-server-generation-manifest.XXXXXX")"
b02_cleanup() {
  local prior=$?
  local name="${b02_root##*/}"
  if [[ -d "$b02_root" && ! -L "$b02_root" && -O "$b02_root" &&
        "${b02_root%/*}" == "$b02_temp_parent" &&
        "$name" =~ ^media-server-generation-manifest\.[A-Za-z0-9]+$ ]]; then
    rm -rf -- "$b02_root"
  fi
  if [[ -e "$b02_root" ]]; then
...
b02_sources=(
  ...
  "$b02_script_dir/recording_generation_manifest_smoke.cpp"
)
"${CXX:-c++}" -std=c++17 -Wall -Wextra -Werror -pthread -I"$b02_repo/include" \
  ...
  -o "$b02_root/manifest-crypto"
"$b02_root/manifest-crypto"

"${CXX:-c++}" -std=c++17 -Wall -Wextra -Werror -pthread -I"$b02_repo/include" \
  -DMEDIA_SERVER_USE_OPENSSL=0 "${b02_sources[@]}" \
  -o "$b02_root/manifest-no-crypto"
"$b02_root/manifest-no-crypto"
