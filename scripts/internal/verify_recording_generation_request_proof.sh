#!/usr/bin/env bash
# 파일 용도: 녹화 세대 request proof smoke의 빌드·실행·정리를 수행한다.
# 요청 내부 cold 증명 focused. 실서버/운영 자료에 접근하지 않는다.
set -euo pipefail
task_script="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
task_repo="$(cd "$task_script/../.." && pwd)"
task_parent="$(cd "${TMPDIR:-/tmp}" && pwd -P)"
task_root="$(mktemp -d "$task_parent/media-server-generation-request-proof.XXXXXX")"
...
if(p.dirname(root)!==parent||!/^media-server-generation-request-proof\.[A-Za-z0-9]+$/.test(p.basename(root))||!s.isDirectory()||s.isSymbolicLink()||s.uid!==process.getuid()||s.uid!==expected.uid||s.dev!==expected.dev||s.ino!==expected.ino||f.realpathSync(root)!==root)throw Error("cleanup ownership");
...
task_sources=(
 ...
 scripts/internal/recording_generation_request_proof_smoke.cpp
)
cd "$task_repo"
...
shasum -a 256 "${task_sources[@]}" include/recording/recording_catalog.h include/recording/recording_journal.h include/recording/recording_generation_recovery_session.h \
 ...
 scripts/internal/recording_catalog_generation_scratch_smoke.cpp scripts/internal/recording_catalog_generation_projection_smoke.cpp scripts/internal/recording_journal_generation_readonly_smoke.cpp "$task_script/verify_recording_generation_request_proof.sh"
