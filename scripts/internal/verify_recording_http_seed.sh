#!/usr/bin/env bash
# 파일 용도: 실제 HTTP용 V2 seed를 준비하고 자체 compile 임시물만 정리한다.
set -euo pipefail
...
"${CXX:-c++}" -std=c++17 -Wall -Wextra -Werror -pthread -I"$ROOT_DIR/include" "${FLAGS[@]}" \
 ...
 "$SCRIPT_DIR/recording_http_seed.cpp" \
 "${LINK_LIBS[@]}" -o "$RUN_DIR/check"
if [[ "${1:-}" == "--self-test" ]]; then
 "$RUN_DIR/check" "$RUN_DIR/recordings" "$RUN_DIR/manifest.json" 0
else
 test "$#" = 3
 "$RUN_DIR/check" "$1" "$2" "$3"
fi
