...
if((suite==='journal-location-crypto-off'||suite==='ownership-metrics')&&mode!=='green')throw Error('LP18_CRYPTO_OFF_GREEN_ONLY');
if(!['red','green'].includes(mode)||!['envelope','accepted','binding','job','content','envelope-cost','context','transition-comparison','intent-comparison','journal-location','journal-location-crypto-off','journal-cold','journal-checkpoint-snapshot','journal-logical','catalog-thin','ownership-metrics'].includes(suite)||!id||!/^[a-z0-9-]{1,40}$/.test(id)||extra.length)throw Error('LP18_ARGUMENTS');
...
if(suite==='journal-location'||suite==='journal-location-crypto-off'||suite==='journal-cold'||suite==='journal-checkpoint-snapshot'||suite==='journal-logical')names.push('scripts/internal/recording_journal_location_smoke.cpp','scripts/internal/recording_journal_location_counter.h');
const manifest=()=>names.map(name=>({name,sha256:crypto.createHash('sha256').update(fs.readFileSync(path.join(repo,name))).digest('hex')}));
if(suite==='catalog-thin'||suite==='ownership-metrics')names.push('scripts/internal/recording_catalog_thin_link_smoke.cpp','scripts/internal/recording_journal_location_counter.h','scripts/internal/recording_catalog_comparison_ownership.h','scripts/internal/recording_process_memory_probe.h');
let before=null;
...
try{
 ...
 if(suite==='journal-logical'){
  const notRun=run.stdout.split('\n').filter(l=>l.startsWith('[not-run] '));
  ok&&=mode==='red'?JSON.stringify(notRun)===JSON.stringify(['[not-run] LP18 logical ref scenarios=24 reason=capability-unavailable']):notRun.length===0;
 }
 if(suite==='catalog-thin'){
  const notRun=run.stdout.split('\n').filter(l=>l.startsWith('[not-run] '));
  ok&&=mode==='red'?JSON.stringify(notRun)===JSON.stringify(['[not-run] LP18 thin-link scenarios=28 reason=capability-unavailable']):notRun.length===0;
 }
