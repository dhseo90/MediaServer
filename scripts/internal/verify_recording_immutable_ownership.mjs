...
if(!['red','green'].includes(mode)||!['envelope','accepted','binding','job','content','envelope-cost','context','transition-comparison','intent-comparison','journal-location','journal-location-crypto-off','journal-cold'].includes(suite)||!id||!/^[a-z0-9-]{1,40}$/.test(id)||extra.length)throw Error('LP18_ARGUMENTS');
...
if(suite==='journal-location'||suite==='journal-location-crypto-off'||suite==='journal-cold')names.push('scripts/internal/recording_journal_location_smoke.cpp','scripts/internal/recording_journal_location_counter.h');
