#!/usr/bin/env bash
# 파일 용도: 숫자 source/channel 참조의 실제 writer·원장·조회을 검사한다.
set -euo pipefail
...
"${CXX:-c++}" -std=c++17 -Wall -Wextra -Werror -pthread -I"$ROOT_DIR/include" "${FLAGS[@]}" \
 ...
 "$SCRIPT_DIR/recording_numeric_reference_smoke.cpp" \
