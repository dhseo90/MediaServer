#!/usr/bin/env bash
# 파일 용도: 실제 public media의 격리 권한·보호을 검사한다.
set -euo pipefail
...
"${CXX:-c++}" -std=c++17 -Wall -Wextra -Werror -pthread -I"$ROOT_DIR/include" "${FLAGS[@]}" \
 ...
 "$SCRIPT_DIR/recording_public_media_smoke.cpp" \
