#!/usr/bin/env bash
# 파일 용도: 공개 timeline 투영의 격리 시간·상태을 검사한다.
set -euo pipefail
...
"${CXX:-c++}" -std=c++17 -Wall -Wextra -Werror -pthread -I"$ROOT_DIR/include" "${FLAGS[@]}" \
 ...
 "$SCRIPT_DIR/recording_public_timeline_smoke.cpp" \
