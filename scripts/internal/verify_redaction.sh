...
run_live_redaction() {
  local query live_ffmpeg_timeout live_rtsp_attempts
  query="$(redaction_query)"
  live_ffmpeg_timeout="${MEDIA_SERVER_VERIFY_REDACTION_LIVE_FFMPEG_TIMEOUT_S:-}"
  if [[ -z "${live_ffmpeg_timeout}" && "${DURATION_S}" =~ ^[0-9]+$ ]]; then
    live_ffmpeg_timeout=$((DURATION_S + 120))
  fi
  live_rtsp_attempts="${MEDIA_SERVER_VERIFY_REDACTION_LIVE_RTSP_ATTEMPTS:-3}"
  run_step "live-va-redaction" \
    "MEDIA_SERVER_SKIP_LOCAL_ENV=${MEDIA_SERVER_SKIP_LOCAL_ENV:-0} MEDIA_SERVER_LISTEN_PORT=${MEDIA_SERVER_LISTEN_PORT:-} MEDIA_SERVER_HTTP_LISTEN_PORT=${HTTP_PORT} MEDIA_SERVER_VERIFY_VA_HTTP_BASE='${HTTP_BASE}' MEDIA_SERVER_VERIFY_VA_FILE='${FILE_TOKEN}' MEDIA_SERVER_VERIFY_VA_DURATION_S=${DURATION_S} MEDIA_SERVER_VERIFY_VA_FFMPEG_TIMEOUT_S=${live_ffmpeg_timeout} MEDIA_SERVER_VERIFY_VA_RTSP_ATTEMPTS=${live_rtsp_attempts} MEDIA_SERVER_VERIFY_VA_EXTRA_QUERY='${query}' MEDIA_SERVER_VERIFY_VA_REDACTION=person-mosaic MEDIA_SERVER_VERIFY_VA_REDACTION_CLASSES='${REDACTION_CLASSES}' MEDIA_SERVER_VERIFY_VA_REDACTION_BLOCK_SIZE=${REDACTION_BLOCK_SIZE} MEDIA_SERVER_VERIFY_VA_REDACTION_MARGIN_RATIO=${REDACTION_MARGIN_RATIO} ./server.sh verify-va" || true
}
