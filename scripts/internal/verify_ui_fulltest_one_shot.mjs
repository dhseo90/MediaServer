...
function writeSummary(result) {
  const summary = {
    ...
    visualWidths,
    manualResult,
    uiFulltestEvidenceStatus: inAppEvidence ? "provided" : "not-provided",
    manualResultStatus: manualResult ? (skipManualResult ? "skipped" : "provided") : "not-provided",
    browserMode,
    inAppEvidence,
    allowChromeFallback,
    ...
    longrunStatus: {
      predev30: "not-run-by-this-wrapper",
      predev120: "not-run-by-this-wrapper",
      runtimeConsole120: "not-run-by-this-wrapper",
    },
    evidenceBoundary: "wrapperResult is not UI fulltest, 30-minute, 120-minute, or manual-result execution evidence",
...
function renderMarkdown(summary) {
  const lines = [
    ...
    `- wrapperResult: ${summary.wrapperResult}`,
    `- resultScope: ${summary.resultScope}`,
    `- outputDir: ${summary.outputDir}`,
    `- widths: ${summary.widths}`,
    `- visualWidths: ${summary.visualWidths}`,
    `- browserMode: ${summary.browserMode}`,
    `- uiFulltestEvidenceStatus: ${summary.uiFulltestEvidenceStatus}`,
    `- manualResultStatus: ${summary.manualResultStatus}`,
    ...
    `- longrunStatus.predev30: ${summary.longrunStatus.predev30}`,
    `- longrunStatus.predev120: ${summary.longrunStatus.predev120}`,
    `- longrunStatus.runtimeConsole120: ${summary.longrunStatus.runtimeConsole120}`,
