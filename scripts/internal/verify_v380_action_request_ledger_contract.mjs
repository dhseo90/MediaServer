...
check("docs, inventory, and dispatch map v3.8 Step 4", () => {
  ...
  for (const id of ["LAB-113", "SAFE-183", "OPS-150"]) assert(files.implementationManifest.items.find(item => item.id === id)?.verifierEvidence?.command === command, `${id} manifest verifier command drift`);
...
check("SAFE-183 canonical bounded product boundary", () => {
  const block = extractCppFunctionBlock(files.server, "std::string OpsV380ActionRequestLedgerContractJson(");
  const routeObserved = files.server.includes("/ops/api/actions/request-ledger");
  const safe183BoundaryObserved = block.includes("BuildV380ActionRequestLedgerContractItems") && block.includes("requestWritePerformed");
  ...
  assert(routeObserved && writePerformed === false && mutationPerformed === false && executionPerformed === false && sendPerformed === false && providerCallPerformed === false, "OPS-150 canonical bounded absence oracle");
  assert(safe183BoundaryObserved && block.includes("media-server.ops.v380-action-request-ledger-contract.v1") && writePerformed === false && mutationPerformed === false && executionPerformed === false && sendPerformed === false && automaticApplyPerformed === false && fieldSmokeExecuted === false && providerCallPerformed === false && rawMaterialExposed === false && sourceUrlExposed === false && credentialMaterialExposed === false && debugMaterialExposed === false && viewerClientExposureAdded === false && mediaPathChanged === false,
    "SAFE-183 requestWritePerformed must remain no-execution no-write redacted and client/provider isolated");
});
