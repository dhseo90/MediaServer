...
check("SAFE-184 canonical bounded product boundary", () => {
  const block = extractCppFunctionBlock(files.server, "std::string OpsV380ApprovalDecisionGateJson(");
  const routeObserved = files.server.includes("/ops/api/actions/approval-decision-gate");
  const outcomeObserved = block.includes("auditRef");
  const safe184BoundaryObserved = block.includes("BuildV380ApprovalDecisionGateItems") && block.includes("decisionWritePerformed");
  ...
  assert(routeObserved && outcomeObserved && writePerformed === false && mutationPerformed === false && executionPerformed === false && sendPerformed === false && providerCallPerformed === false, "OPS-151 canonical bounded absence oracle");
  assert(safe184BoundaryObserved && block.includes("media-server.ops.v380-approval-decision-gate.v1") && writePerformed === false && mutationPerformed === false && executionPerformed === false && sendPerformed === false && automaticApplyPerformed === false && fieldSmokeExecuted === false && providerCallPerformed === false && rawMaterialExposed === false && sourceUrlExposed === false && credentialMaterialExposed === false && debugMaterialExposed === false && viewerClientExposureAdded === false && mediaPathChanged === false,
    "SAFE-184 decisionWritePerformed must remain no-execution no-write redacted and client/provider isolated");
});
