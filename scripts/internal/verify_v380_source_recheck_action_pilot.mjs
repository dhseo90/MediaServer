...
check("docs, inventory, and dispatch map v3.8 Step 7", () => {
  ...
  for (const id of ["LAB-116", "SAFE-186", "OPS-153"]) assert(files.implementationManifest.items.find(item => item.id === id)?.verifierEvidence?.command === command, `${id} manifest verifier command drift`);
...
check("SAFE-186 canonical bounded product boundary", () => {
  const block = extractCppFunctionBlock(files.server, "std::string OpsV380SourceRecheckActionPilotJson(");
  const routeObserved = files.server.includes("/ops/api/actions/source-recheck-pilot");
  const safe186BoundaryObserved = block.includes("BuildV380SourceRecheckActionPilotItems") && block.includes("sourceRecheckExecuted");
  ...
  assert(routeObserved && writePerformed === false && mutationPerformed === false && executionPerformed === false && sendPerformed === false && providerCallPerformed === false, "OPS-153 canonical bounded absence oracle");
  assert(safe186BoundaryObserved && block.includes("media-server.ops.v380-source-recheck-action-pilot.v1") && writePerformed === false && mutationPerformed === false && executionPerformed === false && sendPerformed === false && automaticApplyPerformed === false && fieldSmokeExecuted === false && providerCallPerformed === false && rawMaterialExposed === false && sourceUrlExposed === false && credentialMaterialExposed === false && debugMaterialExposed === false && viewerClientExposureAdded === false && mediaPathChanged === false,
    "SAFE-186 sourceRecheckExecuted must remain no-execution no-write redacted and client/provider isolated");
});
