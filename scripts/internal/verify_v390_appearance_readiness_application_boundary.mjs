...
check("CMake and current graph bind Slice 17", () => {
  ...
  const assertGraphBoundary = value => {
    ...
    assert(applicationOwner?.exactFiles.includes(headerPath) && applicationOwner.exactFiles.includes(sourcePath) &&
      applicationOwner.expectedFileCount === 15 && applicationOwner.expectedCppCount === 5,
    ...
    assert(edge("transport-and-auth-adapter -> analysis-services")?.witnessCount === 17 &&
      edge("transport-and-auth-adapter -> analysis-services")?.allowedByTarget === false,
    "transport-to-analysis witness must reduce exactly 18 to 17");
    assert(edge("application-service-interfaces -> analysis-services")?.witnessCount === 2 &&
      edge("application-service-interfaces -> analysis-services")?.allowedByTarget === true &&
      edge("transport-and-auth-adapter -> application-service-interfaces")?.witnessCount === 8 &&
  ...
  mutated.observedModuleEdges.find(item =>
    item.direction === "transport-and-auth-adapter -> analysis-services").witnessCount = 18;
  let rejected = false;
