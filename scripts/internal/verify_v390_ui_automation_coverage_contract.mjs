...
check("positive matrix covers the exact 424 manual UI test IDs from all feature families", () => {
  ...
  assert(summary.coverageStatus === "review4-57-60-pending", "coverage status mismatch");
  ...
  assert(summary.exactNativeWorkflowReadinessComplete === false,
    "matrix must not claim exact native readiness before REVIEW4-57~60");
...
check("durable matrix and release docs record exact-ID partial coverage boundaries", () => {
  ...
  for (const snippet of ["V390-ADD1-11", command, contractCommand,
    "exact UI test ID 424", "prefix/range 판정 제거", "review4-57-60-pending"]) {
