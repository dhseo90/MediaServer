#!/usr/bin/env node
// 파일 용도: v4.0.0 release readiness(네 영역 판정, 30/UI 미실행 blocker, close-out dry-run)를 검증한다.
...
Checks:
  - four AGENTS test areas are judged with 30/UI as unrun required blockers
...
check("release-readiness fixture records four-area judgment and unrun blockers", () => {
  ...
  assert(fixture.implementationStatus === "policy-steps-complete-execution-gates-not-run",
    "fixture must not claim 30/UI/release-action PASS");
  ...
  assert(byCategory["30분 테스트"].approvalStatus === "unrun-required-blocker", "30분 blocker drifted");
...
check("backlog records v4.0.0 (8) readiness complete without claiming execution PASS", () => {
...
check("release readiness does not claim 30/UI/tag PASS", () => {
  for (const snippet of [
    "30-minute soak PASS",
    "UI fulltest PASS",
    "v4.0.0 tag",
  ...
  const recordsBlockers = files.releaseRecords.includes("미실행 필수 blocker") ||
    files.releaseRecords.includes("unrun-required-blocker");
  assert(recordsBlockers, "release records must keep 30/UI as unrun required blockers");
});
...
console.log("- soak30: unrun-required-blocker");
