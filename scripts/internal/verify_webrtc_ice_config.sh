...
WHIP_HTTP_AUTHORITY="${HTTP_BASE#*://}"
WHIP_HTTP_AUTHORITY="${WHIP_HTTP_AUTHORITY%%/*}"
WHIP_HTTP_HOST="${WHIP_HTTP_AUTHORITY%%:*}"
WHIP_HTTP_PORT="${WHIP_HTTP_AUTHORITY##*:}"
if [[ "${WHIP_HTTP_PORT}" == "${WHIP_HTTP_AUTHORITY}" ]]; then
  if [[ "${HTTP_BASE}" == https://* ]]; then
    WHIP_HTTP_PORT="443"
  else
    WHIP_HTTP_PORT="80"
  fi
...
if [[ "${SKIP_WHIP}" == "1" ]]; then
  ...
  if MEDIA_SERVER_SKIP_LOCAL_ENV=1 \
      MEDIA_SERVER_AUTH_MODE="${MEDIA_SERVER_AUTH_MODE:-}" \
      MEDIA_SERVER_LISTEN_ADDRESS="${MEDIA_SERVER_LISTEN_ADDRESS:-}" \
      MEDIA_SERVER_HTTP_LISTEN_ADDRESS="${WHIP_HTTP_HOST}" \
      MEDIA_SERVER_LISTEN_PORT="${MEDIA_SERVER_LISTEN_PORT:-}" \
      MEDIA_SERVER_HTTP_LISTEN_PORT="${WHIP_HTTP_PORT}" \
      MEDIA_SERVER_ROUTE="${MEDIA_SERVER_ROUTE:-}" \
      MEDIA_SERVER_VERIFY_RTSP_HOST="${MEDIA_SERVER_VERIFY_RTSP_HOST:-}" \
      MEDIA_SERVER_VERIFY_HTTP_HOST="${WHIP_HTTP_HOST}" \
      MEDIA_SERVER_VERIFY_SKIP_RTSP=1 MEDIA_SERVER_VERIFY_SOURCE_FILTER=webrtc_local_publish_h264_opus \
