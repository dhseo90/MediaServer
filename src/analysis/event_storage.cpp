...
void AppendFourCc(std::vector<unsigned char>* output, const char (&fcc)[5]) {
    if (output == nullptr) {
        return;
    }
    output->push_back(static_cast<unsigned char>(fcc[0]));
    output->push_back(static_cast<unsigned char>(fcc[1]));
    output->push_back(static_cast<unsigned char>(fcc[2]));
    output->push_back(static_cast<unsigned char>(fcc[3]));
}

void AppendU16Le(std::vector<unsigned char>* output, std::uint16_t value) {
    if (output == nullptr) {
        return;
    }
...
void AppendU32Le(std::vector<unsigned char>* output, std::uint32_t value) {
    if (output == nullptr) {
        return;
    }
    output->push_back(static_cast<unsigned char>(value & 0xFFU));
...
void PatchU32Le(std::vector<unsigned char>* output, std::size_t offset, std::uint32_t value) {
    if (output == nullptr || offset + 4U > output->size()) {
        return;
    }
    (*output)[offset] = static_cast<unsigned char>(value & 0xFFU);
    (*output)[offset + 1U] = static_cast<unsigned char>((value >> 8U) & 0xFFU);
    (*output)[offset + 2U] = static_cast<unsigned char>((value >> 16U) & 0xFFU);
    (*output)[offset + 3U] = static_cast<unsigned char>((value >> 24U) & 0xFFU);
}

std::size_t BeginRiffList(std::vector<unsigned char>* output, const char (&type)[5]) {
    AppendFourCc(output, "LIST");
    const std::size_t size_offset = output != nullptr ? output->size() : 0;
    AppendU32Le(output, 0);
    AppendFourCc(output, type);
    return size_offset;
}

void EndRiffList(std::vector<unsigned char>* output, std::size_t size_offset) {
    if (output == nullptr || size_offset + 4U > output->size()) {
        return;
    }
    PatchU32Le(output,
               size_offset,
               static_cast<std::uint32_t>(output->size() - size_offset - 4U));
}

void AppendRiffChunk(std::vector<unsigned char>* output,
                     const char (&fourcc)[5],
                     const std::vector<unsigned char>& payload) {
    AppendFourCc(output, fourcc);
    AppendU32Le(output, static_cast<std::uint32_t>(payload.size()));
    if (output != nullptr) {
...
bool BuildDibFramePayload(const RawVideoFrame& frame,
                          int expected_width,
                          int expected_height,
    ...
    if (output == nullptr) {
        if (error_message != nullptr) {
            *error_message = "missing encoded clip frame output";
        }
        return false;
    }
    output->clear();
    if (frame.width != expected_width || frame.height != expected_height ||
        ...
        if (error_message != nullptr) {
            *error_message = "encoded clip frames must share non-empty dimensions";
        }
        return false;
    }
    const int channels = PixelChannelCount(frame.format);
    if (channels <= 0) {
        if (error_message != nullptr) {
            *error_message = "encoded clip supports RGB/BGR/Gray8 frames only";
        }
        return false;
    }
    const std::size_t source_row_bytes =
        static_cast<std::size_t>(frame.width) * static_cast<std::size_t>(channels);
    const std::size_t expected_bytes = source_row_bytes * static_cast<std::size_t>(frame.height);
    if (frame.data.size() < expected_bytes) {
        if (error_message != nullptr) {
            *error_message = "encoded clip frame data is smaller than expected";
        }
        return false;
    }
    const std::size_t output_row_bytes =
        ((static_cast<std::size_t>(frame.width) * 3U + 3U) / 4U) * 4U;
    output->assign(output_row_bytes * static_cast<std::size_t>(frame.height), 0);
    for (int row = 0; row < frame.height; ++row) {
        const int source_row = frame.height - 1 - row;
        for (int col = 0; col < frame.width; ++col) {
            const std::size_t source_offset =
                (static_cast<std::size_t>(source_row) * static_cast<std::size_t>(frame.width) +
                 static_cast<std::size_t>(col)) *
                static_cast<std::size_t>(channels);
            const std::size_t target_offset =
                static_cast<std::size_t>(row) * output_row_bytes +
                static_cast<std::size_t>(col) * 3U;
            if (frame.format == PixelFormat::Gray8) {
                const unsigned char gray = frame.data[source_offset];
                (*output)[target_offset] = gray;
                (*output)[target_offset + 1U] = gray;
                (*output)[target_offset + 2U] = gray;
            } else if (frame.format == PixelFormat::RGB) {
                (*output)[target_offset] = frame.data[source_offset + 2U];
                (*output)[target_offset + 1U] = frame.data[source_offset + 1U];
                (*output)[target_offset + 2U] = frame.data[source_offset];
            } else {
                (*output)[target_offset] = frame.data[source_offset];
                (*output)[target_offset + 1U] = frame.data[source_offset + 1U];
                (*output)[target_offset + 2U] = frame.data[source_offset + 2U];
            }
        }
    }
    if (error_message != nullptr) {
...
struct EncodedClipResult {
    ...
    std::string format{"avi"};
    std::string codec{"raw-bgr24-dib"};
...
bool BuildRawAviClip(const std::vector<std::pair<BufferedEventFrame, std::filesystem::path>>& frames,
                     std::vector<unsigned char>* output,
                     int* fps,
                     int* duration_ms,
                     std::string* error_message) {
    ...
    if (width <= 0 || height <= 0) {
        if (error_message != nullptr) {
            *error_message = "encoded clip frame dimensions are invalid";
        }
        return false;
    }
    std::vector<std::vector<unsigned char>> dib_frames;
    dib_frames.reserve(frames.size());
    for (const auto& item : frames) {
        std::vector<unsigned char> payload;
        if (!BuildDibFramePayload(item.first.frame, width, height, &payload, error_message)) {
            return false;
        }
        dib_frames.push_back(std::move(payload));
    }

    const int local_fps = EstimateClipFps(frames);
    const std::uint32_t frame_bytes = static_cast<std::uint32_t>(dib_frames.front().size());
    const std::uint32_t frame_count = static_cast<std::uint32_t>(dib_frames.size());
    const std::uint32_t micros_per_frame = static_cast<std::uint32_t>(1000000 / local_fps);

    AppendFourCc(output, "RIFF");
    const std::size_t riff_size_offset = output->size();
    AppendU32Le(output, 0);
    AppendFourCc(output, "AVI ");

    const std::size_t hdrl_size_offset = BeginRiffList(output, "hdrl");
    std::vector<unsigned char> avih;
    AppendU32Le(&avih, micros_per_frame);
    AppendU32Le(&avih, frame_bytes * static_cast<std::uint32_t>(local_fps));
    AppendU32Le(&avih, 0);
    AppendU32Le(&avih, 0x10);
    AppendU32Le(&avih, frame_count);
    AppendU32Le(&avih, 0);
    AppendU32Le(&avih, 1);
    AppendU32Le(&avih, frame_bytes);
    AppendU32Le(&avih, static_cast<std::uint32_t>(width));
    AppendU32Le(&avih, static_cast<std::uint32_t>(height));
    AppendU32Le(&avih, 0);
    AppendU32Le(&avih, 0);
    AppendU32Le(&avih, 0);
    AppendU32Le(&avih, 0);
    AppendRiffChunk(output, "avih", avih);

    const std::size_t strl_size_offset = BeginRiffList(output, "strl");
    std::vector<unsigned char> strh;
    AppendFourCc(&strh, "vids");
    AppendFourCc(&strh, "DIB ");
    AppendU32Le(&strh, 0);
    AppendU16Le(&strh, 0);
    AppendU16Le(&strh, 0);
    AppendU32Le(&strh, 0);
    AppendU32Le(&strh, 1);
    AppendU32Le(&strh, static_cast<std::uint32_t>(local_fps));
    AppendU32Le(&strh, 0);
    AppendU32Le(&strh, frame_count);
    AppendU32Le(&strh, frame_bytes);
    AppendU32Le(&strh, 0xFFFFFFFFU);
    AppendU32Le(&strh, 0);
    AppendU32Le(&strh, 0);
    AppendU32Le(&strh, 0);
    AppendU32Le(&strh, static_cast<std::uint32_t>(width));
    AppendU32Le(&strh, static_cast<std::uint32_t>(height));
    AppendRiffChunk(output, "strh", strh);

    std::vector<unsigned char> strf;
    AppendU32Le(&strf, 40);
    AppendU32Le(&strf, static_cast<std::uint32_t>(width));
    AppendU32Le(&strf, static_cast<std::uint32_t>(height));
    AppendU16Le(&strf, 1);
    AppendU16Le(&strf, 24);
    AppendU32Le(&strf, 0);
    AppendU32Le(&strf, frame_bytes);
    AppendU32Le(&strf, 0);
    AppendU32Le(&strf, 0);
    AppendU32Le(&strf, 0);
    AppendU32Le(&strf, 0);
    AppendRiffChunk(output, "strf", strf);
    EndRiffList(output, strl_size_offset);
    EndRiffList(output, hdrl_size_offset);
    ...
    index_entries.reserve(dib_frames.size());
    const std::size_t movi_size_offset = BeginRiffList(output, "movi");
    const std::size_t movi_data_start = output->size();
    for (const auto& payload : dib_frames) {
        index_entries.push_back(AviIndexEntry{
            static_cast<std::uint32_t>(output->size() - movi_data_start),
            static_cast<std::uint32_t>(payload.size()),
        });
        AppendFourCc(output, "00db");
        AppendU32Le(output, static_cast<std::uint32_t>(payload.size()));
        output->insert(output->end(), payload.begin(), payload.end());
        if ((payload.size() % 2U) != 0U) {
            output->push_back(0);
    ...
    EndRiffList(output, movi_size_offset);

    std::vector<unsigned char> idx1;
    for (const auto& entry : index_entries) {
        AppendFourCc(&idx1, "00db");
        AppendU32Le(&idx1, 0x10);
        AppendU32Le(&idx1, entry.offset);
        AppendU32Le(&idx1, entry.size);
    }
    AppendRiffChunk(output, "idx1", idx1);
    PatchU32Le(output, riff_size_offset, static_cast<std::uint32_t>(output->size() - 8U));

    if (fps != nullptr) {
        *fps = local_fps;
    }
    if (duration_ms != nullptr) {
        *duration_ms = static_cast<int>((static_cast<std::int64_t>(frame_count) * 1000LL) /
                                        std::max(1, local_fps));
    }
    if (error_message != nullptr) {
        error_message->clear();
...
bool WriteEncodedClipManifest(const EventRecord& record,
    ...
    manifest << "{"
             << "\"schema\":\"media-server.va.encoded-event-clip.v1\","
             << "\"status\":\"completed\","
...
bool EncodeEventClipArtifact(const EventRecord& record,
    ...
    *result = EncodedClipResult{};
    ...
    std::vector<unsigned char> avi;
    if (!BuildRawAviClip(frames, &avi, &result->fps, &result->duration_ms, error_message)) {
        return false;
    }
    const std::filesystem::path media_path = encoded_dir / "event-clip.avi";
    if (!WriteBinaryFile(media_path, avi, error_message)) {
        return false;
    }
    result->media_path = media_path.string();
    result->byte_size = static_cast<std::uint64_t>(avi.size());
    result->manifest_path = (encoded_dir / "encoded-manifest.json").string();
    if (!WriteEncodedClipManifest(record, options, frame_bundle_manifest_path, frames, *result, error_message)) {
        return false;
    }
    if (error_message != nullptr) {
        error_message->clear();
...
bool WriteClipMedia(const EventRecord& record,
    ...
    if (!EncodeEventClipArtifact(record, options, manifest_path, written_frames, &encoded_clip, error_message)) {
        return false;
    }
    std::ofstream manifest(manifest_path, std::ios::out | std::ios::trunc);
    if (!manifest.good()) {
