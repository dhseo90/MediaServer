...
std::string ProductSharedUiScript() {
    ...
    window.MediaServerUi = window.MediaServerUi || (() => {
      ...
      const koToEn = new Map(Object.entries({
        ...
        '구성, 런타임 상태, 위험 신호와 다음 조치를 한 화면에서 선택합니다.': 'Choose configuration, runtime status, risk signals, and next actions from one screen.',
        'VLM 설치/연결 준비': 'VLM Setup and Connection Readiness',
        'local runtime과 cloud opt-in 후보를 dry-run으로 비교합니다.': 'Compare local runtime and cloud opt-in candidates in dry-run mode.',
        'VLM 준비': 'Open VLM Readiness',
        'profile 저장 지원': 'Profile storage supported',
        'runtime 호출 없음': 'No runtime calls',
        '채널 목록, source detail, 입력 준비, PublishedView, audit을 작업 단위로 관리합니다.': 'Manage channels, source details, input readiness, PublishedViews, and audits as task units.',
        'Onboarding 품질': 'Onboarding Quality',
        '저장 전 validation 이슈가 없습니다.': 'No pre-save validation issues.',
        '감지': 'Detection',
        '미설정': 'Not configured',
        '진입': 'Enter',
        '이탈': 'Exit',
        '라인 통과 후 영역 침입': 'Zone intrusion after line crossing',
        '영역 점유': 'Zone occupancy',
        '사용자, 초대, 승인, role/scope, audit 흐름을 작업 단위로 관리합니다.': 'Manage users, invites, approvals, role/scope, and audits as task units.',
        '로그인 가능': 'Login allowed',
        '변경 없음': 'No changes',
        'Source, runtime, event evidence를 같은 진단 흐름에서 판독합니다.': 'Review source, runtime, and event evidence in one diagnostic flow.',
        '런타임 추세': 'Runtime Trend',
        '현재 페이지 세션의 refresh sample만 baseline/sparkline 후보로 봅니다.': 'Only refresh samples from the current page session are used for baseline and sparkline candidates.',
        'longrun evidence 아님': 'Not long-run evidence',
        '세션 변동 없음': 'No session change',
        '스트림 변동 없음': 'No stream change',
        '분석 변동 없음': 'No analysis change',
        '메타데이터 변동 없음': 'No metadata change',
        '관리자 preview': 'Admin preview',
        'viewer-safe 경계 확인': 'Viewer-safe boundary review',
        '인시던트 상태': 'Incident Status',
        '이 view는 이벤트 요약 권한이 꺼져 있어 상태와 신호만 표시합니다.': 'This view has event-summary access disabled and shows only status and signal information.',
        '소스 상태 digest': 'Source Status Digest',
        '허용된 소스 상태와 연결 요약만 표시됩니다.': 'Only permitted source status and connection summaries are shown.',
        'locator 숨김': 'Locator hidden',
        '정비 상태 digest': 'Maintenance Status Digest',
        '정비/복구/미제공 요약만 표시됩니다.': 'Only maintenance, recovery, and unavailable summaries are shown.',
        'raw 숨김': 'Raw data hidden',
        '운영 notice': 'Operations Notice',
        '운영 상태와 timeline hint만 표시됩니다.': 'Only operations status and timeline hints are shown.',
        '상태와 일정만 표시됩니다.': 'Only status and schedule information are shown.',
        '영향 forecast': 'Impact Forecast',
        'source/view/command plan 영향이 안전 요약으로 표시됩니다.': 'Source, view, and command-plan impacts are shown as a safe summary.',
        'command detail 숨김': 'Command details hidden',
        '허용된 이벤트 요약만 표시됩니다.': 'Only permitted event summaries are shown.',
        'clip path 숨김': 'Clip path hidden',
        '이벤트 digest': 'Event Digest',
        '이벤트 digest 없음': 'No Event Digest',
        '표시할 viewer-safe event digest가 없습니다.': 'No viewer-safe event digest is available.',
        '판정 digest': 'Resolution Digest',
        '허용된 판정 상태 요약만 표시됩니다.': 'Only permitted resolution-status summaries are shown.',
        'operator note 숨김': 'Operator note hidden',
        '판정 digest 없음': 'No Resolution Digest',
        '표시할 viewer-safe resolution digest가 없습니다.': 'No viewer-safe resolution digest is available.',
        '인시던트 요약': 'Incident Summary',
        '안전 요약만 표시됩니다.': 'Only safe summaries are shown.',
        '인시던트 요약 없음': 'No Incident Summary',
        '표시할 viewer-safe digest가 없습니다.': 'No viewer-safe digest is available.',
        '후속 조치 요약': 'Follow-up Summary',
        '허용된 view의 상태만 표시됩니다.': 'Only status for permitted views is shown.',
        '후속 조치 요약 없음': 'No Follow-up Summary',
        '표시할 viewer-safe follow-up digest가 없습니다.': 'No viewer-safe follow-up digest is available.',
        'source:write scope 확인됨. 채널 생성/수정/삭제를 수행할 수 있습니다.': 'source:write scope confirmed. Channel create/edit/delete is allowed.',
        '읽기 전용 범위입니다. ops:read로 채널 조회만 가능하며 source:write가 필요한 생성/수정/삭제 UI는 잠깁니다.': 'Read-only scope. ops:read can list channels; create/edit/delete UI stays locked without source:write.',
        '운영 콘솔': 'Ops console',
        '룰 관리': 'Rule management',
        '개발/검증 API': 'Lab/verification API',
        '범위 없음': 'No scopes',
        '미배정 채널': 'Unassigned channel',
        '조회': 'Read',
        '저장된 VLM observation 후보를 이벤트 템플릿 폼 초안으로만 가져옵니다.': 'Import saved VLM observation candidates only as event-template form drafts.',
        '후보 종류': 'Candidate kind',
        'VLM rule draft 후보 종류': 'VLM rule draft candidate kind',
        '후보 새로고침': 'Refresh candidates',
        '후보를 불러오는 중입니다.': 'Loading candidates.',
        '수동 저장 가능한 VLM rule draft 후보가 없습니다.': 'No manual-save VLM rule draft candidates are available.',
        'VLM rule draft 후보를 불러오지 못했습니다.': 'Failed to load VLM rule draft candidates.',
        'VLM rule draft 후보 로드 실패': 'Failed to load VLM rule draft candidates',
        '후보 요약 없음': 'No candidate summary',
        '저장 전 registry write 없음': 'No registry write before save',
        'draftEventId와 whatIfPreview=1 query가 있으면 selected incident 저장 전 preview context를 표시합니다. 저장은 운영자가 수동으로 실행해야 합니다.': 'When draftEventId and whatIfPreview=1 are present, selected-incident preview context is shown before save. An operator must save manually.',
        'draftEventId와 whatIfPreview=1 query가 있으면 selected incident preview context를 표시합니다. 저장은 운영자가 수동으로 실행해야 합니다.': 'When draftEventId and whatIfPreview=1 are present, selected-incident preview context is shown. An operator must save manually.',
        'approvalDraft=1 query가 있으면 저장 전 approval state, validation summary, staged draft context를 표시합니다.': 'When approvalDraft=1 is present, approval state, validation summary, and staged draft context are shown before save.',
        '저장은 기존 `/ops/rules` 수동 저장 버튼에서만 수행됩니다.': 'Save is performed only with the existing `/ops/rules` manual save button.',
        'rule write 없음': 'no rule write',
        'rule write 확인 필요': 'rule write check required',
        'approvalDraft 대기': 'approvalDraft pending',
        '표시할 Approval-gated Rule Draft Readiness가 없습니다.': 'No Approval-gated Rule Draft Readiness is available.',
        'EventRecord와 review state를 불러오면 approval-gated staged draft readiness가 표시됩니다.': 'Approval-gated staged draft readiness appears after EventRecord and review state load.'
      }));
      const translatePattern = text => {
        ...
        const rules = [
          ...
          [/^(\d+)개 범위\s+([\s\S]+)$/u, (_match, count, detail) => `${count} scopes\n${koToEn.get(detail.trim()) || translatePattern(detail.trim())}`],
          [/^(\d+)개 범위$/u, (_match, count) => `${count} scopes`],
          [/^수동 저장 후보\s+(\d+)\/(\d+)개$/u, (_match, count, matched) => `Manual-save candidates ${count}/${matched}`],
          [/^자동 적용 제외\s+(\d+)개$/u, (_match, count) => `Auto-apply excluded ${count}`],
          [/^(.+)\s+외\s+(\d+)개$/u, (_match, preview, extra) => `${String(preview).split(', ').map(part => koToEn.get(part.trim()) || translatePattern(part.trim())).join(', ')} + ${extra} more`],
          [/^(\d+)개 source\s*\/\s*ready\s+(\d+)\s*\/\s*warning\s+(\d+)\s*\/\s*blocked\s+(\d+)$/u, (_match, total, ready, warning, blocked) => `${total} sources / ready ${ready} / warning ${warning} / blocked ${blocked}`],
