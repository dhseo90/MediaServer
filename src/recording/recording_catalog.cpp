...
#undef MOVE_MAP
    ...
    std::swap(recovery_report_,source.recovery_report_);generation_backend_=true;
    generation_read_only_=!options_.enable_generation_writes;opened_=true;
}
bool RecordingCatalog::OpenGenerationLocked(std::string* error) {
    ...
    try {
        if(options_.enable_generation_writes&&!journal_.EnableGenerationWrites(this,error))return failed();
...
bool RecordingCatalog::PrepareGenerationSqliteLocked(const std::shared_ptr<RecordingGenerationRecoverySession>& session,std::string* error) {
    catalog_mode_=options_.enable_generation_writes?"generation-jsonl":"generation-jsonl-readonly";
...
#undef CURRENT_ROWS
    ...
    catalog_mode_=options_.enable_generation_writes?"generation-sqlite":"generation-sqlite-readonly";return true;
#else
...
bool RecordingCatalog::UpdateDerivedJob(const void* owner,const DerivedJobRecordV1& record,std::string* error) {
    ...
    if(!AppendAndApplyLocked(std::move(mutation),error,&prepared)){if(!generation_backend_)derived_job_state_authoritative_=false;return false;}
    switch(record.state){
...
bool RecordingCatalog::DerivedJobProtectsLocked(const std::string& id) const {
    ...
    for(const auto& [_,job]:derived_jobs_){if(!job)return true;if(job.Active()) {
        for(const auto& source:job.source_ids)if(source==id)return true;
        for(const auto& output:job.output_ids)if(output==id)return true;
    }}
    return false;
}
...
bool RecordingCatalog::PreparedDerivedMatchesLocked(const RecordingMutationV1& mutation,const PreparedDerivedMutation& prepared,bool applied,std::string* error) const {
    ...
    if(old==derived_jobs_.end()||!old->second||(!applied&&(old->second.WarmOwned()!=prepared.prior||old->second.state!=prepared.prior_state||old->second.files!=prepared.prior_files))||
       (applied && old->second.WarmOwned()!=prepared.applied))return Fail(error,"derived prepared prior 결박 거부");
    return true;
}
RecordingCatalog::DerivedJobHandle RecordingCatalog::ShareValidatedJob(DerivedJobRecordV1 record,const DerivedJobPool* pool) {
    if(pool){
        ...
        const auto owned=found==pool->end()?DerivedJobHandle{}:found->second.WarmOwned();
        if(owned&&owned->state==record.state&&owned->files.size()==record.files.size()&&
           SerializeDerivedJobRecord(*owned)==SerializeDerivedJobRecord(record))return owned;
    }
...
bool RecordingCatalog::FailDerivedJobAfterCleanup(const std::string& id,const std::string& attempt,
    ...
    DerivedJobHandle found;if(!AcquireDerivedJobOwnedLocked(id,&found,error)||!found||found->intent.attempt_id!=attempt)return Fail(error,"derived job cleanup 소유권 거부");
    auto record=*found;record.state=DerivedJobState::Failed;record.failure_reason=reason;record.cleaned_at_ms=cleaned;
    const auto payload=SerializeDerivedJobRecord(record);
    if(payload.empty())return Fail(error,"derived job cleanup payload 거부");
    if(found->state==DerivedJobState::Failed)return SerializeDerivedJobRecord(*found)==payload;
    if(found->state!=DerivedJobState::Intent)return Fail(error,"derived job cleanup 전이 거부");
    ...
    if(!AppendAndApplyLocked(std::move(mutation),error)){if(!generation_backend_)derived_job_state_authoritative_=false;return false;}return true;
}
...
std::vector<std::string> RecordingCatalog::ProjectionSignatureLocked() const {
    ...
    for(const auto& [id,entry]:source_bindings_){(void)entry;SourceBindingHandle v;if(!AcquireSourceBindingOwnedLocked(id,&v,nullptr)||!v)throw std::runtime_error("binding projection unavailable");const auto bytes=SerializeRecordingSourceBindingV1(*v);if(bytes.empty())throw std::runtime_error("binding projection invalid");add("source-binding",id,bytes);}
    for(const auto& [id,entry]:derived_jobs_){(void)entry;DerivedJobHandle v;if(!AcquireDerivedJobOwnedLocked(id,&v,nullptr)||!v)throw std::runtime_error("job projection unavailable");const auto bytes=SerializeDerivedJobRecord(*v);if(bytes.empty())throw std::runtime_error("job projection invalid");add("derived-job",id,bytes);}
...
bool RecordingCatalog::CheckpointLocked(bool recover_only,std::string* error,const DerivedJobContentProof* proof) {
    ...
    bool identical=false;
    {
        ...
        if(reuse)for(std::size_t i=0;i<cached->prefix.size();++i){
            bool matches=false;
            if(!journal_.MatchMutationLinkView(cached->prefix[i],views[i],&matches,error))return false;
            if(!matches){reuse=false;break;}
        }
        ...
        candidate_views.resize(candidate.size());
        // 변경 receipt에는 이전 입력의 출처를 붙이지 않는다. 같은 행의 전체 필드가 같은 경우만 전달한다.
        for(std::size_t i=0;i<candidate.size()&&i<original.size();++i){
            if(!candidate[i]||!original[i])continue;
            const auto& a=*candidate[i];const auto& b=*original[i];
    ...
    if(!identical){
        ...
        for(std::size_t i=0;i<candidate.size();++i){const auto& m=candidate[i];
            if(!m||!after->ApplyMutationLocked(*m,false,error,nullptr,m,&source_bindings_,&derived_jobs_,proof,candidate_views[i])){
                if(!after->derived_job_state_authoritative_)derived_job_state_authoritative_=false;
                return false;
            }}
        try {
            if(before->ProjectionSignatureLocked()!=after->ProjectionSignatureLocked())return Fail(error,"checkpoint 투영 불일치");
        }catch(...){
            // cold 상세 실패만 live의 불확실 상태로 전달한다. 다른 Apply 예외의 기존
            // unwind/cache 폐기·다음 호출 full 검증 계약까지 변경하지 않는다.
            if(before->derived_job_state_authoritative_&&after->derived_job_state_authoritative_)throw;
            derived_job_state_authoritative_=false;
            return Fail(error,"checkpoint 상세 투영 미확인");
        }
    }
    ...
    if(!recover_only&&detail::CheckpointCacheAdmissible(candidate)){
        ...
        for(std::size_t i=0;i<candidate.size();++i){
            ...
            if(!journal_.MakeMutationLink(candidate_views[i],*candidate[i],candidate[i],&link,error))return false;
            next->prefix.push_back(std::move(link));
        }
    }
...
bool RecordingCatalog::ReadCatalogReplay(RecordingMutationHandles* owned,RecordingJournalReplayResult* replay,
                                         std::string* error,RecordingJournalOwnedViews* views) const {
    if(!owned||!replay)return Fail(error,"catalog replay output 없음");
    owned->clear();*replay={};if(views)views->clear();
    ...
    if(!journal_.ReadCheckpointRecords(this,owned,error,nullptr,views)){++replay->io_error_count;return false;}
...
bool RecordingCatalog::ValidateMutationLocked(const RecordingMutationV1& mutation,std::string* error,PreparedDerivedMutation* prepared) {
    ...
    if(parsed.schema!=mutation.schema||parsed.mutation_id!=mutation.mutation_id||parsed.entity_id!=mutation.entity_id||
       parsed.mutation_type!=mutation.mutation_type||parsed.occurred_at_ms!=mutation.occurred_at_ms||
       parsed.payload_json!=mutation.payload_json)return Fail(error,"preappend envelope 정규화 불일치");
    if(!journal_.ValidatePreappend(this,parsed,error))return false;
    if(prepared)return PreparedDerivedMatchesLocked(parsed,*prepared,false,error);
...
bool RecordingCatalog::ApplyMutationLocked(const RecordingMutationV1& mutation,
    ...
    const bool segment_state = mutation.mutation_type == RecordingMutationType::ReferencedObservationPut ||
                               ...
                               mutation.mutation_type == RecordingMutationType::SegmentV2BoundFinalized ||
                               mutation.mutation_type == RecordingMutationType::CorruptionDetected;
    if(apply&&segment_state&&!owned&&!generation_row)owned=std::make_shared<const RecordingMutationV1>(mutation);
    RecordingMutationLink accepted_link;
    if(apply&&generation_row) {
#if MEDIA_SERVER_ENABLE_RECORDING_GENERATION_BACKEND
    ...
    switch (mutation.mutation_type) {
        ...
        case RecordingMutationType::ConsumerReferencePut: {
            ...
            if(ok) {
                ...
                if(mutation.mutation_type==RecordingMutationType::DerivedReferenceAccepted) {
                    ...
                    if(ok&&apply){derived_accepted_references_.insert(reference.reference_id);touch("derived-reference-accepted",reference.reference_id);}
                } else {
                    ...
                    if(ok&&apply){const auto id=reference.reference_id;consumer_references_.emplace(id,std::move(reference));touch("consumer-reference",id);}
                }
            }
        ...
        case RecordingMutationType::DeletionCompleted: {
            ...
            if (ok&&apply) {
                ...
                touch("tombstone-v1",tombstone.segment_id);touch("segment-v1",tombstone.segment_id);
                touch("media-path",tombstone.segment_id);touch("hold",tombstone.segment_id);touch("deletion-reason",tombstone.segment_id);
            }
            break;
        }
...
bool RecordingCatalog::AppendGenerationLocked(RecordingMutationV1 mutation,std::string* error,PreparedDerivedMutation* prepared) {
    ...
    if(row->retry){if(error)error->clear();return true;}
    try {
#if MEDIA_SERVER_RECORDING_GENERATION_TESTING
        const int fault=generation_apply_fault_;generation_apply_fault_=0;
        if(fault==1){Fail(error,"B test current apply 실패");return PoisonGenerationLocked(error);}
        if(fault==2)throw std::runtime_error("B test post-fsync exception");
#endif
...
bool RecordingCatalog::ReserveRecordingOrder(const std::string& store,const std::string& request,
    ...
    recording::latency::Lock lock(mu_,recording::latency::Source::Catalog,__LINE__);
    if(!generation_backend_)return journal_.ReserveRecordingOrder(store,request,segment,channel,result,error);
#if MEDIA_SERVER_ENABLE_RECORDING_GENERATION_BACKEND && MEDIA_SERVER_USE_OPENSSL && !defined(_WIN32)
    ...
    try {
        if(!row->retry) {
            GenerationDelta delta;
            if(!ApplyMutationLocked(row->mutation,false,error,nullptr,{},nullptr,nullptr,nullptr,{},row.get(),true,&delta)||
               !ProjectGenerationDeltaLocked(delta,*row,error))return PoisonGenerationLocked(error);
        }
        *result=std::move(order);if(error)error->clear();return true;
    }catch(...){Fail(error,"B durable 예약 투영 예외");return PoisonGenerationLocked(error);}
#else
    (void)store;(void)request;(void)segment;(void)channel;(void)result;return Fail(error,"B 예약 unsupported");
#endif
}
bool RecordingCatalog::ProjectGenerationDeltaLocked(const GenerationDelta& delta,const RecordingGenerationRecoveryRow& row,std::string* error) {
#if MEDIA_SERVER_ENABLE_RECORDING_GENERATION_BACKEND && MEDIA_SERVER_USE_SQLITE3 && !defined(_WIN32)
    auto* db=generation_sqlite_db_;
    if(!db)return journal_.CommitGenerationDelta(this,[]{return true;},error);
    if(!journal_.ValidateGenerationOwner(this,error)||!Exec(db,"BEGIN IMMEDIATE",error))return false;
    const auto fail=[&](){Exec(db,"ROLLBACK",nullptr);return false;};
    try {
        const auto statement=[&](const char* sql,const std::vector<std::string>& values) {
            sqlite3_stmt* query=nullptr;
            if(sqlite3_prepare_v2(db,sql,-1,&query,nullptr)!=SQLITE_OK)return Fail(error,sqlite3_errmsg(db));
            bool ok=true;int at=1;
            for(const auto& value:values)if(sqlite3_bind_text(query,at++,value.data(),static_cast<int>(value.size()),SQLITE_TRANSIENT)!=SQLITE_OK)ok=false;
            ok=ok&&sqlite3_step(query)==SQLITE_DONE;sqlite3_finalize(query);
            return ok?true:Fail(error,sqlite3_errmsg(db));
        };
...
#define DELTA_VALUE(map,serializer) do {const auto i=map.find(id);if(i!=map.end()){present=true;value=serializer(i->second);}}while(false)
            ...
            else if(kind=="referenced-observation")DELTA_VALUE(referenced_observations_,SerializeReferencedObservationV1);
            else if(kind=="media-path"||kind=="deletion-reason") {
...
#undef DELTA_VALUE
            ...
            if(kind=="source-binding"||kind=="derived-job"||kind=="hold") {
                const char* upsert=kind=="source-binding"?"INSERT INTO b_source_summary VALUES(?,?) ON CONFLICT(id) DO UPDATE SET summary_json=excluded.summary_json":
                    ...
                    "INSERT INTO b_hold VALUES(?,?) ON CONFLICT(id) DO UPDATE SET count=excluded.count";
                const char* remove=kind=="source-binding"?"DELETE FROM b_source_summary WHERE id=?":kind=="derived-job"?"DELETE FROM b_job_summary WHERE id=?":"DELETE FROM b_hold WHERE id=?";
                if(!statement(present?upsert:remove,present?std::vector<std::string>{id,value}:std::vector<std::string>{id}))return fail();
            } else if(!statement(present?"INSERT INTO b_current VALUES(?,?,?) ON CONFLICT(kind,id) DO UPDATE SET value_json=excluded.value_json":"DELETE FROM b_current WHERE kind=? AND id=?",
                present?std::vector<std::string>{kind,id,value}:std::vector<std::string>{kind,id}))return fail();
        }
        const auto& m=row.mutation;
        if(!statement("INSERT INTO b_identity VALUES(?,?,?,?,?,?)",{m.mutation_id,RecordingMutationTypeName(m.mutation_type),m.entity_id,
            std::to_string(m.occurred_at_ms),std::to_string(row.global_ordinal),row.identity}))return fail();
        if(!journal_.CommitGenerationDelta(this,[&]{return Exec(db,"COMMIT",error);},error))return fail();
        return true;
    }catch(...){Exec(db,"ROLLBACK",nullptr);throw;}
#else
    (void)delta;(void)row;return journal_.CommitGenerationDelta(this,[]{return true;},error);
#endif
}
...
bool RecordingCatalog::AppendAndApplyLocked(RecordingMutationV1 mutation, std::string* error,PreparedDerivedMutation* prepared) {
    ...
    if(generation_backend_)return AppendGenerationLocked(std::move(mutation),error,prepared);
    if(prepared&&!PreparedDerivedMatchesLocked(mutation,*prepared,false,error))return false;
    mutation.mutation_id = mutation.mutation_id.empty() ? NextMutationId() : mutation.mutation_id;
...
bool RecordingCatalog::PreflightV2Locked(const RecordingJournalReplayResult& replay,std::string* error,
                                       ...
                                       const RecordingSourceBindingV1* binding,const RecordingMutationHandles& owned,const RecordingJournalOwnedViews& views) const {
    if(!owned.empty()&&owned.size()!=replay.mutations.size())return Fail(error,"preflight owned 기록 개수 불일치");
    if(!views.empty()&&views.size()!=replay.mutations.size())return Fail(error,"preflight view 기록 개수 불일치");
    ...
    for(std::size_t ordinal=0;ordinal<replay.mutations.size();++ordinal) {
        ...
        const bool accepted=scratch.ApplyMutationLocked(m,false,error,nullptr,owned.empty()?RecordingMutationHandle{}:owned[ordinal],nullptr,nullptr,nullptr,views.empty()?RecordingJournalOwnedViewHandle{}:views[ordinal]);
...
RecordingCatalog::SourceBindingHandle RecordingCatalog::FindSourceBindingOwned(const SourceBindingPool& pool,const std::string& id) {
    const auto found=pool.find(id);return found==pool.end()?SourceBindingHandle{}:found->second.WarmOwned();
}
...
bool RecordingCatalog::AcquireDerivedJobOwnedWithEnvelopeLocked(const std::string& id,DerivedJobHandle* out,RecordingMutationHandle* envelope,std::string* error) const {
    ...
    try {
        ...
        const bool state_matches=(record.state==DerivedJobState::Intent&&(type==RecordingMutationType::DerivedJobIntent||type==RecordingMutationType::DerivedJobFiles))||
            (record.state==DerivedJobState::Ready&&type==RecordingMutationType::DerivedJobReady)||
            (record.state==DerivedJobState::Committed&&type==RecordingMutationType::DerivedJobCommitted)||
            (record.state==DerivedJobState::Complete&&type==RecordingMutationType::DerivedJobComplete)||
            (record.state==DerivedJobState::Failed&&type==RecordingMutationType::DerivedJobFailed);
        if(!state_matches||record.intent.job_id!=entry.id||record.intent.reference.channel_id!=entry.channel||record.intent.reference.reference_id!=entry.reference||
...
bool RecordingCatalog::ReleaseInactiveDetailsLocked(const std::string* changed,std::string* error) {
    try {
        // shadow에는 owner를 저장하지 않는다. 현재 호출의 live 상태로 활성 보호를 결정한다.
        const auto release=[&](RecordingCatalog& target,const std::string& id){
            const auto binding=target.source_bindings_.find(id);
            if(binding!=target.source_bindings_.end()&&journal_.CanReleaseMutationLink(binding->second.mutation))binding->second.resident.reset();
            const auto job=target.derived_jobs_.find(id);if(job==target.derived_jobs_.end())return;
            const auto live=derived_jobs_.find(id);
            const bool active=live!=derived_jobs_.end()&&live->second&&live->second.Active();
            if(!active&&journal_.CanReleaseMutationLink(job->second.mutation))job->second.resident.reset();
        };
        if(changed){release(*this,*changed);if(checkpoint_cache_&&checkpoint_cache_->shadow)release(*checkpoint_cache_->shadow,*changed);}
        else {
            for(const auto& [id,entry]:source_bindings_){(void)entry;release(*this,id);}
            for(const auto& [id,entry]:derived_jobs_){(void)entry;release(*this,id);}
            if(checkpoint_cache_&&checkpoint_cache_->shadow){auto& shadow=*checkpoint_cache_->shadow;
                for(const auto& [id,entry]:shadow.source_bindings_){(void)entry;release(shadow,id);}
                for(const auto& [id,entry]:shadow.derived_jobs_){(void)entry;release(shadow,id);}}
        }
...
bool RecordingCatalog::ResolveOriginalSample(const std::string& channel,const std::string& source,
    ...
    recording::latency::Lock lock(mu_,recording::latency::Source::Catalog,__LINE__);
    if(!opened_||!derived_job_state_authoritative_||(generation_backend_&&!CanReadLocked(error)))return Fail(error,"source lookup 미open/미확인");
    for(const auto& [id,handle]:source_bindings_) {
        ...
        if(segment==segments_v2_.end()||EffectiveLifecycleV2Locked(id)!=RecordingLifecycle::Finalized||
           handle.channel!=channel||handle.source!=source||handle.generation!=generation||handle.order!=order||handle.track!=track)continue;
        SourceBindingHandle owned;if(!AcquireSourceBindingOwnedLocked(id,&owned,error)||!owned){*result={};return false;}const auto& b=*owned;
...
bool RecordingCatalog::SnapshotLocationsV2(const std::string& channel, RecordingLocationCatalogSnapshot* result, std::string* error) const {
    ...
    recording::latency::Lock lock(mu_,recording::latency::Source::Catalog,__LINE__);
    if(!opened_||(generation_backend_&&!CanReadLocked(error)))return Fail(error,"catalog not open");
...
bool RecordingCatalog::AcceptDerivedReference(const RecordingConsumerReferenceV1& reference, std::string* error) {
    ...
    if(!AppendAndApplyLocked(std::move(mutation),error)) {
        if(!generation_backend_)derived_job_state_authoritative_=false;
        return false;
    }
    return true;
}
...
std::vector<RecordingConsumerReferenceV1> RecordingCatalog::QueryConsumerReferences(
    ...
    if(!opened_||!options_.enable_v2_storage||(generation_backend_&&!CanReadLocked(nullptr))||!ValidateRecordingReferenceId(channel,nullptr)||!ValidateOpaqueId(owner,nullptr)||
...
std::vector<AnalysisObservationV2> RecordingCatalog::QueryObservationsV2(const std::string& channel_id) const {
    ...
    if(generation_backend_&&!CanReadLocked(nullptr))return output;
...
bool RecordingCatalog::AdjustHoldCountLocked(const std::string& segment_id,std::int64_t delta,std::string* error) {
    ...
    if(generation_backend_&&!UpdateGenerationHoldsLocked({{segment_id,next}},error))return false;
#if MEDIA_SERVER_USE_SQLITE3
    if (sqlite_db_ != nullptr) {
        sqlite3_stmt* statement = nullptr;
        if (sqlite3_prepare_v2(sqlite_db_,
    ...
    for (std::size_t ordinal = 0; ordinal < replay.mutations.size(); ++ordinal) {
        ...
        if (mutation.mutation_type == RecordingMutationType::SegmentFinalized ||
            ...
            if (accepted_segment_state_replay_ordinals_.count(ordinal) == 0 ||
                accepted == accepted_segment_state_mutations_.end()) continue;
            RecordingMutationHandle accepted_owned;
            bool current_view=false;
            if(!views.empty()&&!journal_.MatchMutationLinkView(accepted->second,views[ordinal],&current_view,error))return false;
            if(current_view)accepted_owned=owned[ordinal];
